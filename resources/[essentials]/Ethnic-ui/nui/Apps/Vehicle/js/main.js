...
CloseVehicleMenu = function() {
    $.post('https://Ethnic-vehicles/Vehicle/Close', JSON.stringify({}));
...
$(document).on('click', '.vehicle-button', function(Events) {
    ...
    if (!HasClass) {
        if (Type == 'Seat1' || Type == 'Seat2' || Type == 'Seat3' || Type == 'Seat4') {
            $.post('https://Ethnic-vehicles/Vehicle/Action', JSON.stringify({Action: 'Seat', Number: Number(TNumber)}));
        } else if (Type == 'WLeftF' || Type == 'WRightF' || Type == 'WLeftB' || Type == 'WRightB') {
            $.post('https://Ethnic-vehicles/Vehicle/Action', JSON.stringify({Action: 'Window', Number: Number(TNumber)}));
        } else if (Type == 'DLeftF' || Type == 'DRightF' || Type == 'DLeftB' || Type == 'DRightB') {
            $.post('https://Ethnic-vehicles/Vehicle/Action', JSON.stringify({Action: 'Door', Number: Number(TNumber)}));
        }
    }
});
