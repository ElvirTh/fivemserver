...
CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-vehicles/server/get-vehicle-position', function(Source, Cb, Plate)
    ...
    CallbackModule.CreateCallback('Ethnic-vehicles/server/get-garage-vehs', function(Source, Cb, Garage)
    ...
    CallbackModule.CreateCallback('Ethnic-vehicles/server/can-spawn-vehicle', function(Source, Cb, Plate)
        DatabaseModule.Execute('SELECT * FROM player_vehicles WHERE plate = ?', {
            Plate
        }, function(Vehicle)
            if Vehicle[1] ~= nil then
    ...
    CallbackModule.CreateCallback('Ethnic-vehicles/server/get-veh-by-plate', function(Source, Cb, Plate)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    CallbackModule.CreateCallback('Ethnic-vehicles/server/can-retreive-veh', function(Source, Cb, Plate)
        DatabaseModule.Execute('SELECT * FROM player_vehicles WHERE plate = ?', {
            Plate
        }, function(Vehicle)
            if Vehicle[1] ~= nil then
    ...
    CallbackModule.CreateCallback('Ethnic-vehicles/server/pay-release-fee', function(Source, Cb, Plate, Amount)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    CallbackModule.CreateCallback('Ethnic-vehicles/server/get-depot-vehicles', function(Source, Cb)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local DepotVehs = {}
    ...
    CallbackModule.CreateCallback('Ethnic-vehicles/client/get-recent-depots', function(Source, Cb)
        Cb(RecentlyImpounded)
    end)

    CallbackModule.CreateCallback('Ethnic-vehicles/server/is-veh-owner', function(Source, Cb, Plate)
...
RegisterNetEvent("Ethnic-vehicles/server/send-message-to-impound", function()
    for k, v in pairs(PlayerModule.GetPlayers()) do
        local Player = PlayerModule.GetPlayerBySource(v)
        if exports['Ethnic-business']:IsPlayerInBusiness(Player, 'Los Santos Depot') then
            TriggerClientEvent('Ethnic-chat/client/post-message', Player.PlayerData.Source, "Help is required at the depot!", "normal")
        end
    end
end)

RegisterNetEvent("Ethnic-vehicles/server/park-vehicle", function(VehNet, CurrentGarage)
...
RegisterNetEvent("Ethnic-vehicles/server/set-veh-state", function(Plate, State, NetId)
    local src = source
...
RegisterNetEvent("Ethnic-vehicles/server/depot-vehicle", function(NetId, ImpoundId, InputResult)
