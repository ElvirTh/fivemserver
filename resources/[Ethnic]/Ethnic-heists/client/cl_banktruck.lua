RegisterNetEvent('Ethnic-items/client/used-thermite-charge', function()
    ...
    if exports['Ethnic-police']:GetTotalOndutyCops() >= Config.BanktruckCops then
        local CanRob = CallbackModule.SendCallback('Ethnic-heists/server/banktruck/can-rob-truck', NetworkGetNetworkIdFromEntity(Entity))
        if not CanRob then return exports['Ethnic-ui']:Notify("banktruck-error", "Already burnt..", "error") end
        Citizen.SetTimeout(450, function()
            ...
            if DidRemove then
                ...
                if Success then
                    EventsModule.TriggerServer("Ethnic-heists/server/banktruck/set-truck-state", NetworkGetNetworkIdFromEntity(Entity), true)
                    EventsModule.TriggerServer('Ethnic-ui/server/send-banktruck-rob', FunctionsModule.GetStreetName())
                end
            end
        end)
    else
        exports['Ethnic-ui']:Notify("banktruck-error", "You can't do this now..", "error")
    end
end)

RegisterNetEvent("Ethnic-heists/client/banktruck/remove-truck", function(NetId)
    local Vehicle = NetworkGetEntityFromNetworkId(NetId)
    EntityModule.DeleteEntity(Vehicle)
end)

RegisterNetEvent("Ethnic-heists/client/banktruck/setup", function(NetId)
    local Vehicle = NetworkGetEntityFromNetworkId(NetId)
        ...
        while RobbingTruck do
            ...
            if Distance < 3.0 then
                if not ShowingInteraction then
                    ...
                    exports['Ethnic-ui']:SetInteraction('[E] Bank Truck')
                end
                if IsControlJustPressed(0, 38) then
                    if not exports['Ethnic-ui']:IsProgressBarActive() then
                        local Stealing = true
                        RobbingTruck = false
                        ...
                        exports['Ethnic-ui']:ProgressBar('Stealing..', math.random(34000, 58000), {['AnimName'] = 'grab', ['AnimDict'] = "anim@heists@ornate_bank@grab_cash_heels", ['AnimFlag'] = 16}, "HeistBag", true, true, function(DidComplete)
                            if DidComplete then
                                Stealing = false
                            end
                        end)
                        Citizen.CreateThread(function()
                            while Stealing do
                                EventsModule.TriggerServer('Ethnic-ui/server/set-stress', 'Add', math.random(2, 3))
                                EventsModule.TriggerServer("Ethnic-heists/server/banktruck/receive-goods")
                                Citizen.Wait(6500)
                            end
                        end)
                    end
