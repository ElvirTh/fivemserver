...
Citizen.CreateThread(function()
    ...
    EventsModule.RegisterServer('Ethnic-business/server/foodchain/create-meal', function(Source, ItemName)
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/foodchain/get-payment-data', function(Source, Cb, Business, RegisterId)
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/get-clocked-in-employees', function(Source, Cb, Business)
...
RegisterNetEvent('Ethnic-business/server/foodchain/set-duty', function(Data)
    local src = source
    if Data.Clocked then
        ...
        TriggerClientEvent('Ethnic-business/client/foodchain/set-duty-data', -1, Data.Business, Config.ActiveEmployees[Data.Business])	
    else
        ...
        if  ClockedData then 
            if Config.ActiveEmployees[ClockedData.Business] ~= nil then 
                ...
                TriggerClientEvent('Ethnic-business/client/foodchain/set-duty-data', -1, Data.Business, Config.ActiveEmployees[ClockedData.Business])	
            end
        else
            print('[DEBUG:FoodChain]: Something went wrong while trying to clock out..')
        end
    end
    TriggerClientEvent('Ethnic-business/client/foodchain/set-duty', src, Data)
end)


RegisterNetEvent("Ethnic-business/server/foodchain/add-to-register", function(Business, RegisterId, Cost, Order)
    local src = source
    Config.ActivePayments[Business][RegisterId] = {Creator = src, Costs = Cost, Order = Order}
end)

RegisterNetEvent("Ethnic-business/server/foodchain/pay-register", function(Data)
    local src = source
    ...
    if Payment ~= nil then
        if Data.PaymentType == 'Bank' then
            if Player.Functions.RemoveMoney('Bank', Payment.Costs, 'FoodChain-Order-Bank') then
                TriggerClientEvent('Ethnic-business/client/foodchain/receive-receipt', Payment.Creator, Data['Foodchain'], Payment)
