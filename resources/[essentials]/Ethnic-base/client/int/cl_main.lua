	...
	while true do
		...
		if NetworkIsSessionStarted() and not Spawned then
			...
			SetTimeout(500, function()
				TriggerEvent('Ethnic-base/client/player-spawned')
				TriggerServerEvent('Ethnic-base/server/load-token')
				TriggerServerEvent('Ethnic-base/server/load-user')
...
function RequestSyncExecution(Native, Entity, ...)
    if DoesEntityExist(Entity) then
        TriggerServerEvent('Ethnic-base/server/sync-request', Native, GetPlayerServerId(NetworkGetEntityOwner(Entity)), NetworkGetNetworkIdFromEntity(Entity), ...)
    end
end
...
RegisterNetEvent("Ethnic-base/client/sync-execute", function(Native, NetId, ...)
	local VehicleModule = exports['Ethnic-base']:FetchModule('Vehicle')
	local EntityModule = exports['Ethnic-base']:FetchModule('Entity')
...
RegisterNetEvent('Ethnic-base/client/command-gotocoords', function(Coords)
	SetEntityCoords(PlayerPedId(), Coords.x, Coords.y, Coords.z)
end)

RegisterNetEvent('Ethnic-admin/client/command-go-to-marker', function(Coords)
...
RegisterNetEvent('Ethnic-base/client/duty-menu', function()
	local src = source
	local PlayerModule = exports['Ethnic-base']:FetchModule('Player')
	...
	if Player ~= nil then
					...
					['SecondMenu'] = {
						{
							...
							['Data'] = {['Event'] = 'Ethnic-base/server/set-duty', ['Type'] = 'Server', ['State'] = true},
							['CloseMenu'] = true,
						},
						{
							['Title'] = 'Go Off Duty',
							['Desc'] = 'Use this to go off duty.',
							['Data'] = {['Event'] = 'Ethnic-base/server/set-duty', ['Type'] = 'Server', ['State'] = false},
							['CloseMenu'] = true,
						}
					}
				}
		...
		exports['Ethnic-ui']:Notify("player-error", "Something went wrong..", "error")
	end
end)

RegisterNUICallback("NUIBlock", function(data)
	local EventsModule = exports['Ethnic-base']:FetchModule('Events')
	EventsModule.TriggerServer('Ethnic-base/server/check-nui')
end)
