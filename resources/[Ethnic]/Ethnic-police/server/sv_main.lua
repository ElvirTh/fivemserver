...
Citizen.CreateThread(function()
    ...
    CommandsModule.Add({"barricade", "placebarricade"}, "Place down", {}, false, function(source, args)
        ...
        TriggerClientEvent('Ethnic-police/client/open-barricademenu', source, Config.BarricadeObjects, ClosestBarricade)
    end)
    ...
    CommandsModule.Add({"cam", "camera", "opencam"}, "Open Camera", {}, false, function(source, args)
        ...
        TriggerClientEvent('Ethnic-police/client/show-camera-input', source)
    end)
    ...
    CommandsModule.Add("311", "Police chat", {{Name="Message", Help="Message"}}, false, function(source, args)
        ...
        TriggerClientEvent('Ethnic-police/client/send-311', -1, MessageData)
    end)
    ...
    CommandsModule.Add("911", "Call emergency services", {{Name="Message", Help="Message"}}, false, function(source, args)
        ...
        TriggerClientEvent('Ethnic-police/client/send-911', source, CallData, false)
        -- TriggerClientEvent('Ethnic-police/client/send-911-chat', -1, CallData, false) -- Enable to have chat alerts
    end)

    CommandsModule.Add("911a", "Call emergency services anonymously", {{Name="Message", Help="Message"}}, false, function(source, args)
        local CallData = {}
        CallData['Message'] = table.concat(args, ' ')
        TriggerClientEvent('Ethnic-police/client/send-911', source, CallData, true)
        -- TriggerClientEvent('Ethnic-police/client/send-911-chat', -1, CallData, true) -- Enable to have chat alerts
    end)
    ...
    --     CallData['Message'] = table.concat(args, ' ')
    --     TriggerClientEvent('Ethnic-police/client/send-reaction-to-dispatch', -1, CallData)
    -- end)
    ...
    --         CallData['Id'] = source
    --         CallData['Message'] = table.concat(args, ' ')
    --         TriggerClientEvent('Ethnic-police/client/send-911-dispatch', Target, CallData, true)
    --     end
    -- end)
    ...
    CommandsModule.Add("department", "View your department", {}, false, function(source, args)
        ...
        TriggerClientEvent('Ethnic-chat/client/post-message', source, 'DEPARTMENT', Player.PlayerData.Job.Department, 'warning')
    end)
    ...
    CommandsModule.Add({"cuff"}, "(Un)cuff a player", {{Name="Id", Help="Player ID"}, {Name="Bool", Help="True or False"}}, true, function(source, args)
        ...
        if TargetPlayer then
            ...
            TriggerClientEvent('Ethnic-police/client/set-cuff-state', TargetPlayer.PlayerData.Source, Bool)
        end
    end, "admin")

    CommandsModule.Add({"fine"}, "Give a fine", {}, true, function(source, args)
        TriggerClientEvent('Ethnic-police/client/give-fine', source)
    end)
    ...
    CommandsModule.Add("jail", "Send a criminal to jail.", {{Name="Id", Help="Id"}, {Name="Time", Help="Time"}, {Name="Parole", Help="Parole"}}, false, function(source, args)
        ...
        if TPlayer then
            if Player.PlayerData.Job.Name ~= 'police' then return end
            TriggerClientEvent('Ethnic-police/client/enter-jail', TPlayer.PlayerData.Source, tonumber(args[2]), args[3], false)
        else
    ...
    CallbackModule.CreateCallback('Ethnic-police/server/get-all-cops-db', function(Source, Cb)
        local Cops = {}
    ...
    Citizen.CreateThread(function()
        while true do 
            ...
            TriggerEvent('Ethnic-police/server/update-cop-count')
        end
    end)

    CallbackModule.CreateCallback('Ethnic-police/server/get-active-cops', function(Source, Cb)
        local CopAmount = 0
    ...
    CallbackModule.CreateCallback('Ethnic-police/server/is-plate-flagged', function(Source, Cb, Plate)
        DatabaseModule.Execute("SELECT * FROM player_vehicles WHERE plate = ? ", {Plate}, function(VehData)
            if VehData ~= nil and VehData[1] ~= nil then
                Cb(VehData[1])
            else
    ...
    CallbackModule.CreateCallback('Ethnic-police/server/get-vehicle-data', function(Source, Cb, Plate)
        DatabaseModule.Execute("SELECT * FROM player_vehicles WHERE plate = ?  ", {Plate}, function(VehData)
            if VehData ~= nil then
                local VehicleData = {}
    ...
    CallbackModule.CreateCallback('Ethnic-police/server/get-garage', function(Source, Cb, Type)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local VehicleList = {}
    ...
    CallbackModule.CreateCallback('Ethnic-police/server/get-vehicle-state', function(Source, Cb, Plate)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer("Ethnic-police/server/receive-evidence", function(Source, ClosestEvidence)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if Player.Functions.RemoveItem("evidence", 1, false, true) then
            for k, v in pairs(ClosestEvidence) do
                if Player.Functions.AddItem('evidence-'..v['Type']:lower(), 1, false, v, true) then
                    Evidence[v['Id']] = {}
                    TriggerClientEvent('Ethnic-police/client/set-evidence', -1, v['Id'], Evidence[v['Id']])
                end
            end
        else
    ...
    EventsModule.RegisterServer("Ethnic-police/server/set-player-cuffs", function(Source, Cuffed)
        local Player = PlayerModule.GetPlayerBySource(Source)
        Player.Functions.SetMetaData("Handcuffed", Cuffed)
        TriggerClientEvent('Ethnic-police/client/set-cuff-state', Source, Cuffed)
    end)

    CallbackModule.CreateCallback("Ethnic-police/server/get-player-cuffs", function(Source, Cb, Target)
        local Player = PlayerModule.GetPlayerBySource(Target)
        if not Player then return Cb(false) end
        Cb(Player.PlayerData.MetaData['Handcuffed'])
    end)

    EventsModule.RegisterServer("Ethnic-police/server/cuff-player", function(Source, TargetServer)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local CuffedPlayer = PlayerModule.GetPlayerBySource(tonumber(TargetServer))
        if CuffedPlayer then
            TriggerClientEvent("Ethnic-police/client/getting-cuffed", CuffedPlayer.PlayerData.Source)
            TriggerClientEvent("Ethnic-police/client/do-cuff-anim", Player.PlayerData.Source, CuffedPlayer.PlayerData.MetaData['Handcuffed'] and 'Uncuff' or 'Cuff')
            if not CuffedPlayer.PlayerData.MetaData['Handcuffed'] then
                TriggerClientEvent("Ethnic-police/client/do-cuff-anim", CuffedPlayer.PlayerData.Source, 'Getcuff', Player.PlayerData.Source)
            end
        else
    ...
    EventsModule.RegisterServer("Ethnic-police/server/set-garage-state", function(Source, Plate, VehNet, State)
        -- if State == 'In' then
    ...
    EventsModule.RegisterServer("Ethnic-police/server/place-barricade", function(Source, Prop, Coords, Heading)
        local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        TriggerClientEvent('Ethnic-police/client/set-prop-data', -1, "Add", nil, NewObject)
    end)

    EventsModule.RegisterServer("Ethnic-police/server/delete-barricade", function(Source)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if not Player then return end
        ...
        if ClosestBarricade ~= nil then
            TriggerClientEvent('Ethnic-police/client/set-prop-data', -1, "Remove", ClosestBarricade.Id)
        end
    end)

    -- Escort
    EventsModule.RegisterServer("Ethnic-police/server/escort-player", function(Source, Escorted)
        TriggerClientEvent('Ethnic-police/client/do-escort', Escorted, Source)
    end)

    -- Seat
    EventsModule.RegisterServer("Ethnic-police/server/set-in-vehicle", function(Source, TargetServer, VehicleNet, VehicleSeat)
        local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        if EscortPlayer.PlayerData.MetaData["Handcuffed"] or EscortPlayer.PlayerData.MetaData["Dead"] then
            TriggerClientEvent("Ethnic-police/client/set-in-vehicle", EscortPlayer.PlayerData.Source, VehicleNet, VehicleSeat)
        else
            Player.Functions.Notify('not-cuf', "Person is not dead or cuffed!", "error")
        end
    end)

    EventsModule.RegisterServer("Ethnic-police/server/set-out-of-vehicle", function(Source, TargetServer, VehicleNet)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local EscortPlayer = PlayerModule.GetPlayerBySource(TargetServer)
        if EscortPlayer.PlayerData.MetaData["Handcuffed"] or EscortPlayer.PlayerData.MetaData["Dead"] then
            TriggerClientEvent("Ethnic-police/client/set-out-of-vehicle", EscortPlayer.PlayerData.Source, VehicleNet)
        else
            Player.Functions.Notify('not-cuf', "Person is not dead or cuffed!", "error")
        end
    end)
    ...
    EventsModule.RegisterServer("Ethnic-police/server/remove-face-wear", function(Source, TargetServer)
        TriggerClientEvent('Ethnic-clothing/client/take-off-face-wear', TargetServer, 'Mask')
    end)

    -- Seize

    EventsModule.RegisterServer("Ethnic-police/server/seize-possesions", function(Source, TargetServer)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local TPlayer = PlayerModule.GetPlayerBySource(tonumber(TargetServer))
        -- Remove Items
        exports['Ethnic-inventory']:SaveInventoryData('Stash', 'jail-'..TPlayer.PlayerData.CitizenId, TPlayer.PlayerData.Inventory)
    ...
    EventsModule.RegisterServer("Ethnic-police/server/hire-employee", function(Source, StateId)    
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer("Ethnic-police/server/fire-employee", function(Source, StateId)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local TPlayer = PlayerModule.GetPlayerByStateId(StateId)
        if TPlayer then
            if TPlayer.PlayerData.Job.Name ~= 'police' then
    ...
    EventsModule.RegisterServer("Ethnic-police/server/receive-jail-item", function(Source, Item)
        local Player = PlayerModule.GetPlayerBySource(Source)
        Player.Functions.AddItem(Item, 1, 'jail-item', false, true)
    end)

    EventsModule.RegisterServer("Ethnic-police/server/reduce-jail-time", function(Source, Amount)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if Amount > 0 then
            if Amount - 1 > 0 then
                Amount = Amount - 1
    ...
    EventsModule.RegisterServer("Ethnic-police/server/set-in-jail", function(Source, Time, Parole)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if Time >= 1 then
    ...
    EventsModule.RegisterServer("Ethnic-police/server/purchase-police-vehicle", function(Source, Data, StateId)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    -- VEHICLE RECORDS
    CallbackModule.CreateCallback("Ethnic-police/server/get-veh-record", function(Source, Cb, Plate)
        Cb(VehicleRecords[Plate])
    end)

    CallbackModule.CreateCallback("Ethnic-police/server/delete-veh-record", function(Source, Cb, NetId, Plate)
...
RegisterServerEvent('Ethnic-police/server/update-cop-count', function()
    ...
    TriggerClientEvent("Ethnic-police/set-cop-count", -1, PoliceAmount)
end)

-- Spikes
RegisterNetEvent("Ethnic-police/server/lay-down-spikes", function(SendData)
    TriggerClientEvent('Ethnic-police/client/lay-down-spikes', -1, SendData)
end)

-- Blips
RegisterNetEvent("Ethnic-police/server/clear-blip", function()
    TriggerClientEvent('Ethnic-police/client/clear-service-blips', -1)
end)

-- Badge
RegisterNetEvent("Ethnic-police/server/request-pd-badge", function(Cid, Image)
    local src = source
...
RegisterNetEvent("Ethnic-police/server/helicam/sync-spotlight", function(Type, NetId, State, Coords)
    TriggerClientEvent('Ethnic-police/client/helicam/sync-spotlight', -1, Type, NetId, State, Coords)
end)

RegisterNetEvent("Ethnic-police/server/create-evidence", function(Type, WeaponData)
    local src = source
    ...
    TriggerClientEvent('Ethnic-police/client/set-evidence', -1, RandomId, Evidence[RandomId])
end)

RegisterNetEvent("Ethnic-police/server/evidence/set-status", function(Status)
    local src = source
    PlayerStatus[src] = Status
end)

RegisterNetEvent("Ethnic-police/server/get-target-status", function(TargetServer)
    local src = source
    local Retval = 'None'
    ...
    TriggerClientEvent('Ethnic-chat/client/post-message', src, 'Status', Retval, 'warning')
end)

RegisterNetEvent("Ethnic-police/server/gsr-result", function(TargetServer)
    local src = source
    ...
    TriggerClientEvent('Ethnic-chat/client/post-message', src, 'GSR', Retval, 'warning')
end)

RegisterNetEvent("Ethnic-police/server/finger-result", function(TargetServer)
    local src = source
    local Target = PlayerModule.GetPlayerBySource(TargetServer)
    if not Target then return end

    TriggerClientEvent('Ethnic-chat/client/post-message', src, 'FINGERPRINT', Target.PlayerData.MetaData["FingerPrint"], 'warning')
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if PlayerModule ~= nil then
            ...
            TriggerClientEvent('Ethnic-police/client/update-service-blips', -1, DutyPlayers)
            Citizen.Wait(2000)
        else
            Citizen.Wait(2000)
        end
