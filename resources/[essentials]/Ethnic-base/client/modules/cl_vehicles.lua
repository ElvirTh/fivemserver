...
VehicleModule = {
    SpawnVehicle = function(VehicleName, Coords, Plate, Warp)
        ...
        local VehicleNet = CallbackModule.SendCallback('Ethnic-base/server/create-vehicle', VehicleName, TargetCoords, Plate)
        if not VehicleNet then
            exports['Ethnic-ui']:Notify('vehicle-engine', "Vehicle failed to spawn, please try again later..", 'error')
            return false
        end
    ...
    ApplyVehicleMods = function(Vehicle, VehicleMods, Plate, FadeIgnore)
        ...
        if VehicleMods == 'Request' then
            local CallbackModule = exports[GetCurrentResourceName()]:FetchModule('Callback')
            VehicleMods = CallbackModule.SendCallback('Ethnic-base/server/get-vehicle-mods', Plate)
        end
    ...
    SaveVehicle = function(Vehicle, Plate, MetaData, Type)
        ...
        if VehicleMods ~= nil then
            local DidSave = CallbackModule.SendCallback('Ethnic-base/server/save-vehicle-mods', VehicleMods, VehicleDamage, MetaData, Plate, Type)
            return DidSave
        else
            return false
        end
...
RegisterNetEvent("Ethnic-base/client/spawn-vehicle", function(Model)
    ...
    if FunctionModule.RequestModel(VehicleModel) then
        ...
        TriggerEvent('Ethnic-ui/client/notify', "base-vehicle-error", "This vehicle does not exist..", 'error')
    end
end)

RegisterNetEvent("Ethnic-base/client/delete-vehicle", function()
    ...
    if GetEntityType(Vehicle) == 2 then
        ...
        TriggerEvent('Ethnic-ui/client/notify', "base-vehicle-error", "No vehicle found..", 'error')
    end
end)

RegisterNetEvent("Ethnic-base/client/repair-vehicle", function()
    local Vehicle = GetVehiclePedIsIn(PlayerPedId())
    local VehicleModule = exports[GetCurrentResourceName()]:FetchModule('Vehicle')
    if not IsPedInAnyVehicle(PlayerPedId()) then Vehicle = exports[GetCurrentResourceName()]:FetchModule('Functions').GetEntityPlayerIsLookingAt(3.0, 0.2, 286, PlayerPedId()) end
    if GetEntityType(Vehicle) == 2 then
