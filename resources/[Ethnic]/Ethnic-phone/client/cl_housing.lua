...
function Housing.OnPlayerLoad()
    local Houses = CallbackModule.SendCallback("Ethnic-phone/server/housing/get-owned-houses")
    Housing.OwnedProperties = Houses
end

RegisterNetEvent('Ethnic-housing/client/sync-house-data', function(Name, HouseData)
    Citizen.SetTimeout(500, function()
        if Housing.OwnedProperties[Name] then
            if exports['Ethnic-housing']:IsKeyholderHouseId(Name) then
                Housing.OwnedProperties[Name] = HouseData
            else
                Housing.OwnedProperties[Name] = nil
            end
            if CurrentApp == 'housing' then
                exports['Ethnic-ui']:SendUIMessage("Phone", "RenderHousingApp", {
...
RegisterNUICallback('Housing/SearchCurrentLocation', function(Data, Cb)
    ...
    Cb({
        ...
        Category = exports['Ethnic-housing']:GetCategoryFromTier(HouseData.Interior),
...
RegisterNUICallback('Housing/PurchaseHousing', function(Data, Cb)
    local Purchased = CallbackModule.SendCallback("Ethnic-phone/server/housing/purchase-house", Data)
    if Purchased then
        local Houses = CallbackModule.SendCallback("Ethnic-phone/server/housing/get-owned-houses")
        Housing.OwnedProperties = Houses
        exports['Ethnic-ui']:SendUIMessage("Phone", "RenderHousingApp", {
...
RegisterNUICallback('Housing/ToggleLock', function(Data, Cb)
    local NewState = CallbackModule.SendCallback("Ethnic-phone/server/housing/toggle-locks", Data)
    Cb(NewState)
end)

RegisterNUICallback('Housing/SellHouse', function(Data, Cb)
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/housing/sell-house", Data)
    local Houses = CallbackModule.SendCallback("Ethnic-phone/server/housing/get-owned-houses")
    Housing.OwnedProperties = Houses
        exports['Ethnic-ui']:SendUIMessage("Phone", "RenderHousingApp", {
        OwnedProperties = Housing.OwnedProperties,
...
RegisterNUICallback('Housing/Locate', function(Data, Cb)
    ...
    if Data.HouseId == 'apartment' then
        local Coords = exports['Ethnic-apartment']:GetApartmentLocation()
    ...
    exports['Ethnic-ui']:Notify("phone-success", "Waypoint set!", "primary")
end)

RegisterNUICallback('Housing/SetHouseLocation', function(Data, Cb)
    TriggerEvent('Ethnic-housing/client/set-house-location', Data)
    Cb('Ok')
end)

RegisterNUICallback('Housing/OpenFurniture', function(Data, Cb)
    ClosePhone()
    TriggerEvent('Ethnic-housing/client/open-furniture', Data)
    Cb('Ok')
end)
...
RegisterNUICallback('Housing/AddKey', function(Data, Cb)
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/housing/add-key", Data)
    Cb(Result)
end)

RegisterNUICallback('Housing/GetKeyholders', function(Data, Cb)
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/housing/get-keyholders", Data)
    Cb(Result)
end)

RegisterNUICallback('Housing/RemoveKey', function(Data, Cb)
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/housing/remove-key", Data)
    Cb(Result)
end)