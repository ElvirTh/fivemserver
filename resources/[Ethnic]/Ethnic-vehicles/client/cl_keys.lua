...
CreateThread(function()
    ...
    KeybindsModule.Add('toggleEngineOn', 'Vehicle', 'Turn Engine On', 'IOM_WHEEL_UP', false, 'Ethnic-vehicles/client/toggle-engine-on', false, "MOUSE_WHEEL")
    KeybindsModule.Add('toggleEngineOff', 'Vehicle', 'Turn Engine Off', 'IOM_WHEEL_DOWN', false, 'Ethnic-vehicles/client/toggle-engine-off', false, "MOUSE_WHEEL")
    KeybindsModule.Add('toggleLocks', 'Vehicle', 'Toggle Vehicle Locks', 'L', false, 'Ethnic-vehicles/client/toggle-locks', false)
        ...
        if LocalPlayer.state.LoggedIn and not StealingKeys then
            if GetVehiclePedIsTryingToEnter(PlayerPedId()) ~= 0 and GetSeatPedIsTryingToEnter(PlayerPedId()) == -1 then
                ...
                if Driver ~= 0 and not IsPedAPlayer(Driver) and IsEntityDead(Driver) then
                    StealingKeys = true
                    exports['Ethnic-ui']:ProgressBar('Stealing keys..', 3000, false, false, false, false, function(DidComplete)
                        if DidComplete then
                            local Plate = GetVehicleNumberPlateText(Vehicle)
                            TriggerEvent('Ethnic-vehicles/client/on-veh-lockpick', Vehicle, Plate)
...
function SetVehicleKeys(Plate, Bool, CitizenId)
    ...
    TriggerServerEvent('Ethnic-vehicles/server/set-keys', Plate, Bool, CitizenId)
end
...
RegisterNetEvent('Ethnic-items/client/used-lockpick', function(IsAdvanced, isBank)
    ...
    if GetVehicleClass(Entity) == 18 then
        return exports['Ethnic-ui']:Notify('stronger-device', "You need a stronger device to do this..", "error")
    end

    if LastCartheftAlert ~= Entity then
        EventsModule.TriggerServer('Ethnic-ui/server/send-stealing-vehicle', FunctionsModule.GetStreetName(), GetVehicleDescription(Entity))
    ...
    if InVehicle then
        TriggerEvent('Ethnic-vehicles/client/on-start-lockpick', Entity, Plate)
        EventsModule.TriggerServer('Ethnic-ui/server/play-sound-in-distance', {['Position'] = {[1] = EntityCoords.x, [2] = EntityCoords.y, [3] = EntityCoords.z}, ['Distance'] = 5.0, ['MaxDistance'] = 0.20, ['Name'] = 'lockpick', ['Volume'] = 0.7, ['Type'] = 'Spatial'})
        ...
        LoopAnimation(false)
        if not Outcome then
            exports['Ethnic-ui']:Notify('keys-error', "Failed attempt..", 'error')
            exports['Ethnic-assets']:RemoveLockpickChance(IsAdvanced)
            return
        end

        TriggerEvent('Ethnic-vehicles/client/on-veh-lockpick', Entity, Plate)
        exports['Ethnic-ui']:Notify('keys', "Vehicle Lockpicked.", 'success')
        SetVehicleKeys(Plate, true)
    else
        ...
        EventsModule.TriggerServer('Ethnic-ui/server/play-sound-in-distance', {['Position'] = {[1] = EntityCoords.x, [2] = EntityCoords.y, [3] = EntityCoords.z}, ['Distance'] = 12.0, ['MaxDistance'] = 0.20, ['Name'] = 'car-unlock', ['Volume'] = 1.0, ['Type'] = 'Spatial'})
        TriggerEvent('Ethnic-vehicles/client/on-veh-lockpick', Entity, Plate)
        exports['Ethnic-ui']:Notify('keys', "Unlocked.", 'success')
        VehicleModule.SetVehicleDoorsLocked(Entity, 1)
    end
end)

RegisterNetEvent("Ethnic-vehicles/client/set-veh-keys", function(Plate, Keyholders)
    Config.VehicleKeys[Plate] = Keyholders
end)

RegisterNetEvent('Ethnic-vehicles/client/give-keys', function()
    ...
    if IsInVehicle and GetPedInVehicleSeat(Entity, -1) ~= PlayerPedId() then
        ...
        if TargetServer ~= GetPlayerServerId(PlayerId()) then
            ...
            exports['Ethnic-ui']:Notify('keys-error', "You gave the keys.", 'success')
        end
    else
        ...
        if ClosestPlayer['ClosestPlayerPed'] == -1 and ClosestPlayer['ClosestServer'] == -1 then
            exports['Ethnic-ui']:Notify('keys-error', "An error occured! (No one near!)", 'error')
            return
        end
...
RegisterNetEvent('Ethnic-vehicles/client/get-keys', function()
    local IsInVehicle = IsPedInAnyVehicle(PlayerPedId())
    local Entity, EntityType, EntityCoords = FunctionsModule.GetEntityPlayerIsLookingAt(4.0, 0.2, 286, PlayerPedId())
    ...
    TriggerEvent('Ethnic-vehicles/client/on-veh-lockpick', Entity, Plate)
    exports['Ethnic-ui']:Notify('keys-error', "You got the keys.", 'success')
end)

RegisterNetEvent('Ethnic-vehicles/client/toggle-engine-on', function(OnPress)
...
RegisterNetEvent('Ethnic-vehicles/client/toggle-engine-off', function(OnPress)
    if not OnPress then return end
...
RegisterNetEvent('Ethnic-vehicles/client/toggle-locks', function(OnPress)
    if not OnPress then return end
    ...
    if GetVehicleDoorLockStatus(Entity) == 1 then
        ...
        VehicleModule.SetVehicleDoorsLocked(Entity, 1)
        ClearPedTasks(PlayerPedId())
        exports['Ethnic-ui']:Notify('keys', "Vehicle Unlocked.", 'success')
        EventsModule.TriggerServer('Ethnic-ui/server/play-sound-in-distance', {['Position'] = {[1] = EntityCoords.x, [2] = EntityCoords.y, [3] = EntityCoords.z}, ['Distance'] = 12.0, ['MaxDistance'] = 0.20, ['Name'] = 'car-unlock', ['Volume'] = 1.0, ['Type'] = 'Spatial'})
    end
