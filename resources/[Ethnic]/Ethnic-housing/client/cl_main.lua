            ...
            if not InsideHouse then
                ...
                if HouseData ~= nil and (not HouseData.Locked or IsKeyholderClosestHouse()) and HouseData.Active then
                    ...
                    if FrontDistance < 1.5 then
                        ...
                        if not ShowingInteraction then
                            exports['Ethnic-ui']:SetInteraction("[E] Enter House")
                ...
                if HouseData ~= nil and OffSets ~= nil then
                    ...
                    if Distance < 1.5 then
                        ...
                        if not ShowingInteraction then
                            exports['Ethnic-ui']:SetInteraction("[E] Leave House")
                            ShowingInteraction = true
                        end
...
RegisterNetEvent('Ethnic-housing/client/set-house-location', function(Data)
    if InsideHouse == false and Data.Location ~= 'Backdoor' then return exports['Ethnic-ui']:Notify("housing-error", "You have to be in that house to change this location..", "error") end
    if InsideHouse and Data.HouseId ~= InsideHouseData.Name then return exports['Ethnic-ui']:Notify("housing-error", "You have to be in that house to change this location..", "error") end
    TriggerServerEvent('Ethnic-housing/server/set-house-location', Data, InsideHouse)
end)

RegisterNetEvent('Ethnic-housing/client/try-add-new-house', function()
    local InputChoices = {}
...
RegisterNetEvent('Ethnic-housing/client/sync-house-data', function(Name, HouseData, RefreshInterior)
...
RegisterNetEvent('Ethnic-housing/client/lock-closest-house', function()
    if ClosestHouse == nil or Config.Houses[ClosestHouse] == nil then return end
    CallbackModule.SendCallback('Ethnic-phone/server/housing/toggle-locks', { HouseId = Config.Houses[ClosestHouse].Name })
end)

RegisterNetEvent('Ethnic-housing/client/lock-house-id', function(Name)
    if Name == nil or Config.Houses[Name] == nil then return end
    CallbackModule.SendCallback('Ethnic-phone/server/housing/toggle-locks', { HouseId = Name })
end)

RegisterNetEvent('Ethnic-housing/client/enter-house', function(Id)
...
function GetHouseConfig()
    local Config = CallbackModule.SendCallback('Ethnic-housing/server/get-houses')
    return Config
end
...
function EnterHouse(IsBackdoor)
    ...
    if InsideHouseData ~= nil then
        ...
        local Interior = exports['Ethnic-interiors']:CreateInterior(InsideHouseData.Interior, HouseCoords, false)
                ...
                if InsideHouseData.Locations['Stash'] and #(PlayerCoords - vector3(InsideHouseData.Locations['Stash'].Coords.x, InsideHouseData.Locations['Stash'].Coords.y, InsideHouseData.Locations['Stash'].Coords.z)) <= 1.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if exports['Ethnic-inventory']:CanOpenInventory() then
                            ...
                            EventsModule.TriggerServer('Ethnic-inventory/server/open-other-inventory', 'House-'..InsideHouseData.Name, 'Stash', Config.InventoryData[InsideHouseData.Interior].Slots, Config.InventoryData[InsideHouseData.Interior].Weight)
                        end
                    end
                else
            ...
            if InsideHouseData.Locations['Wardrobe'] and #(PlayerCoords - vector3(InsideHouseData.Locations['Wardrobe'].Coords.x, InsideHouseData.Locations['Wardrobe'].Coords.y, InsideHouseData.Locations['Wardrobe'].Coords.z)) <= 5.0 then
                ...
                if InsideHouseData.Locations['Wardrobe'] and #(PlayerCoords - vector3(InsideHouseData.Locations['Wardrobe'].Coords.x, InsideHouseData.Locations['Wardrobe'].Coords.y, InsideHouseData.Locations['Wardrobe'].Coords.z)) <= 1.0 then
                    ...
                    if IsControlJustReleased(0, 47) then
                        local Interior = exports['Ethnic-interiors']:DespawnInterior(HouseObject)
                        UnloadDecoration()
                        InsideHouseData = nil
                        InsideHouse = false
                        HouseObject = nil
                ...
                if InsideHouseData.Locations['Backdoor'] and #(PlayerCoords - vector3(InsideHouseData.Locations['Backdoor'].Coords.x, InsideHouseData.Locations['Backdoor'].Coords.y, InsideHouseData.Locations['Backdoor'].Coords.z)) <= 1.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if InsideHouseData.Coords.Back ~= nil and InsideHouseData.Coords.Back.X ~= 0 and InsideHouseData.Coords.Back.Y ~= 0 and InsideHouseData.Coords.Back.Z ~= 0 then
                            ...
                            exports['Ethnic-ui']:Notify("housing-error", "Back door not set on outside!", "error")
                        end
                    end
