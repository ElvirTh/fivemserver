...
AddEventHandler('Modules/client/ready', function()
    TriggerEvent('Modules/client/request-dependencies', {
        ...
        VehiclesModule = exports['Ethnic-base']:FetchModule('Vehicle')
...
RegisterNetEvent('Ethnic-base/client/on-login', function()
    ...
    Citizen.SetTimeout(3500, function()
        TriggerEvent('Ethnic-assets/client/toggle-items', false)
    end)
end)
...
RegisterCommand('degen', function(source, args, RawCommand)
    ...
    EventsModule.TriggerServer('Ethnic-inventory/server/degen-item', exports['Ethnic-inventory']:GetSlotForItem(Item), Amount)
end)

-- [ Code ] -- 

RegisterNetEvent("Ethnic-inventory/client/open-inventory", function()
                        ...
                        if not Config.InventoryBusy then 
                            if not UsingKey then
                                ...
                                Citizen.SetTimeout(100, function()
                                    EventsModule.TriggerServer("Ethnic-inventory/server/use-item", Key)
                                    LastUsedSlot = Key
                                    Citizen.Wait(250)
                                    UsingKey = false
                                end)
...
RegisterNetEvent('Ethnic-inventory/client/sync-drops', function(ConfigData)
    Config.Drops = ConfigData
end)

RegisterNetEvent('Ethnic-inventory/client/set-inventory-state', function(Bool)
    Config.InventoryBusy = Bool
    if Config.InventoryBusy then
        TriggerEvent('Ethnic-inventory/client/force-close')
    end
end)

RegisterNetEvent('Ethnic-inventory/client/open-empty-other', function()
    SetTimecycleModifier('hud_def_blur')
    SetTimecycleModifierStrength(1.0)
    -- Config.InventoryBusy = true
    local PlayerData = PlayerModule.GetPlayerData()
...
RegisterNetEvent('Ethnic-inventory/client/open-inventory-other', function(OtherData)
    SetTimecycleModifier('hud_def_blur')
    SetTimecycleModifierStrength(1.0)
    local PlayerData = PlayerModule.GetPlayerData()
    SendNUIMessage({
...
RegisterNetEvent('Ethnic-inventory/client/item-box', function(Type, ItemData, Amount)
...
RegisterNetEvent('Ethnic-inventory/client/inventory-log', function(Text)
    SendNUIMessage({
        Action = 'InventoryLog',
        Text = Text
    })
...
RegisterNetEvent('Ethnic-inventory/client/set-required', function(ItemData, Bool)
    if Bool then
        SendNUIMessage({
            Action = "ShowRequired",
            data = ItemData
...
RegisterNetEvent('Ethnic-inventory/client/update-drops', function(DropData, DropId)
    Config.Drops[DropId] = DropData
end)

RegisterNetEvent('Ethnic-inventory/client/use-weapon', function(ItemData)
    ...
    if GetHashKey(WeaponName) == CurrentWeapon then -- If we have the weapon out
        TriggerEvent('Ethnic-inventory/client/reset-weapon', true)
        return
    end
    ...
    Citizen.SetTimeout(15, function()
        ...
        TriggerEvent('Ethnic-weapons/client/set-current-weapon', ItemData)
        TriggerEvent('Ethnic-assets/client/attach-items')
    end)
end)

RegisterNetEvent("Ethnic-inventory/client/reset-weapon", function(Anim)
    TriggerEvent('Ethnic-assets/client/reset-holster')
    ...
    Citizen.SetTimeout(100, function()
        ...
        TriggerEvent('Ethnic-weapons/client/set-current-weapon', nil)
        Citizen.SetTimeout(100, function()
            TriggerEvent('Ethnic-assets/client/attach-items')
        end)
    end)
...
RegisterNetEvent('Ethnic-inventory/client/update-slot-quality', function(Slot, ItemName, CreateDate)
    SendNUIMessage({
...
RegisterNetEvent("Ethnic-inventory/client/on-fully-degen-item", function(ItemData)
    if ItemData.Type == 'Weapon' then
        TriggerEvent('Ethnic-inventory/client/reset-weapon')
        TriggerEvent('Ethnic-assets/client/attach-items')
    end
    if ItemData.ItemName == 'weapon_grapple' then
        TriggerEvent('Ethnic-grapple/client/used-grapple', true)
    end
...
RegisterNetEvent("Ethnic-inventory/client/show-button-steal", function(TargetId)
    CurrentStealNumber = TargetId
    SendNUIMessage({Action = "ShowStealButton"})
end)

RegisterNetEvent("Ethnic-inventory/client/force-close", function()
...
RegisterNetEvent("Ethnic-inventory/client/craft", function(ItemName, Amount, ToSlot, Info, Cost)
    SendNUIMessage({Action = "ForceClose"})
    Citizen.SetTimeout(550, function()
        if not Config.IsCrafting then
            Config.InventoryBusy, Config.IsCrafting = true, true
            exports['Ethnic-ui']:ProgressBar('Crafting..', math.random(5000, 7500), {['AnimName'] = "fixing_a_player", ['AnimDict'] = "mini@repair", ['AnimFlag'] = 16}, false, false, true, function(DidComplete)
                if DidComplete then
                    StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
                    TriggerServerEvent("Ethnic-inventory/server/done-crafting", ItemName, Amount, ToSlot, Info, Cost)
                else
                    StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
                end
...
function InitInventory()
    KeybindsModule.Add('openInventory', 'Player', 'Open Inventory', 'K', function(IsPressed)
        ...
        local Apartment = exports['Ethnic-apartment']:IsInsideApartment()
        ...
        elseif CurrentDrop ~= 0 then
            ...
            TriggerEvent('Ethnic-inventory/client/open-inventory')
            return
        end
        EventsModule.TriggerServer('Ethnic-inventory/server/open-other-inventory', InvName, InvType, MaxSlots, MaxWeight, Vehicle or nil)
    end)
...
function HasEnoughQuality(MinQuality, Slot)
    local HasEnoughQuality = CallbackModule.SendCallback('Ethnic-inventory/server/has-item-enough-quality', MinQuality, Slot)
...
RegisterNUICallback('UseItem', function(Data)
    ...
    Citizen.SetTimeout(150, function()
        EventsModule.TriggerServer('Ethnic-inventory/server/use-item', Data.Slot)
        Citizen.Wait(450)
        UsingKey = false
    end)
end)
...
RegisterNUICallback('DoItemData', function(data, Cb)
    ...
    local DidData = CallbackModule.SendCallback('Ethnic-inventory/server/do-item-data', SendData)
    Cb(DidData)
end)

RegisterNUICallback('CloseInventory', function(Data, Cb)
    -- Config.InventoryBusy = false
    TriggerServerEvent('Ethnic-inventory/server/closed', Data.OtherInv, Data.OtherName)
    ...
    TriggerServerEvent('Ethnic-inventory/server/check-other', Data.OtherInv, Data.OtherName)
...
RegisterNUICallback('StealMoney', function()
    TriggerServerEvent('Ethnic-inventory/server/steal-money', CurrentStealNumber)
    CurrentStealNumber = nil
end)
...
RegisterNUICallback('CopyToClipboard', function()
    ...
    exports['Ethnic-ui']:Notify("copied", "Copied item info..", "success", 3000)
end)
...
RegisterNUICallback('SaveInventory', function(data)
    ...
    elseif data.Type == 'SaveNow' then
        TriggerServerEvent('Ethnic-inventory/server/set-player-items', InventorySaveData)
        InventorySaveData = {}
    end
end)
...
RegisterNUICallback('CombineItems', function(data)
    Citizen.SetTimeout(1500, function()
        ...
        exports['Ethnic-ui']:ProgressBar('Combining..', math.random(5000, 6500), {['AnimName'] = "idle_c", ['AnimDict'] = "amb@world_human_clipboard@male@idle_a", ['AnimFlag'] = 49}, false, false, true, function(DidComplete)
            if DidComplete then
                StopAnimTask(PlayerPedId(), "amb@world_human_clipboard@male@idle_a", "idle_c", 1.0)
                TriggerServerEvent("Ethnic-inventory/server/done-combinding", data.FromSlot, data.FromItem, data.ToSlot, data.ToItem, data.Reward)
