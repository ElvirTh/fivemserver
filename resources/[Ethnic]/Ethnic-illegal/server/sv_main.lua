...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-illegal/server/is-dryer-busy', function(Source, Cb)
        Cb(DryerBusy)
    end)

    EventsModule.RegisterServer("Ethnic-illegal/server/sell", function(Source, Data)
...
RegisterNetEvent('Ethnic-illegal/server/start-dry-process', function()
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if not Player then return end
    local HasBranches = false

    local StashItems = exports['Ethnic-inventory']:GetInventoryItems('dry-rack')
    ...
    if HasBranches then
        ...
        SetTimeout((1000 * 60) * Config.WeedRackDryTime, function()
            ...
            exports['Ethnic-inventory']:SetInventoryItems('dry-rack', StashItems)
            Wait(500)
            DryerBusy = false
