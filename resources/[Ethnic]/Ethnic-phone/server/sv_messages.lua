Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/messages/get-chats', function(Source, Cb)
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/messages/get-chat-messages', function(Source, Cb, Data)
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/messages/send-message', function(Source, Cb, Data)
        ...
        DatabaseModule.Execute('SELECT * FROM player_phone_messages WHERE citizenid = ? AND number = ? AND name = ?', {
            ...
            if ChatData[1] ~= nil then -- Check if sender already has chat with receiver.
                ...
                DatabaseModule.Execute('UPDATE player_phone_messages SET messages = ? WHERE citizenid = ? AND number = ? AND name = ?', {
                    ...
                    TriggerClientEvent('Ethnic-phone/client/messages/refresh-chat', Source, Data.ContactData.number, Messages)
                        ...
                        DatabaseModule.Execute('SELECT * FROM player_phone_messages WHERE citizenid = ? AND number = ? AND name = ?', {
                            ...
                            if TargetChatData[1] ~= nil then -- Target already has chat with sender
                                ...
                                DatabaseModule.Execute('UPDATE player_phone_messages SET messages = ? WHERE citizenid = ? AND number = ? AND name = ?', {
                                    ...
                                    TriggerClientEvent('Ethnic-phone/client/messages/refresh-chat', Receiver.PlayerData.Source, SenderPhone, Messages)
                                end)
                                Cb(true)
                                ...
                                DatabaseModule.Execute('INSERT INTO player_phone_messages (citizenid, name, number, messages) VALUES (?, ?, ?, ?)', {
                                    ...
                                    MessagesDebugPrint('NewMessageOnline', ('Saved Receiver\'s chat, currently refreshing.. | CitizenId: %s'):format(Receiver.PlayerData.CitizenId))
                                    TriggerClientEvent('Ethnic-phone/client/messages/refresh-chat', Receiver.PlayerData.Source, Data.ContactData.number, Messages)
                                end)
                                Cb(true)
                            end
                        end)
