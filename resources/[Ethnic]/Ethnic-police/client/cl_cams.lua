...
RegisterNetEvent('Ethnic-police/client/show-camera-input', function()
	local PlayerData = PlayerModule.GetPlayerData()
    if PlayerData.Job.Name ~= "police" or not PlayerData.Job.Duty then return end

	if InsideCam then
		return exports['Ethnic-ui']:Notify('cam-error', 'You are already accessing a cam..', "error")
	end
    ...
    if Result then
        if Config.SecurityCams[tonumber(Result.CamId)] ~= nil then
			TriggerEvent('Ethnic-police/client/open-cam', tonumber(Result.CamId))
		else
			exports['Ethnic-ui']:Notify('cam-error', "This camera does not exist..", 'error')
		end
    end
end)

RegisterNetEvent("Ethnic-police/client/open-cam", function(CamId)
	...
	exports['Ethnic-ui']:SetInteraction('[ESC/BACKSPACE] Close Cam', 'error')
	exports['Ethnic-assets']:AttachProp('Tablet')
