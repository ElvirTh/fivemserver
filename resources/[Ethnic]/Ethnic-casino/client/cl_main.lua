...
AddEventHandler('Modules/client/ready', function()
    TriggerEvent('Modules/client/request-dependencies', {
        ...
        local WheelVehicle = CallbackModule.SendCallback('Ethnic-casino/server/get-current-wheel-veh')
...
AddEventHandler('onResourceStop', function(resource)
    if resource == GetCurrentResourceName() then
        if WheelPed ~= nil then
            ...
            exports['Ethnic-assets']:ReleaseDui('Casino-Wheel')
            exports['Ethnic-assets']:ReleaseDui('Casino-TV-Car')
...
RegisterNetEvent("Ethnic-casino/client/casino-actions", function()
    local MenuData = {
        {
            ['Title'] = 'Buy Chips',
            ['Desc'] = 'Buy chips to play with.',
            ['Data'] = { ['Event'] = 'Ethnic-casino/client/casino-action', ['Type'] = 'Client', ['Action'] = 'Buy' }
        },
        {
            ['Title'] = 'Buy Chips (Dirty)',
            ['Desc'] = 'Buy chips to play with.',
            ['Data'] = { ['Event'] = 'Ethnic-casino/client/casino-action', ['Type'] = 'Client', ['Action'] = 'BuyDirty' }
        },
        {
            ['Title'] = 'Withdraw (Cash)',
            ['Desc'] = 'Withdraw chips to cash.',
            ['Data'] = { ['Event'] = 'Ethnic-casino/client/casino-action', ['Type'] = 'Client', ['Action'] = 'CashWithdraw' }
        },
        {
            ['Title'] = 'Withdraw (Bank)',
            ['Desc'] = 'Withdraw chips to bank account.',
            ['Data'] = { ['Event'] = 'Ethnic-casino/client/casino-action', ['Type'] = 'Client', ['Action'] = 'BankWithdraw' }
        },
        {
            ['Title'] = 'Transfer',
            ['Desc'] = 'Transfer chips to other person.',
            ['Data'] = { ['Event'] = 'Ethnic-casino/client/casino-action', ['Type'] = 'Client', ['Action'] = 'Transfer' }
        },
    }
...
local Timeout = false
RegisterNetEvent("Ethnic-casino/client/casino-action", function(Data)
    if not HasCasinoMembership() then return exports['Ethnic-ui']:Notify("casino", "You must have a casino membership to do this..", "error") end
    local Type = Data['Action']

    if Type == 'Buy' then
        Wait(100)
        local ChipsInput = exports['Ethnic-ui']:CreateInput({
            {
                Name = 'chips-amount', 
        ...
        if ChipsInput['chips-amount'] then
            EventsModule.TriggerServer("Ethnic-casino/server/buy-chips", ChipsInput['chips-amount'])
        end
    elseif Type == 'BuyDirty' then
        local Payment = math.random(10, 110)
        if exports["Ethnic-inventory"]:HasEnoughOfItem("markedbills", 20) then
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'markedbills', 20, false, true)
            if DidRemove then
                Payment = Payment + (250 * 20) -- $5k / $250 per
            end
        end
        if exports["Ethnic-inventory"]:HasEnoughOfItem("cash-rolls", 5, false, true) then
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'cash-rolls', 5, false, true)
            if DidRemove then
                Payment = Payment + (30 * 5) -- $150 / $30 per
            end
        end
        if exports["Ethnic-inventory"]:HasEnoughOfItem("cash-bands", 5, false, true) then
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'cash-bands', 5, false, true)
            if DidRemove then
                Payment = Payment + (300 * 5) -- $1500, / $300 per
            end
        end
        EventsModule.TriggerServer("Ethnic-casino/server/buy-chips", Payment, true)
    elseif Type == 'CashWithdraw' then
        if Timeout then return exports['Ethnic-ui']:Notify("casino-withdraw", "You are already withdrawing, please wait..", "error") end
        if not Timeout then
            Timeout = true
            local Added = CallbackModule.SendCallback('Ethnic-casino/server/withdraw-money', 'Cash')
            if Added then
                Timeout = false
            else
                Timeout = false
    ...
    elseif Type == 'BankWithdraw' then
        ...
        if not Timeout then
            Timeout = true
            local Added = CallbackModule.SendCallback('Ethnic-casino/server/withdraw-money', 'Bank')
            if Added then
                Timeout = false
            else
                Timeout = false
    ...
    elseif Type == 'Transfer' then
        Wait(100)
        local TransferInput = exports['Ethnic-ui']:CreateInput({
            {
                Name = 'state-id', 
                Label = 'State Id', 
                Icon = 'fas fa-user',
        ...
        if TransferInput['state-id'] and TransferInput['chips-amount'] then
            EventsModule.TriggerServer("Ethnic-casino/server/transfer-chips", TransferInput['state-id'], ChipsInput['chips-amount'])
        end
    end
end)
...
function HasCasinoMembership()
    ...
    local CardData = CallbackModule.SendCallback('Ethnic-base/server/get-membership')
...
function EnterCasino(Bool)
    ...
    if not InCasino then
        ...
        -- InitBlackjack(false)
        InitSlots(false)
        TriggerEvent("Ethnic-casino/client/exited")
        PostGateTriggered = false
        return
    end
    DoInitStuff()
    TriggerEvent("Ethnic-casino/client/entered")
end
...
function InitTVImage(Bool)
    if Bool then
        if TVDui then -- If tv exists, reset
            exports['Ethnic-assets']:ChangeDuiURL(TVDui['DuiId'], Config.TVImage)
            AddReplaceTexture('vw_prop_vw_cinema_tv_01', 'script_rt_tvscreen', TVDui['TxdDictName'], TVDui['TxdName'])
        else
            TVDui = exports['Ethnic-assets']:GenerateNewDui(Config.TVImage, 512, 256, 'Casino-TV-Car')
