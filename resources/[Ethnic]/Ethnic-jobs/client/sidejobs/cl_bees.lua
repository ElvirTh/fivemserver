...
Citizen.CreateThread(function()
    ...
    exports['Ethnic-ui']:AddEyeEntry(GetHashKey("beehive"), {
        Type = 'Model',
        Model = 'beehive',
        SpriteDistance = 5.0,
        Options = {
            {
                ...
                Label = 'Check Hive',
                EventType = 'Client',
                EventName = 'Ethnic-jobs/client/bees/open-context',
                EventParams = '',
            ...
            },
            {
                ...
                Label = 'Pickup Hive',
                EventType = 'Client',
                EventName = 'Ethnic-jobs/client/bees/hive-pickup',
                EventParams = '',
                Enabled = function(Entity)
                    return true
                end,
    ...
    exports['Ethnic-ui']:AddEyeEntry(GetHashKey("beehive02"), {
        Type = 'Model',
        Model = 'beehive02',
        SpriteDistance = 5.0,
        Options = {
    ...
    exports['Ethnic-ui']:AddEyeEntry(GetHashKey("beehive03"), {
        Type = 'Model',
        Model = 'beehive03',
        SpriteDistance = 5.0,
        Options = {
            ...
            },
            {
                ...
                Label = 'Harvest Hive',
                EventType = 'Client',
                EventName = 'Ethnic-jobs/client/bees/hive-harvest',
                EventParams = '',
                Enabled = function(Entity)
                    return true
                end,
    ...
    exports['Ethnic-ui']:AddEyeEntry("bee-shop", {
        Type = 'Entity',
        EntityType = 'Ped',
        SpriteDistance = 10.0,
        Distance = 5.0,
...
RegisterNetEvent('Ethnic-jobs/client/bees/action', function(Type, Data)
    if Type == 1 then
...
RegisterNetEvent('Ethnic-jobs/client/bees-place-hive', function()
                ...
                if Hit then
                    for k, v in pairs(MaterialHashes) do
                        if MaterialHash == v then
                            FoundPlace = true
                            local DidRemove = CallbackModule.SendCallback("Ethnic-base/server/remove-item", 'beehive', 1, false, true)
                            if DidRemove then
                                EventsModule.TriggerServer('Ethnic-jobs/server/bees/place', Coords, Heading)
                            end
                        end
                    end
                else
                    TriggerEvent('Ethnic-ui/client/notify', "bees-error", "You can not place any hives here..", 'error')
                end
            else
                exports['Ethnic-ui']:Notify("bees-error", "You stopped placing the hive or an error occured..", "error")
            end
        end)
    end)
end)

RegisterNetEvent('Ethnic-jobs/client/bees/add-queen', function(Data)
    exports['Ethnic-inventory']:SetBusyState(true)
    exports['Ethnic-ui']:ProgressBar('Adding Mlady..', 2000, nil, nil, true, true, function(DidComplete)
        if DidComplete then
            local DidRemove = CallbackModule.SendCallback("Ethnic-base/server/remove-item", 'bee-queen', 1, false, true)
            if DidRemove then
                TriggerServerEvent('Ethnic-jobs/server/bees/do-stuff', 'Queen', Data.HiveId)
            end
        end
...
RegisterNetEvent('Ethnic-jobs/client/bees/hive-pickup', function(Nothing, Entity)
    local HiveId = GetHiveId(Entity)
    local HiveData = GetHiveById(HiveId)
    if HiveData ~= nil then
        exports['Ethnic-ui']:ProgressBar('Grabbing Beehive..', 5000, nil, nil, true, true, function(DidComplete)
            if DidComplete then
                TriggerServerEvent('Ethnic-jobs/server/bees/do-stuff', 'Destroy', HiveData.Id)
            end
            exports['Ethnic-inventory']:SetBusyState(false)
            ClearPedTasks(PlayerPedId())
        end) 
    else
        TriggerEvent('Ethnic-ui/client/notify', "bees-error", "An error occured!", 'error')
    end
end)

RegisterNetEvent('Ethnic-jobs/client/bees/hive-harvest', function(Nothing, Entity)
    local HiveId = GetHiveId(Entity)
    local HiveData = GetHiveById(HiveId)
    if HiveData ~= nil then
        exports['Ethnic-ui']:ProgressBar('Harvesting Beehive..', 5000, nil, nil, true, true, function(DidComplete)
            if DidComplete then
                TriggerServerEvent('Ethnic-jobs/server/bees/do-stuff', 'Harvest', HiveData.Id)
            end
            exports['Ethnic-inventory']:SetBusyState(false)
            ClearPedTasks(PlayerPedId())
        end) 
...
RegisterNetEvent('Ethnic-jobs/client/bees/open-context', function(Nothing, Entity)
    local HiveId = GetHiveId(Entity)
    if HiveId and exports['Ethnic-inventory']:CanOpenInventory() then
        ShowHiveMenu(HiveId)
    end
end)
...
function ShowHiveMenu(HiveId)
    ...
    if HiveData ~= nil then
        ...
        local HasQueen = exports['Ethnic-inventory']:HasEnoughOfItem('bee-queen', 1)
        ...
        if not HiveData.Queen then
            MenuData[#MenuData + 1] = {
                ['Title'] = 'Add Queen',
                ['Desc'] = 'Mlady..',
                ['Data'] = {['Event'] = 'Ethnic-jobs/client/bees/add-queen', ['Type'] = 'Client', ['HiveId'] = HiveId},
...
function GetBeesConfig()
    local Data = CallbackModule.SendCallback("Ethnic-jobs/server/bees/get-config")
    return Data
end