            ...
            if Menu.IsMenuOpened('EmoteMenu') then
                ...
                for k, v in pairs(Config.Animations) do
                    if not v.Hidden and Menu.MenuButton(v.Name, 'EmoteMenu', false, {Emote = v.Id}) then
                        ...
                        TriggerEvent('Ethnic-animations/client/play-animation', CurrentEmote)
                    end
                end
                Menu.Display()
            end 
...
RegisterNetEvent('Ethnic-animations/client/open-emote-menu', function()
    SetupAnimationMenu()
    EmoteMenuOpen = true
    Menu.OpenMenu('EmoteMenu')
end)

RegisterNetEvent('Ethnic-animations/client/play-animation', function(Animation)
    ...
    if AnimationData ~= nil then
        ...
        exports['Ethnic-ui']:Notify('no-emote', 'Emote '..Animation..' does not exist.', 'error')
    end
end)

RegisterNetEvent('Ethnic-animations/client/clear-animation', function()
    if PlayingAnimData['Prop'] ~= nil and PlayingAnimData['Prop'] then
        exports['Ethnic-assets']:RemoveProps(PlayingAnimData['Prop'])
    end
    ClearPedTasks(PlayerPedId())
    PlayingAnim, PlayingAnimData = false, {}
end)
