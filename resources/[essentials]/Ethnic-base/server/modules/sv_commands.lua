...
CommandsModule = {
                ...
                RegisterCommand(v:lower(), function(source, args, rawCommand)
                    if Permission == "user" or Permission == nil then
                        if (ArgsRequired and #Arguments ~= 0 and args[#Arguments] == nil) then
                            TriggerClientEvent('Ethnic-ui/client/notify', source, "args-command", "All arguments should be filled in!", 'error', 3000)
                            local agus = ""
                            for name, Help in pairs(Arguments) do
                                agus = agus .. " ["..Help.Name.."]"
                            end
                            TriggerClientEvent('Ethnic-ui/client/notify', source, "command-preview", "/"..Command.." "..agus, 'error', 3000)
                        else
                        ...
                        if HasPermission then
                            ...
                            TriggerClientEvent('Ethnic-ui/client/notify', source, "access-command", "No Access.", 'error', 3000)
                        end
                    end
                end, false)
            end
    ...
    Refresh = function(source)
        TriggerClientEvent('Ethnic-chat/client/refresh-suggestion', source)
        ...
        for Command, Info in pairs(CommandsList) do
            ...
            if HasPerm then
                TriggerClientEvent('Ethnic-chat/client/add-suggestion', source, Command, Info.Help, Info.Arguments)
            end
        end
    end,
