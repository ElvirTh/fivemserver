...
RegisterNetEvent("Ethnic-vehicles/server/load-veh-meta", function(NetId, Data)
...
RegisterNetEvent("Ethnic-vehicles/server/set-veh-meta", function(NetId, Key, Value)
    local Vehicle = NetworkGetEntityFromNetworkId(NetId)
    local Meta = Entity(Vehicle).state.meta
    if Meta == nil then
        Entity(Vehicle).state:set('meta', Config.DefaultMeta, true)
