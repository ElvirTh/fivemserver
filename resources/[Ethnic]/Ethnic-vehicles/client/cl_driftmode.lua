...
function ToggleDrift(Veh)
	...
	if VehSpeed <= 1 then 
        exports['Ethnic-ui']:ProgressBar('Toggling drift mode..', 2000, false, false, true, false, function(DidComplete)
            if DidComplete then
				if not DriftMode then
					ToggledDriftMode = false
					exports['Ethnic-ui']:Notify('drift-mode-en', 'Vehicle is in standard mode!', 'error')
					TriggerEvent('Ethnic-ui/client/set-hud-values', 'DriftMode', 'Show', false)
				else
					ToggledDriftMode = true
					exports['Ethnic-ui']:Notify('drift-mode-en', 'Enjoy driving sideways!', 'success')
					TriggerEvent('Ethnic-ui/client/set-hud-values', 'DriftMode', 'Show', true)
				end    
			else
				exports['Ethnic-ui']:Notify('drift-mode-en', 'Cancelled..', 'error')
			end
        end)
	else
		exports['Ethnic-ui']:Notify('drift-mode-en', 'You can only toggle drift mode when the vehicle is stationary!', 'error')
	end
end

RegisterNetEvent("Ethnic-vehicles/client/toggle-driftmode", function(Pressed)
