...
RegisterNetEvent('Ethnic-illegal/client/plants-action', function(Type, Data)
    if Type == 1 then
...
RegisterNetEvent('Ethnic-items/client/used-seeds-female', function()
        ...
        if Hit then
            for k, v in pairs(MaterialHashes) do
                if MaterialHash == v then
                    ...
                    exports['Ethnic-ui']:ProgressBar('Planting..', 5000, nil, nil, true, true, function(DidComplete)
                        if DidComplete then
                            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'weed-seed-female', 1, false, true)
                            if DidRemove then
                                EventsModule.TriggerServer('Ethnic-illegal/server/plants-plant', vector3(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z - 0.97))
                            end
            ...
            if not FoundPlace then
                TriggerEvent('Ethnic-ui/client/notify', "plant-error", "You can not place any plants here..", 'error')
            end
        else
            TriggerEvent('Ethnic-ui/client/notify', "plant-error", "You can not place any plants here..", 'error')
        end
...
RegisterNetEvent('Ethnic-illegal/client/plants-pick-plant', function(Nothing, Entity)
    local PlantId = GetPlantId(Entity)
    local PlantData = GetPlantById(PlantId)
    if PlantData ~= nil then
        TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
        exports['Ethnic-ui']:ProgressBar('Harvesting Plant..', 5000, nil, nil, true, true, function(DidComplete)
            if DidComplete then
                TriggerServerEvent('Ethnic-illegal/server/do-plant-stuff', 'Harvest', PlantData.Id)
            end
            exports['Ethnic-inventory']:SetBusyState(false)
            ClearPedTasks(PlayerPedId())
        end)  
    else
        TriggerEvent('Ethnic-ui/client/notify', "plant-error", "An error occured!", 'error')
    end
end)

RegisterNetEvent('Ethnic-illegal/client/plants-water-plant', function(Data)
    exports['Ethnic-inventory']:SetBusyState(true)
    exports['Ethnic-ui']:ProgressBar('Watering..', 5000, {['AnimName'] = 'fire', ['AnimDict'] = 'weapon@w_sp_jerrycan', ['AnimFlag'] = 49}, nil, true, true, function(DidComplete)
        if DidComplete then
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'water', 1, false, true)
            if DidRemove then
                TriggerServerEvent('Ethnic-illegal/server/do-plant-stuff', 'Water', Data['PlantId'], math.random(40, 45))
            end
        end
        exports['Ethnic-inventory']:SetBusyState(false)
    end)
...
RegisterNetEvent('Ethnic-illegal/client/plants-fertelize-plant', function(Data)
    exports['Ethnic-inventory']:SetBusyState(true)
    exports['Ethnic-ui']:ProgressBar('Fertilizing..', 5000, {['AnimName'] = 'fire', ['AnimDict'] = 'weapon@w_sp_jerrycan', ['AnimFlag'] = 49}, nil, true, true, function(DidComplete)
        if DidComplete then
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'fertilizer', 1, false, true)
            if DidRemove then
                TriggerServerEvent('Ethnic-illegal/server/do-plant-stuff', 'Fertilizer', Data['PlantId'], math.random(40, 45))
            end
        end
        exports['Ethnic-inventory']:SetBusyState(false)
    end)
...
RegisterNetEvent('Ethnic-illegal/client/plants-add-male', function(Data)
    exports['Ethnic-inventory']:SetBusyState(true)
    exports['Ethnic-ui']:ProgressBar('Making Babies..', 2000, nil, nil, true, true, function(DidComplete)
        if DidComplete then
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'weed-seed-male', 1, false, true)
            if DidRemove then
                TriggerServerEvent('Ethnic-illegal/server/do-plant-stuff', 'Pregnant', Data['PlantId'])
            end
        end
        exports['Ethnic-inventory']:SetBusyState(false)
    end)
...
RegisterNetEvent('Ethnic-illegal/client/plants-destroy', function(Nothing, Entity)
    local PlantId = GetPlantId(Entity)
    local PlantData = GetPlantById(PlantId)
    if PlantData ~= nil then
        TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
        exports['Ethnic-ui']:ProgressBar('Cutting Plant..', 5000, nil, nil, true, true, function(DidComplete)
            if DidComplete then
                TriggerServerEvent('Ethnic-illegal/server/do-plant-stuff', 'Destroy', PlantData.Id)
            end
            exports['Ethnic-inventory']:SetBusyState(false)
            ClearPedTasks(PlayerPedId())
        end)
...
RegisterNetEvent('Ethnic-illegal/client/plants-open-context', function(Nothing, Entity)
    local PlantId = GetPlantId(Entity)
    if PlantId and exports['Ethnic-inventory']:CanOpenInventory() then
...
function ShowPlantMenu(PlantId)
    ...
    if PlantData ~= nil then
        ...
        local HasWater = exports['Ethnic-inventory']:HasEnoughOfItem('water', 1)
        local HasFertilizer = exports['Ethnic-inventory']:HasEnoughOfItem('fertilizer', 1)
        local HasMaleSeed = exports['Ethnic-inventory']:HasEnoughOfItem('weed-seed-male', 1)
        ...
        if PlantData.Water < 100 then
            MenuData[#MenuData + 1] = {
                ...
                ['Data'] = {['Event'] = 'Ethnic-illegal/client/plants-water-plant', ['Type'] = 'Client', ['PlantId'] = PlantId},
        ...
        if PlantData.Fertilizer < 100 then
            MenuData[#MenuData + 1] = {
                ...
                ['Data'] = {['Event'] = 'Ethnic-illegal/client/plants-fertelize-plant', ['Type'] = 'Client', ['PlantId'] = PlantId},
        ...
        if (PlantData.Pregnant == 'False' and PlantStage <= 2) then -- Only add male in the 2 stages
            MenuData[#MenuData + 1] = {
                ...
                ['Data'] = {['Event'] = 'Ethnic-illegal/client/plants-add-male', ['Type'] = 'Client', ['PlantId'] = PlantId},
...
function GetPlantsConfig()
    local Config = CallbackModule.SendCallback('Ethnic-illegal/server/get-plants')
    return Config
end
