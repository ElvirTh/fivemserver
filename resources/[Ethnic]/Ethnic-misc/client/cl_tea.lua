...
function InitTea()
    for k, v in pairs(TeaZones) do
        exports['Ethnic-ui']:AddEyeEntry("tea_kettle_" .. k, {
            Type = 'Zone',
            SpriteDistance = 10.0,
            Distance = 1.5,
            State = false,
            ...
            Options = {
                {
                    ...
                    EventType = 'Client',
                    EventName = 'Ethnic-misc/client/make-tea',
...
RegisterNetEvent("Ethnic-misc/client/make-tea", function(Data, Entity)
    local HasWater = exports['Ethnic-inventory']:HasEnoughOfItem('water', 1)
    if not HasWater then
        return exports['Ethnic-ui']:Notify("tea-water", "You need water for the boiler.")
    end
    ...
    exports['Ethnic-ui']:ProgressBar('Making a cup of tea...', 30000, {
        ...
        if DidComplete then
            EventsModule.TriggerServer("Ethnic-misc/server/get-tea")
        end
    end)
end)

local IsDrinkingTea = false
RegisterNetEvent("Ethnic-misc/client/used-tea", function()
    if IsDrinkingTea then return end
    IsDrinkingTea = true

    local Drinks = 4

    local Removed = CallbackModule.SendCallback("Ethnic-base/server/remove-item", 'mugoftea', 1)
    if not Removed then return end

    exports['Ethnic-ui']:Notify("take-tea", "Use E to sip tea")
    exports['Ethnic-assets']:AttachProp('Tea')
    ...
    Citizen.CreateThread(function()
        while IsDrinkingTea do
            ...
            if IsControlJustPressed(0, 38) then -- E
                if Drinks <= 0 then
                    exports['Ethnic-ui']:Notify("empty-tea", "Cup of tea is empty..")
                    goto Skip
                end

                Drinks = Drinks - 1
                TaskPlayAnim(PlayerPedId(), "amb@world_human_drinking@coffee@male@idle_a", "idle_b", 1.0, 0.1, -1, 50, 0, 0, 0, 0)
                TriggerServerEvent("Ethnic-base/server/set-meta-data", "Water", PlayerModule.GetPlayerData().MetaData.Water + 5)
