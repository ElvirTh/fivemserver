...
RegisterNetEvent('Ethnic-base/server/set-playerdata', function(PlayerData)
...
PlayerModule = {
        ...
        Database.Execute("SELECT * FROM players WHERE CitizenId = ? ", {CitizenId}, function(CharData)
            if CharData[1] ~= nil and CharData[1].Inventory ~= nil then
                ...
                if InventoryItems ~= nil then
                    for k, v in pairs(InventoryItems) do
                        if exports['Ethnic-inventory']:GetItemData(v.ItemName) ~= nil then
                            local ItemInfo = exports['Ethnic-inventory']:GetItemData(v.ItemName)
                            ReturnItems[v.Slot] = {
    ...
    LoadPlayer = function(PlayerData)
        ...
        self.Functions.Notify = function(id, Text, Type, Length)
            ...
            TriggerClientEvent("Ethnic-ui/client/notify", self.PlayerData.Source, id, Text, Type, Length)
        end
    
        self.Functions.UpdatePlayerData = function()
            -- print(GetInvokingResource() .. " is updating player data for " .. self.PlayerData.CitizenId)
            TriggerClientEvent("Ethnic-base/client/set-playerdata", self.PlayerData.Source, self.PlayerData)
        end
        ...
        self.Functions.SetJob = function(JobN, Reason)
            ...
            if Shared.Jobs[JobName] ~= nil then
                ...
                TriggerClientEvent("Ethnic-base/client/on-job-update", self.PlayerData.Source, self.PlayerData.Job, false)
            else
        ...
        self.Functions.SetJobDuty = function(OnDuty)
            ...
            TriggerClientEvent("Ethnic-base/client/on-job-update", self.PlayerData.Source, self.PlayerData.Job, true)
            self.Functions.UpdatePlayerData()
        end
        ...
        self.Functions.RemoveMoney = function(Type, Amount, Reason)
            ...
            if self.PlayerData.Money[Type] ~= nil then
                ...
                self.Functions.UpdatePlayerData()
                if Type == 'Cash' then
                    TriggerClientEvent("Ethnic-ui/client/money-change", self.PlayerData.Source, 'Remove', Amount, self.PlayerData.Money[Type])
                else
                    TriggerEvent('Ethnic-financials/server/sync-main-bank', self.PlayerData.Source, self.PlayerData.Money['Bank'])
                end
                return true
            end
            return false
        ...
        self.Functions.AddMoney = function(Type, Amount, Reason)
            ...
            if self.PlayerData.Money[Type] ~= nil then
                ...
                self.Functions.UpdatePlayerData()
                if Type == 'Cash' then
                    TriggerClientEvent("Ethnic-ui/client/money-change", self.PlayerData.Source, 'Add', Amount, self.PlayerData.Money[Type])
                else
                    TriggerEvent('Ethnic-financials/server/sync-main-bank', self.PlayerData.Source, self.PlayerData.Money['Bank'])
                end
                return true
        ...
        self.Functions.AddItem = function(Item, Amount, Slot, Info, Show, Type) 
            ...
            local ItemData = Item ~= nil and exports['Ethnic-inventory']:GetItemData(Item:lower())
            local Amount = tonumber(Amount)
            ...
            if ItemData then
                ...
                if NewWeight <= Config.Player['InventoryMaxWeight'] then -- Max weight
                    ...
                    if (Slot ~= nil and self.PlayerData.Inventory[Slot] ~= nil) and (self.PlayerData.Inventory[Slot].ItemName:lower() == Item:lower()) and (ItemData["Type"] == "Item" and not ItemData["Unique"]) then
                        ...
                        self.Functions.UpdatePlayerData()
                        if Show then
                            TriggerClientEvent('Ethnic-inventory/client/item-box', self.PlayerData.Source, 'Add', ItemData, Amount)
                        end
                        TriggerClientEvent('Ethnic-inventory/client/update-player', self.PlayerData.Source)
                        return true
                    ...
                    elseif (ItemData["Unique"]) or (not Slot or Slot == nil) or (ItemData["Type"] == "Weapon") then
                        ...
                        if FreeSlot then
                            ...
                            TriggerEvent('Ethnic-inventory/server/add-new-drop-core', self.PlayerData.Source, ItemData["ItemName"], Amount, Info, os.date(), 100)
                        end
                    end
                else
        ...
        self.Functions.RemoveItem = function(Item, Amount, Slot, Show)
            local ItemInfo = exports['Ethnic-inventory']:GetItemData(Item:lower())
            local Amount = tonumber(Amount)
            local Slot = tonumber(Slot) ~= nil and tonumber(Slot) or PlayerModule.GetFirstSlotByItem(self.PlayerData.Inventory, Item)
            local Show = Show ~= nil and Show or false
            if Slot ~= nil then
                if self.PlayerData.Inventory[Slot] ~= nil then
                    if self.PlayerData.Inventory[Slot].Amount > Amount then
                        ...
                        self.Functions.UpdatePlayerData()
                        if Show then
                            TriggerClientEvent('Ethnic-inventory/client/item-box', self.PlayerData.Source, 'Remove', ItemInfo, Amount)
                        end
                        -- TriggerEvent("Ethnic-logs/server/send-log", "inventory", "Inventory (Remove)", "red", "**"..GetPlayerName(self.PlayerData.Source) .. "** ("..self.PlayerData.CitizenId.." | "..self.PlayerData.Source..") \n **Slot:** " ..Slot.." \n **Name:** " .. Item .. " \n **Amount:** " .. Amount .. " \n **New Amount:** ".. self.PlayerData.Inventory[Slot].Amount)
                        TriggerClientEvent('Ethnic-inventory/client/update-player', self.PlayerData.Source)
                        return true
                    else
                        -- TriggerEvent("Ethnic-logs/server/send-log", "inventory", "Inventory (Remove)", "red", "**"..GetPlayerName(self.PlayerData.Source) .. "** ("..self.PlayerData.CitizenId.." | "..self.PlayerData.Source..") \n **Slot:** " ..Slot.." \n **Name:** " .. Item .. " \n **Amount:** " .. Amount .. " \n **New Amount:** ".. self.PlayerData.Inventory[Slot].Amount)
    ...
    Logout = function(source)
        ...
        TriggerClientEvent('Ethnic-base/client/on-logout', source)
