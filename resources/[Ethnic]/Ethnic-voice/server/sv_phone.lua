...
function StartPhoneCall(PhoneNumber, CallerId, ReceiverId)
    if ActiveCallsByNumber[PhoneNumber] ~= nil then
        ...
        TriggerClientEvent('Ethnic-voice/client/call-start', CallerId, ReceiverId, PhoneNumber)
        TriggerClientEvent('Ethnic-voice/client/call-start', ReceiverId, CallerId, PhoneNumber)
    end
end
...
function EndPhoneCall(PhoneNumber)
    ...
    if Data.Caller then
        ...
        TriggerClientEvent('Ethnic-voice/client/call-stop', Data.Caller, Data.Receiver, PhoneNumber)
    end
    if Data.Receiver then
        ActiveCallsBySource[Data.Receiver] = nil
        TriggerClientEvent('Ethnic-voice/client/call-stop', Data.Receiver, Data.Caller, PhoneNumber)
    end
    ActiveCallsByNumber[PhoneNumber] = nil
end

RegisterNetEvent("Ethnic-voice/server/phone/start-call", function(PhoneNumber, Receiver)
    local src = source
    StartPhoneCall(PhoneNumber, src, Receiver)
end)

RegisterNetEvent("Ethnic-voice/server/phone/end-call", function(PhoneNumber)
