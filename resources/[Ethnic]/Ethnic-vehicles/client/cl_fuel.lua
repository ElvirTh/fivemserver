...
RegisterNetEvent("Ethnic-vehicles/client/fuel/use-charger", function()
    local MenuItems = {
        {
        ...
        },
        {
            ...
            ['Data'] = {['Event'] = 'Ethnic-vehicles/client/fuel/grab-charger-hose', ['Type'] = 'Client'}
        },
    }
    exports['Ethnic-ui']:OpenContext({ ['MainMenuItems'] = MenuItems })
end)

RegisterNetEvent("Ethnic-vehicles/client/fuel/grab-charger-hose", function()
    ...
    TriggerEvent("Ethnic-ui/client/play-sound", "pickupnozzle", 0.4)
    Wait(300)
    StopAnimTask(PlayerPedId(), "anim@am_hold_up@male", "shoplift_high", 1.0)
    exports['Ethnic-assets']:AttachProp('ChargerNozzle')
    local ElectricNozzle = exports['Ethnic-assets']:GetSpecificProp('ChargerNozzle')
    ...
    CreateThread(function()
        while HasChargerNozzle do
            if not exports['Ethnic-assets']:GetSpecificPropStatus('ChargerNozzle') then
                exports['Ethnic-assets']:AttachProp('ChargerNozzle')
            end
            Wait(100)
        end
...
RegisterNetEvent("Ethnic-vehicles/client/fuel/return-charger-hose", function()
    FunctionsModule.RequestAnimDict("anim@am_hold_up@male")
    TaskPlayAnim(PlayerPedId(), "anim@am_hold_up@male", "shoplift_high", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
    TriggerEvent("Ethnic-ui/client/play-sound", "putbacknozzle", 0.4)
    Wait(300)
    RopeUnloadTextures()
    DeleteRope(Rope)
    Rope = nil
...
RegisterNetEvent("Ethnic-vehicles/client/fuel/use-pump", function()
    local MenuItems = {
        {
            ['Title'] = 'Gas Pump',
        ...
        },
        {
            ...
            ['Data'] = {['Event'] = 'Ethnic-vehicles/client/fuel/grab-fuel-hose', ['Type'] = 'Client'}
        }
    }
    exports['Ethnic-ui']:OpenContext({ ['MainMenuItems'] = MenuItems })
end)
...
RegisterNetEvent("Ethnic-vehicles/client/fuel/grab-fuel-hose", function()
    HasFuelNozzle = true
    FunctionsModule.RequestAnimDict("anim@am_hold_up@male")
    TaskPlayAnim(PlayerPedId(), "anim@am_hold_up@male", "shoplift_high", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
    TriggerEvent("Ethnic-ui/client/play-sound", "pickupnozzle", 0.4)
    Wait(300)    
    exports['Ethnic-assets']:AttachProp('FuelNozzle')
    local FuelNozzle = exports['Ethnic-assets']:GetSpecificProp('FuelNozzle')
    ...
    CreateThread(function()
        while HasFuelNozzle do
            if not exports['Ethnic-assets']:GetSpecificPropStatus('FuelNozzle') then
                exports['Ethnic-assets']:AttachProp('FuelNozzle')
            end
            Wait(100)
        end
...
RegisterNetEvent("Ethnic-vehicles/client/fuel/return-fuel-hose", function()
    FunctionsModule.RequestAnimDict("anim@am_hold_up@male")
    TaskPlayAnim(PlayerPedId(), "anim@am_hold_up@male", "shoplift_high", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
    TriggerEvent("Ethnic-ui/client/play-sound", "putbacknozzle", 0.4)
    Wait(300)
...
RegisterNetEvent("Ethnic-vehicles/client/fuel/refuel-veh", function()
    ...
    if (100 - FuelLevel) < 5 then
        if not IsElectricVeh then
            exports['Ethnic-ui']:Notify('tank-full', "Fuel tank is already at full capacity..", "error") 
        else
            exports['Ethnic-ui']:Notify('tank-full', "Vehicle is fully charged..", "error") 
        end
        return 
    end

    local IsBillPaid = CallbackModule.SendCallback("Ethnic-vehicles/server/fuel/is-bill-paid", GetVehicleNumberPlateText(Entity))
    ...
    local MenuItems = {
        ...
        },
        {
            ...
            ['Data'] = {['Event'] = 'Ethnic-vehicles/client/fuel/start-refuel', ['Type'] = 'Client', ['Amount'] = 100 - FuelLevel, ['IsElectric'] = IsElectricVeh}
        },
        {
            ['Title'] = "<i style='width: 2.0vh;' class='fas fa-credit-card'></i> Send Bill",
            ['Disabled'] = IsBillPaid,
            ['Data'] = {['Event'] = 'Ethnic-vehicles/client/fuel/send-bill', ['Type'] = 'Client', ['SelfServe'] = false, ['IsElectric'] = IsElectricVeh}
        },
        {
            ['Title'] = "<i style='width: 2.0vh;' class='fas fa-portrait'></i> Self-service",
            ['Disabled'] = IsBillPaid,
            ['Data'] = {['Event'] = 'Ethnic-vehicles/client/fuel/send-bill', ['Type'] = 'Client', ['SelfServe'] = true, ['IsElectric'] = IsElectricVeh}
        },
    }
    exports['Ethnic-ui']:OpenContext({ ['MainMenuItems'] = MenuItems })
end)

RegisterNetEvent("Ethnic-vehicles/client/fuel/send-bill", function(Data)
    local Entity, EntityType, EntityCoords = FunctionsModule.GetEntityPlayerIsLookingAt(4.0, 0.2, 286, PlayerPedId())
    if Entity <= 0 or EntityType ~= 2 then return end
    ...
    if Data['SelfServe'] then
        TriggerServerEvent('Ethnic-vehicles/server/fuel/send-bill', Plate, PlayerModule.GetPlayerData().CitizenId, Amount, Data['IsElectric'])
    else
        SetTimeout(250, function()
            ...
            if Result and Result.Cid then
                TriggerServerEvent('Ethnic-vehicles/server/fuel/send-bill', Plate, Result.Cid, Amount, Data['IsElectric'])
            end
        end)
    end
end)

RegisterNetEvent("Ethnic-vehicles/client/fuel/start-refuel", function(Data)
    local Entity, EntityType, EntityCoords = FunctionsModule.GetEntityPlayerIsLookingAt(4.0, 0.2, 286, PlayerPedId())
    if Entity <= 0 or EntityType ~= 2 then return end
    ...
    CreateThread(function()
        while Fueling and not Data.IsElectric do
            if IsVehicleEngineOn(Entity) then
                ...
                EventsModule.TriggerServer('Ethnic-ui/server/send-explosion', FunctionsModule.GetStreetName())
                exports['Ethnic-ui']:Notify('fuel-error', "Refueling the vehicle with engine on is not a good idea..", 'error')
            end
            Wait(250)
        end
    end)
    ...
    if Data.IsElectric then
        TriggerEvent('Ethnic-vehicles/client/fuel/return-charger-hose')
        Wait(300)
        ...
        TriggerEvent('Ethnic-vehicles/client/fuel/return-fuel-hose')
        Wait(300)
    ...
    Wait(300)
    TriggerEvent('Ethnic-ui/client/notify', 'is-fueling', Data.IsElectric and "Vehicle is currently recharging and will be ready in "..math.ceil((500 * Data.Amount) / 1000).." seconds." or "Vehicle is currently refueling and will be ready in "..math.ceil(((500 * Data.Amount) / 1000)).." seconds.", 'success', 4000)
    ...
    DeleteEntity(Prop)
    if Data.IsElectric then
        TriggerEvent('Ethnic-vehicles/client/fuel/grab-charger-hose')
    else
        TriggerEvent('Ethnic-vehicles/client/fuel/grab-fuel-hose')
    end
    local Plate = GetVehicleNumberPlateText(Entity)
    SetFuelLevel(Entity, 100.0)
    TriggerEvent('Ethnic-ui/client/notify', 'fueled', Data.IsElectric and "Recharged Vehicle" or "Refueled Vehicle", 'success', 4000)
    TriggerServerEvent("Ethnic-vehicles/server/fuel/set-paid-state", Plate)
end)
...
function InitGasStations()
    CreateThread(function()
        for k, v in pairs(Config.GasStations) do
            ...
            exports['Ethnic-polyzone']:CreateBox({ center = v.center, length = v.length, width = v.width, data = v.data }, {
                name = 'gas-station-'..k,
                heading = v.heading,
        ...
        for i, j in pairs(Config.Chargers) do
            ...
            exports['Ethnic-polyzone']:CreateBox({ center = Offset, length = 6.0, width = 6.0}, {
                name = 'charger-'..i,
                heading = j.Coords.w,
            ...
            Options = {
                {
                    ...
                    Label = "Use Charger",
                    EventType = "Client",
                    EventName = "Ethnic-vehicles/client/fuel/use-charger",
                    EventParams = {},
                ...
                },
                {
                    ...
                    EventType = "Client",
                    EventName = "Ethnic-vehicles/client/fuel/return-charger-hose",
                    EventParams = {},
                    Enabled = function(Entity)
            ...
            Options = {
                {
                    ...
                    Label = "Use Pump",
                    EventType = "Client",
                    EventName = "Ethnic-vehicles/client/fuel/use-pump",
                    EventParams = {},
                    Enabled = function(Entity)
                ...
                },
                {
                    ...
                    Label = "Return Hose",
                    EventType = "Client",
                    EventName = "Ethnic-vehicles/client/fuel/return-fuel-hose",
                    EventParams = {},
                    Enabled = function(Entity)
