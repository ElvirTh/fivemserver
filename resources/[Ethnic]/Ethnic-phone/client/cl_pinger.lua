...
RegisterNUICallback("Pinger/SendPing", function(Data, Cb)
    local Success = CallbackModule.SendCallback("Ethnic-phone/server/pinger/do-ping-request", Data, GetEntityCoords(PlayerPedId()))
    Cb(Success)
end)

function Pinger.Render()
    exports['Ethnic-ui']:SendUIMessage("Phone", "RenderPingerApp", {
        HasVPN = exports['Ethnic-inventory']:HasEnoughOfItem('vpn', 1) or exports['Ethnic-inventory']:HasEnoughOfItem('darkmarketdeliveries', 1),
    })
end

RegisterNetEvent('Ethnic-phone/client/pinger/receive', function(Coords, Sender, Name)
    -- Reset Data
    Pinger.Accepted = false
            ...
            Buttons = {
                {
                    ...
                    Event = "Ethnic-phone/client/pinger/answer-pinger",
    ...
    Citizen.SetTimeout(10000, function()
        if Pinger.Accepted == false then
            TriggerEvent('Ethnic-phone/client/pinger/answer-pinger', { Declined = true, Sender = Sender, Coords = Coords })
            TriggerEvent('Ethnic-phone/client/hide-notification', Pinger.Id)
        end
    end)
end)

RegisterNetEvent('Ethnic-phone/client/pinger/answer-pinger', function(Data)
    Pinger.Accepted = true
    TriggerServerEvent('Ethnic-phone/server/pinger/sender-message', Data.Sender, (Data.Declined and 'Ping was denied..' or 'Ping was accepted!'))
