...
function Network.OnConnect(NetworkId)
    TriggerEvent('Ethnic-phone/client/network/on-connect', NetworkId)
...
function Network.OnDisconnect(NetworkId)
    TriggerEvent('Ethnic-phone/client/network/on-disconnect', NetworkId)
...
function Network.Ready()
    if not Network._Ready then
        local Zones = CallbackModule.SendCallback("Ethnic-phone/server/network/get-network-zones")
        exports['Ethnic-polyzone']:CreatePoly(Zones, {
            name = "phone_network",
            ...
            if Network.CurrentNetworkZone ~= nil then
                TriggerEvent('Ethnic-phone/client/network/disconnect')
            end
        end)
...
RegisterNUICallback('Network/GetNetworks', function(Data, Cb)
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/network/get-nearest-networks", Network.CurrentNetworkZone)
    Cb(Result)
end)
...
RegisterNUICallback("Network/Connect", function(Data, Cb)
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/network/connect-to-network", Data)
    if Result[2] then
        Network.OnConnect(Result[2])
        print(Result[2])
        exports['Ethnic-ui']:SendUIMessage('Phone', "SetPhoneNetwork", { Id = Result[2] })
    end
    Cb(Result[1])
end)

RegisterNetEvent('Ethnic-phone/client/network/disconnect', function()
...
RegisterNUICallback('Vpn/GetVPNData', function(Data, Cb)
    local HasVPN = exports['Ethnic-inventory']:HasEnoughOfItem('vpn', 1)
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/vpn/get-vpn-data", HasVPN)
    Cb(Result)
end)

RegisterNUICallback('Vpn/SetVPNData', function(Data, Cb)
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/vpn/set-vpn-data", Data)
    Cb(Result)
end)
