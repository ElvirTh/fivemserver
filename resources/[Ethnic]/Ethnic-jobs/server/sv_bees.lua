...
Citizen.CreateThread(function()
    ...
    EventsModule.RegisterServer("Ethnic-jobs/server/bees/place", function(Source, Coords, Heading)
        local NewHive = {
            Stage = 0,
            Queen = false,
        ...
        TriggerClientEvent('Ethnic-jobs/client/bees/action', -1, 2, NewHive)
    end)
end)
...
Citizen.CreateThread(function()
    Citizen.SetTimeout(750, function()
        while true do
            ...
            TriggerClientEvent('Ethnic-jobs/client/bees/action', -1, 1, Config.BeeHives)
...
RegisterNetEvent("Ethnic-jobs/server/bees/do-stuff", function(Type, HiveId)
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if Type == 'Queen' then
        Config.BeeHives[HiveId].Queen = true
        TriggerClientEvent('Ethnic-jobs/client/bees/action', -1, 3, Config.BeeHives[HiveId])
    elseif Type == 'Destroy' then
        Player.Functions.AddItem('beehive', 1, false, false, true)
        TriggerClientEvent('Ethnic-jobs/client/bees/action', -1, 4, Config.BeeHives[HiveId])
