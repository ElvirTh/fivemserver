...
function Messages.Render()
    ...
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/messages/get-chats")
    exports['Ethnic-ui']:SendUIMessage("Phone", "RenderMessagesChats", {
        Chats = Result
    })
end
...
RegisterNUICallback("Messages/GetChat", function(Data, Cb)
    ...
    local Messages = CallbackModule.SendCallback("Ethnic-phone/server/messages/get-chat-messages", Data)
    Cb(Messages or {})
end)
...
RegisterNUICallback("Messages/SendMessage", function(Data, Cb)
    local Success = CallbackModule.SendCallback("Ethnic-phone/server/messages/send-message", Data)
    Cb(Success)
end)

RegisterNetEvent('Ethnic-phone/client/messages/refresh-chat', function(ChatNumber, NewMessages)
    if not Messages.Active then
        local ContactData = CallbackModule.SendCallback("Ethnic-phone/server/contacts/get-contact", ChatNumber)
        ...
        if Messages.ChatNumber == ChatNumber then
            exports['Ethnic-ui']:SendUIMessage("Phone", "RefreshActiveMessagesChat", {
