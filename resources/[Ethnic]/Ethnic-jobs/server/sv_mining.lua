...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-jobs/server/can-mine-this-spot', function(Source, Cb, MiningSpot)
    ...
    EventsModule.RegisterServer("Ethnic-jobs/server/mining/grab", function(Source, ItemName)
        local Player = PlayerModule.GetPlayerBySource(Source)
        Player.Functions.AddItem(ItemName, 1, false, false, true)
    end)
    
    EventsModule.RegisterServer("Ethnic-jobs/server/mining/receive-goods", function(Source, Mined)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if not Mined then
