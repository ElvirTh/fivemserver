...
RegisterNetEvent('Ethnic-vehicles/client/open-vehicle-menu', function()
    exports['Ethnic-ui']:SetNuiFocus(true, true)
    exports['Ethnic-ui']:SendUIMessage('Vehicle', 'OpenVehicleMenu', {
        Settings = CalculateSettings()
    })
end)

RegisterNetEvent('Ethnic-vehicles/client/switch-seat', function(SeatNumber, IsMenu)
    ...
    if IsVehicleSeatFree(Vehicle, SeatNumber) then
        ...
        if IsMenu then
            Wait(25)
            exports['Ethnic-ui']:SendUIMessage('Vehicle', 'RefreshData', {
                Settings = CalculateSettings()
            })
        end
    end
...
RegisterNetEvent('Ethnic-vehicles/client/toggle-door-data', function(Data)
    local Vehicle = CurrentVehicleData.Vehicle
...
RegisterNetEvent('Ethnic-vehicles/client/toggle-door', function(DoorNumber, IsMenu)
    local Vehicle = CurrentVehicleData.Vehicle
    local LockStatus = GetVehicleDoorLockStatus(Vehicle)
    if LockStatus == 1 or LockStatus == 0 then
...
RegisterNetEvent('Ethnic-vehicles/client/toggle-window', function(WindowNumber, IsMenu)
    local Vehicle = CurrentVehicleData.Vehicle
...
RegisterNUICallback('Vehicle/Action', function(Data, Cb)
    if Data.Action == 'Seat' then
        TriggerEvent('Ethnic-vehicles/client/switch-seat', Data.Number, true)
    elseif Data.Action == 'Door' then
        TriggerEvent('Ethnic-vehicles/client/toggle-door', Data.Number, true)
    elseif Data.Action == 'Window' then
        TriggerEvent('Ethnic-vehicles/client/toggle-window', Data.Number, true)
    end
    Cb('Ok')
end)