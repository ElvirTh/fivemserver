...
RegisterNetEvent("Ethnic-scenes/client/update-scene", function(SceneId, Scene)
    if Scene then
...
function InitScenes()
    ScenesEnabled = GetResourceKvpInt("Ethnic_scene_disabled") ~= nil and GetResourceKvpInt("Ethnic_scene_disabled") == 0 or true

    local Scenes = CallbackModule.SendCallback("Ethnic-scenes/server/get-scenes")
    ...
    KeybindsModule.Add("toggleScenes", "Scenes", "Toggle", "", function(IsPressed)
        if IsPressed then
            ...
            SetResourceKvpInt("Ethnic_scene_disabled", ScenesEnabled and 0 or 1)
            exports['Ethnic-ui']:Notify("scene-action", "Scenes " .. (ScenesEnabled and "activated" or "deactivated"), ScenesEnabled and "success" or "error")
            if ScenesEnabled then StartScenesLoop() end
        end
    end)
            ...
            Citizen.CreateThread(function()
                ...
                if Result then
                    if #Result.Text > Config.MaxTextLength then
                        exports['Ethnic-ui']:Notify("scene-action", "Text is too long.. (" .. #Result.Text .. "/" .. Config.MaxTextLength .. ")", "error")
                        return
                    end

                    if #Result.Color == 0 then Result.Color = "white" end
                    if Config.Colors[Result.Color:lower()] == nil then
                        exports['Ethnic-ui']:Notify("color-not-good", "Invalid color..", "error")
                        return
                    end
                    ...
                    if Dist < 0.1 or Dist > 10.0 then
                        exports['Ethnic-ui']:Notify("long-distance", "Invalid distance..", "error")
                        return
                    end

                    TriggerServerEvent("Ethnic-scenes/server/add-scene", Result, ScenePos)
                end
            end)
        end
    end)
    ...
    KeybindsModule.Add("deleteScene", "Scenes", "Delete closest", "", function(IsPressed)
        ...
        if ClosestScene ~= -1 and ClosestDist <= 4.0 then
            TriggerServerEvent("Ethnic-scenes/server/remove-scene", ClosestScene)
        end
    end)
end
