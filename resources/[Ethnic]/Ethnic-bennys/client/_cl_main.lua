...
AddEventHandler('Modules/client/ready', function()
    if not _Ready then
        ...
        exports['Ethnic-polyzone']:CreateBox(Config.Zones, {
            name = "bennys",
    ...
    TriggerEvent('Modules/client/request-dependencies', {
        ...
        BlipManager = exports['Ethnic-base']:FetchModule("BlipManager")
...
-- RegisterNetEvent("Ethnic-threads/entered-vehicle", function(Vehicle)
--     InVehicle = true
...
--                 local WheelFitment = exports['Ethnic-vehicles']:GetVehicleMeta(Vehicle, 'WheelFitment')
                ...
                if Vehicle ~= 0 then
                    if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
                        if not ShowingInteraction then
                            exports['Ethnic-ui']:SetInteraction("Benny's")
...
RegisterNetEvent('Ethnic-bennys/client/open-bennys', function(Admin)
    if not Admin and not CanOpenBennys(CurrentBennyZone.extra.Authorized) then return exports['Ethnic-ui']:Notify("bennys-use-err", "You can't use this Benny's.", "error") end
    
    local Vehicle = GetVehiclePedIsUsing(PlayerPedId())
    InBennys, IsAdmin = true, Admin
    if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
        local IsMechanicOnline = CallbackModule.SendCallback("Ethnic-bennys/server/is-mechanic-online")
        ...
        IsEmployedAtMechanic = IsGov or exports['Ethnic-business']:IsPlayerInBusiness('Bennys Motorworks') or exports['Ethnic-business']:IsPlayerInBusiness('Hayes Repairs') or exports['Ethnic-business']:IsPlayerInBusiness('Harmony Repairs')
        if IsMechanicOnline and not IsEmployedAtMechanic and not IsAdmin then
            return exports['Ethnic-ui']:Notify("bennys-repair", "Speak to one of the vehicle repair companies for help.", "error")
        end
        ...
        exports['Ethnic-ui']:Notify("bennys-error", "You must be the driver to use Benny's.", "error")
    end
end)
...
function CanOpenBennys(Authorized)
    ...
    if Authorized.Business then
        for k, Business in pairs(Authorized.Business) do
            if exports['Ethnic-business']:IsPlayerInBusiness(Business) then
                return true
            end
        end
    end
...
RegisterNUICallback('PurchaseUpgrade', function(Data, Cb)
    ...
    if Data.Menu == 'Repair' then
        if tonumber(Button.Data.Costs) > Cash then
            return exports['Ethnic-ui']:Notify("bennys-error", "Not enough cash..", "error")
        end
        ...
        if MissingEngineHealth > 50 then
            exports['Ethnic-ui']:ProgressBar('Repairing Engine..', 5000 + (MissingEngineHealth / 50), false, false, true, false, function(DidComplete)
                if DidComplete then
        ...
        if MissingBodyHealth > 50 then
            exports['Ethnic-ui']:ProgressBar('Repairing Body..', 5000 + (MissingBodyHealth / 50), false, false, true, false, function(DidComplete)
                if DidComplete then
        ...
        if not IsAdmin then
            EventsModule.TriggerServer('Ethnic-base/server/remove-money', tonumber(Button.Data.Costs))
            TriggerEvent('Ethnic-ui/client/play-sound', 'bennys-wrench', 0.4)
        ...
        if Button.Id == "Wheels" and (VehicleMods['Wheels'] == Button.Data.WheelType and VehicleMods['ModFrontWheels'] == Button.Data.ModIndex) then
            return exports['Ethnic-ui']:Notify("bennys-error", "Mod is already installed..", "error")
        ...
        if Button.Id == "ResprayColor" then
            ...
            if CurrentRespray == 'Primary' and Button.Data.ModIndex == ColorPrimary then
                return exports['Ethnic-ui']:Notify("bennys-error", "Respray is already applied..", "error")
        ...
        if type(Button.Data.Costs) == 'number' and Costs > 0 and not IsAdmin then
            ...
            EventsModule.TriggerServer('Ethnic-base/server/remove-money', Costs) 
        end

        if Data.Menu == 'ResprayMetallic' or Data.Menu == 'ResprayMetal' or Data.Menu == 'ResprayMatte' then
            TriggerEvent('Ethnic-ui/client/play-sound', 'bennys-respray', 1.0)
        else
            TriggerEvent('Ethnic-ui/client/play-sound', 'bennys-wrench', 0.4)
        end
        ...
        local IsOwner = CallbackModule.SendCallback('Ethnic-vehicles/server/is-veh-owner', Plate)
        local IsEmployedAtMechanic = exports['Ethnic-business']:IsPlayerInBusiness('Bennys Motorworks') or exports['Ethnic-business']:IsPlayerInBusiness('Hayes Repairs')
        ...
        local VehicleMeta = { 
            ...
            ['Nitrous'] = exports['Ethnic-vehicles']:GetVehicleMeta(Vehicle, 'Nitrous'),
            ['Harness'] = exports['Ethnic-vehicles']:GetVehicleMeta(Vehicle, 'Harness'),
        }
        local ModsSaved = VehicleModule.SaveVehicle(Vehicle, Plate, VehicleMeta, exports['Ethnic-vehicles']:IsGovVehicle(Vehicle) and 'Police' or 'None')
        if not ModsSaved then
            -- CloseBennys()
            exports['Ethnic-ui']:Notify("bennys-error", "Failed to save upgrades! (Costs: "..Button.Data.Costs..")", "error", 7000)
        end
