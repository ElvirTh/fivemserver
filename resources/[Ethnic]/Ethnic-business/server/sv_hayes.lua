...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/hayes/get-parts', function(Source, Cb)
        Cb(Config.VehicleParts)
    end)

    CallbackModule.CreateCallback('Ethnic-business/server/hayes/get-new-percentage', function(Source, Cb, Misc, CreateDate, Slot, VehNetId)
        -- TODO: Calculate Item Quality from CreateDate / how much item has decayed and return as percentage
        local NewPercentage = 100
        TriggerEvent('Ethnic-vehicles/server/set-veh-meta', VehNetId, Misc, NewPercentage)
        Cb(NewPercentage)
    end)

    EventsModule.RegisterServer('Ethnic-business/server/hayes/repair-part', function(Source, Plate, Part) 
        Config.VehicleParts[Plate][Part] = 100
        SaveVehicleParts(Plate, Config.VehicleParts[Plate])
        TriggerClientEvent('Ethnic-business/client/hayes/sync-parts', -1, Plate, Config.VehicleParts[Plate])
    end)

    EventsModule.RegisterServer('Ethnic-business/server/hayes/do-parts-damage', function(Source, Plate, Model, PartName) 
...
RegisterNetEvent("Ethnic-business/server/hayes/load-parts", function(Plate, Parts)
    if Parts ~= nil then
...
RegisterNetEvent("Ethnic-business/server/hayes/unload-parts", function(Plate)
    SaveVehicleParts(Plate, Config.VehicleParts[Plate])
