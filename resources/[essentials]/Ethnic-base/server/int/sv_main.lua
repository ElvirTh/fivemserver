...
AddEventHandler('Modules/server/ready', function()
    TriggerEvent('Modules/server/request-dependencies', {
        ...
        -- [ Callbacks ] --
        CallbackModule.CreateCallback('Ethnic-base/server/get-specific-coords', function(Source, Cb, PlayerId)
            local Coords = GetEntityCoords(GetPlayerPed(PlayerId))
            Cb(Coords)
        end)

        CallbackModule.CreateCallback('Ethnic-base/server/get-citizenid-by-source', function(Source, Cb, TargetSource)
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/get-closest-player', function(Source, Cb, Coords, Radius)
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/get-active-players', function(Source, Cb)
            local Players = {}
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/is-me-admin', function(Source, Cb)
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/get-active-players-in-radius', function(Source, Cb, Coords, Radius)
            local ActivePlayers = {}
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/create-vehicle', function(Source, Cb, VehicleName, TargetCoords, Plate)
            local Tries = 100
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/get-vehicle-mods', function(Source, Cb, Plate)
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/save-vehicle-mods', function(Source, Cb, VehicleMods, VehicleDamage, MetaData, Plate, Type)
            -- Type is 'Police' or 'NotPolice' or 'None'
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/remove-item', function(Source, Cb, ItemName, Amount, Slot, ShowInv, VehClass)
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/remove-cash', function(Source, Cb, Amount)
            local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        CallbackModule.CreateCallback('Ethnic-base/server/has-enough-crypto', function(Source, Cb, Type, Amount)
            local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        EventsModule.RegisterServer("Ethnic-base/server/check-nui", function(Source, Bucket)
            ...
            if Player then
                PlayerModule.HasPermission(Source, function(HasPermission)
                    if not HasPermission then
                        TriggerClientEvent('Ethnic-ui/client/notify', Source, "access-devtools", "Devtools detected..", 'error', 3000)
        ...
        EventsModule.RegisterServer("Ethnic-base/server/bucketmanager/set-routing-bucket", function(Source, Bucket)
        ...
        EventsModule.RegisterServer("Ethnic-base/server/remove-money", function(Source, Amount)
            local Player = PlayerModule.GetPlayerBySource(Source)
            if Player.Functions.RemoveMoney('Cash', Amount) then
        ...
        EventsModule.RegisterServer("Ethnic-base/server/remove-crypto", function(Source, Type, Amount)
            local Player = PlayerModule.GetPlayerBySource(Source)
            Player.Functions.RemoveCrypto(Type, Amount)
        end)
        
        EventsModule.RegisterServer("Ethnic-base/server/create-log", function(Source, Log)
            local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        EventsModule.RegisterServer("Ethnic-base/server/receive-paycheck", function(Source)
            local Player = PlayerModule.GetPlayerBySource(Source)
            if not Player then return end

            local IsInBusiness = false
            for k, Name in pairs(NoPayCheckBusinesses) do
                if exports['Ethnic-business']:IsPlayerInBusiness(Player, Name) then
                    IsInBusiness = true 
                end
            end
        ...
        EventsModule.RegisterServer("Ethnic-base/server/save-position", function(Source, Coords)
            local Player = PlayerModule.GetPlayerBySource(Source)
            if not Player then return end
            Player.Functions.SetPosition(Coords)
        end)

        CallbackModule.CreateCallback('Ethnic-base/server/get-crypto-data', function(Source, Cb, Type)
            if Type == 'all' then
                Cb(Shared.Cryptos)
            else
...
RegisterNetEvent("Ethnic-base/server/sync-request", function(Native, ServerId, NetId, ...)
   TriggerClientEvent('Ethnic-base/client/sync-execute', ServerId, Native, NetId, ...)
end)

RegisterNetEvent("Ethnic-base/server/set-meta-data", function(Type, Amount)
    local Source = source
    local Player = PlayerModule.GetPlayerBySource(Source)
	if Player then
...
RegisterNetEvent("Ethnic-base/server/reduce-player-food-water", function()
    local Source = source
    local Player = PlayerModule.GetPlayerBySource(Source)
	if Player then
...
RegisterNetEvent("Ethnic-base/server/set-duty", function(Data)
    local src = source
	local Player = PlayerModule.GetPlayerBySource(src)
    if Player then
        Player.Functions.SetJobDuty(Data['State'])
        TriggerClientEvent('Ethnic-ui/client/notify', src, "duty-set", "You are now "..(Data['State'] and "on" or "off").." duty", Data['State'] and "success" or "error")
    end
end)
...
AddEventHandler('playerSpawned', function()
    local src = source
    TriggerClientEvent('Ethnic-base/client/player-spawned', src)
end)
