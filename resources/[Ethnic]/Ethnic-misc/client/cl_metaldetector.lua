...
RegisterNetEvent("Ethnic-inventory/client/update-player", function()
    if IsDetecting and not exports['Ethnic-inventory']:HasEnoughOfItem('metaldetector', 1) then
        DeleteEntity(DetectorObject)
        IsDetecting = false
    end
end)

RegisterNetEvent("Ethnic-misc/client/used-metaldetector", function()
    ...
    if not MaterialHashes[MaterialHash] then
        IsDetecting = false
        return exports['Ethnic-ui']:Notify("metal-error", "I doubt there is anything here..", "error")
    end
...
RegisterNetEvent("Ethnic-misc/client/used-trowel", function()
    -- Am I detecting?
    if IsDetecting then
        return exports['Ethnic-ui']:Notify("metal-error", "I would put that metal detector away first..", "error")
    end
    ...
    exports['Ethnic-ui']:ProgressBar('Digging...', 8000, false, false, true, true, function(DidComplete)
        ClearPedTasks(PlayerPedId())
        exports['Ethnic-inventory']:SetBusyState(false)
        if DidComplete then
            LastDetectCoords = vector3(0.0, 0.0, 0.0)
            EventsModule.TriggerServer("Ethnic-misc/server/metal-detecting/get-loot")
        end
    end)
end)
    ...
    Citizen.CreateThread(function()
        ...
        while IsDetecting do
            ...
            if Distance < 2.0 and GetCloudTimeAsInt() > LastSound + 3 then
                ...
                EventsModule.TriggerServer('Ethnic-ui/server/play-sound-at-pos', 'metal-detector', LastDetectCoords, 10.0, 0.6)
            end
