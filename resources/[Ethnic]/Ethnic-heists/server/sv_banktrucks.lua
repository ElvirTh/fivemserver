...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-heists/server/banktruck/can-rob-truck', function(Source, Cb, NetId)
    ...
    EventsModule.RegisterServer("Ethnic-heists/server/banktruck/receive-goods", function(Source)
    ...
    EventsModule.RegisterServer("Ethnic-heists/server/banktruck/set-truck-state", function(Source, NetId, Bool)
        TruckStates[NetId] = {}
        TruckStates[NetId].Robbed = Bool
        if Bool then
            TriggerClientEvent('Ethnic-heists/client/banktruck/setup', Source, NetId)
            Citizen.SetTimeout((1000 * 60) * Config.ResetTimes['Banktruck'], function() -- 30 Mins
                TruckStates[NetId] = nil
                TriggerClientEvent('Ethnic-heists/client/banktruck/remove-truck', Source, NetId)
            end)
        end
    end)
end)