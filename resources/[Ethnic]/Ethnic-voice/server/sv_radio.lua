...
function RemovePlayerFromRadio(SiD, ChannelId)
    ...
    if ActiveChannels[ChannelId].Count == 0 then
        ...
        for k, v in pairs(ActiveChannels[ChannelId].Subscribers) do
            TriggerClientEvent("Ethnic-voice/client/radio-removed", k, ChannelId, SiD)
        end
    end

    ChannelSubscribers[SiD] = nil
    TriggerClientEvent("Ethnic-voice/client/radio-disconnect", SiD, ChannelId)
...
function AddPlayerToRadio(SiD, ChannelId, RemoveOld)
    ...
    for k, v in pairs(ActiveChannels[ChannelId].Subscribers) do
        TriggerClientEvent("Ethnic-voice/client/radio-added", k, ChannelId, SiD)
    end

    ChannelSubscribers[SiD] = ChannelId
    ActiveChannels[ChannelId].Subscribers[SiD] = SiD
    TriggerClientEvent('Ethnic-voice/client/radio-connect', SiD, ChannelId, ActiveChannels[ChannelId].Subscribers)
    -- print(SiD,ChannelId)
    -- print("Adding player: " .. SiD  .. " to channel: " .. ChannelId)
end

RegisterNetEvent("Ethnic-voice/server/add-player-to-radio", function(ChannelId, RemoveOld)
    local src = source
    AddPlayerToRadio(src, ChannelId, RemoveOld)
end)

RegisterNetEvent("Ethnic-voice/server/remove-player-from-radio", function(Channel)
