...
Citizen.CreateThread(function()
    ...
    exports['Ethnic-ui']:AddEyeEntry("hunting-sells", {
        Type = 'Entity',
        EntityType = 'Ped',
        SpriteDistance = 5.0,
        ...
        Options = {
            {
                ...
                Label = 'Sell',
                EventType = 'Client',
                EventName = 'Ethnic-jobs/client/hunting/sell',
                EventParams = {},
...
RegisterNetEvent('Ethnic-items/client/do-food-buff', function(Item, Bool)
    if not ItemBuffs[Item] then
        ItemBuffs[Item] = Bool
    end
end)

RegisterNetEvent('Ethnic-jobs/client/hunting/sell', function()
    if not CanSell then return end

    EventsModule.TriggerServer('Ethnic-jobs/server/hunting/sell') CanSell = false
    Citizen.SetTimeout(10000, function()
        CanSell = true
    end)
end)

RegisterNetEvent('Ethnic-items/client/used-hunting-bait', function()
    if InsideHuntingZone() then
        if BaitLastPlaced == 0 or GetGameTimer() > (BaitLastPlaced + (60000 * 10)) then
            ...
            exports['Ethnic-ui']:ProgressBar('Placing Bait..', 5000, false, false, true, true, function(DidComplete)
                if DidComplete then
                    local DidRemove = CallbackModule.SendCallback("Ethnic-base/server/remove-item", 'hunting-bait', 1, false, true)
                    if DidRemove then
            ...
            TriggerEvent('Ethnic-ui/client/notify', "hunting-error", "Your nose can\'t take it yet..", 'error')
        end
    else
        TriggerEvent('Ethnic-ui/client/notify', "hunting-error", "You are not in hunting area..", 'error')
    end
end)

RegisterNetEvent('Ethnic-items/client/used-hunting-knife', function()
    ...
    if IsAnimal and IsPedDeadOrDying(MyAnimal) then
        ...
        TriggerEvent('Ethnic-inventory/client/reset-weapon')
        TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
        exports['Ethnic-ui']:ProgressBar('Skinning..', 5000, false, false, true, true, function(DidComplete)
            if DidComplete then
                ...
                --else
                    EventsModule.TriggerServer('Ethnic-jobs/server/hunting/receive-goods', AnimalName, BaitAnimal, IllegalAnimal, ItemBuffs['Beefdish'])
                --end
                if not exports['Ethnic-police']:IsStatusAlreadyActive('huntbleed') then
                    TriggerEvent('Ethnic-police/client/evidence/set-status', 'huntbleed', 300)
                end
            else
                TriggerEvent('Ethnic-ui/client/notify', "hunting-error", "Preparing Cancelled!", 'error')
            end
            ClearPedTasks(PlayerPedId())
        end)
    else
        TriggerEvent('Ethnic-ui/client/notify', "hunting-error", "No animal found!", 'error')
    end
            ...
            Citizen.SetTimeout(math.random(15000, 60000), function()
                ...
                if LoadedProp then
                    ...
                    if RandomAnimal['Illegal'] and math.random(55) < 15 then
                        ...
                        EventsModule.TriggerServer('Ethnic-ui/server/send-hunting-illegal', StreetLabel)
                    end
