...
Citizen.CreateThread(function()
    ...
    Jobs.Fishing.CurrentFishingSpot = CallbackModule.SendCallback("Ethnic-jobs/server/fishing/get-current-spot")
    ...
    exports['Ethnic-ui']:AddEyeEntry("fishing-sells", {
        Type = 'Entity',
        EntityType = 'Ped',
        SpriteDistance = 5.0,
        ...
        Options = {
            {
                ...
                Label = 'Sell',
                EventType = 'Client',
                EventName = 'Ethnic-jobs/client/fishing/try-sell',
                EventParams = {},
...
RegisterNetEvent('Ethnic-jobs/client/fishing/try-sell', function()
    if not Jobs.Fishing.CanSell then return end

    EventsModule.TriggerServer('Ethnic-jobs/server/fishing-sell') 
...
RegisterNetEvent('Ethnic-jobs/client/fishing/set-fishing-spot', function(FishingCoords)
    Jobs.Fishing.CurrentFishingSpot = FishingCoords
    while BlipModule == nil do Citizen.Wait(100) end
    BlipModule.CreateBlip('fishing-spot', Jobs.Fishing.CurrentFishingSpot, 'Fishing Spot', 540, 26, false, 0.43)
end)

RegisterNetEvent('Ethnic-items/client/used/fishing-rod', function()
    if not NearFishingZone() then
        exports['Ethnic-ui']:Notify("fisning-error", "Not a fishing spot..", 'error')
        return
    end

    if Jobs.Fishing.HasRodAnim then
        exports['Ethnic-ui']:Notify('fishing-holding', "Idiot, you are already holding a fishing rod..", "error")
        return
    end

    if IsPedInAnyVehicle(PlayerPedId()) then
        exports['Ethnic-ui']:Notify('fishing-car', "Do we fish in vehicles these days?", "error")
        return
    end
    ...
    Citizen.SetTimeout(math.random(15000, 65000), function()
        ...
        if CalculatedFish == false then
            ...
            exports['Ethnic-ui']:Notify("fisning-error", "No fish was hooked...", 'error')
            return
        end

        if CalculatedFish == 'Big' or CalculatedFish == 'Special' then SkillTimes = 4 end

        exports['Ethnic-ui']:Notify("fishing-fish", "A little nibble...", 'primary')

        local Outcome = exports['Ethnic-ui']:StartSkillTest(SkillTimes, { 10, 15 }, { 3500, 5500 }, false)
        SetRodAnimation(false)
        if not Outcome then
            exports['Ethnic-ui']:Notify("fisning-error", "It got away!!!!!!!", 'error')
            return
        end

        if exports['Ethnic-phone']:IsJobCenterTaskActive('fishing', 3) then
            TriggerEvent('Ethnic-phone/client/jobcenter/request-task-update', 3, 1)
        end
        EventsModule.TriggerServer('Ethnic-jobs/server/fishing-receive-goods', CalculatedFish)
    end)
end)
    ...
    if Leader == PlayerModule.GetPlayerData().CitizenId then
        ...
        elseif NextTaskId == 4 then -- Go tell them the spot is good
            Citizen.CreateThread(function()
                while true do
                    if #(Config.MeetupPoints['fishing'] - GetEntityCoords(PlayerPedId())) < 10.0 then
                        TriggerEvent('Ethnic-phone/client/jobcenter/request-task-success', 4, true)
                        break
                    end
...
function SetRodAnimation(Bool)
    ...
    exports['Ethnic-assets']:AttachProp('FishingRod')
