...
RegisterNetEvent("Ethnic-items/client/used-wingsuit", function(WingsuitType)
    if Wingsuiting then return end
    local Ped = PlayerPedId()
    local Veh = GetVehiclePedIsIn(Ped, false)
    if Veh ~= 0 then return end
    if GetEntityHeightAboveGround(Ped) < 3 then 
        exports['Ethnic-ui']:Notify('close-ground', 'You are too close to the ground or standing on a flat surface!', 'error')
        return 
    end
    Wingsuiting = true
    usedSuperBoost = false
    local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', WingsuitType, 1, false, true)
    if DidRemove then
        ...
        if FunctionsModule.RequestModel(WingsuitName) then
            ...
            exports['Ethnic-ui']:Notify('wingsuit-model', 'Failed to enable wingsuit!', 'error')
            Wingsuiting = false
        end
    end
end)
