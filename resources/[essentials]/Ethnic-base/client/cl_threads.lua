...
local Events = {
    ['IsPedInAnyVehicle'] = {
        ['false'] = "Ethnic-threads/exited-vehicle",
        ['1'] = "Ethnic-threads/entered-vehicle",
    },
    ['IsTalking'] = {
        ['false'] = "Ethnic-threads/stopped-talking",
        ['1'] = "Ethnic-threads/started-talking",
    },
    ['IsPedSwimmingUnderWater'] = {
        ['false'] = "Ethnic-threads/exited-underwater",
        ['1'] = "Ethnic-threads/entered-underwater",
    },
    ['GetPedArmour'] = {
        ['false'] = "Ethnic-threads/change-armor",
        ['1'] = "Ethnic-threads/change-armor",
    },
    ['GetEntityHealth'] = {
        ['false'] = "Ethnic-threads/change-health",
        ['1'] = "Ethnic-threads/change-health",
    },
        ...
        if LocalPlayer.state.LoggedIn then
            ...
            if Entity and EntityType ~= 0 then
                if Entity ~= CurrentTarget then
                    ...
                    TriggerEvent('Ethnic-base/client/target-changed', CurrentTarget, EntityType, EntityCoords)
                end
            elseif CurrentTarget then
                CurrentTarget = nil
                TriggerEvent('Ethnic-base/client/target-changed', CurrentTarget)
            end
            Citizen.Wait(250)
        else
            Citizen.Wait(450)
...
Citizen.CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.LoggedIn then
            ...
            TriggerServerEvent('Ethnic-base/server/reduce-player-food-water')
        else
            Citizen.Wait(5000)
        end
    end
...
Citizen.CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.LoggedIn then
            ...
            EventsModule.TriggerServer('Ethnic-base/server/receive-paycheck')
        end
    end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if LocalPlayer.state.LoggedIn then
            ...
            EventsModule.TriggerServer('Ethnic-base/server/save-position', GetEntityCoords(PlayerPedId()))
		else
			Citizen.Wait(5000)
		end
	end
...
function OnChange(Native, Value)
    LastValues[Native] = Value
    TriggerEvent(('Ethnic-threads/on-change/%s'):format(Native), Value)
