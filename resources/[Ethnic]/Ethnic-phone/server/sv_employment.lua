...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/get-businesses-by-citizenid', function(Source, Cb)
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/get-specific-business', function(Source, Cb, Name)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/employment/request-customer-charge', function(Source, Cb, Data)
        local TargetPlayer = PlayerModule.GetPlayerByStateId(Data['StateId'])
        if not TargetPlayer then return end
        TriggerClientEvent('Ethnic-phone/client/notification', TargetPlayer.PlayerData.Source, {
            ...
            Buttons = {
                {
                    ...
                    Event = "Ethnic-phone/client/employment/pay-charge-customer",
                    EventType = "Client",
                    EventData = {
                        ['Accepted'] = false,
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/employment/pay-external', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        if CompanyBankId then
            if TPlayer then -- Online
                ...
                -- Remove
                if exports['Ethnic-business']:RemoveMoneyFromBusinessAccount(Data['BusinessName'], Data.Result['amount']) then
                    ...
                    -- Logs
                    exports['Ethnic-financials']:AddTransactionCard({['Title'] = 'Employee Payment sent', ['Who'] = Player.PlayerData.CharInfo.Firstname..' '..Player.PlayerData.CharInfo.Lastname, ['Id'] = CompanyBankId, ['Amount'] = Data.Result['amount'], ['Reason'] = Data.Result['comment'], ['Type'] = 'Transfer'})
                    exports['Ethnic-financials']:AddTransactionCard({['Title'] = 'Employee Payment received', ['Who'] = TargetName, ['Id'] = TargetBankId, ['Amount'] = Data.Result['amount'], ['Reason'] = Data.Result['comment'], ['Type'] = 'Transfer Received'})
                    Cb({
                        Success = true
                    })
                else
...
RegisterNetEvent("Ethnic-phone/server/employment/pay-charge-customer", function(Data)
    local TargetPlayer = PlayerModule.GetPlayerBySource(Data['TargetSource'])
    if not TargetPlayer then return end
    TriggerClientEvent('Ethnic-phone/client/hide-notification', TargetPlayer.PlayerData.Source, TargetPlayer.PlayerData.Source)
    Wait(300)
    if Data['Accepted'] then
        if TargetPlayer.Functions.RemoveMoney('Bank', Data['Data']['Amount'], 'paid-employment-charge-bank') then
            if exports['Ethnic-business']:AddMoneyToBusinessAccount(Data['Data']['BusinessName'], Data['Data']['Amount']) then
                TriggerClientEvent('Ethnic-phone/client/notification', Data['SenderSource'], {
