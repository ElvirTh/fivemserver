...
RegisterNetEvent("Ethnic-business/server/create-badge", function(Data, Type)
    local src = source
...
Citizen.CreateThread(function() 
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/get-specific-business', function(Source, Cb, Name)
        local BusinessData = {}
    ...
    EventsModule.RegisterServer('Ethnic-business/server/create-business', function(Source, BusinessName, StateId, Logo, IsSource) 
        local TPlayer = false
        ...
        if not TPlayer then 
            TriggerClientEvent('Ethnic-ui/client/notify', Source, "not-found", "Target not found..", 'error')
            return
        end
        DatabaseModule.Execute("SELECT * FROM player_business WHERE name = ?", {
            BusinessName
            ...
            if BusinessResult[1] == nil then
                ...
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "gave-business", "Successfully created a business. ("..BusinessName..")", 'success')
                DatabaseModule.Execute("SELECT * FROM player_accounts WHERE name = ?", {
                    ...
                    if AccountResult[1] == nil then
                        DatabaseModule.Insert("INSERT INTO player_accounts (CitizenId, Type, Name, BankId, Balance) VALUES (?, ?, ?, ?, ?)", {
                            ...
                            'Business',
                            BusinessName,
                            exports['Ethnic-financials']:GetUniqueAccountId(),
                            0
                        })
                        TriggerClientEvent('Ethnic-ui/client/notify', Source, "gave-business-acc", "Successfully created a business account. ("..BusinessName..")", 'success')
                    else
                        TriggerClientEvent('Ethnic-ui/client/notify', Source, "already-exists-account", "An bank account with this business name already exists..", 'error')
                    end
                end)
                TriggerClientEvent('Ethnic-ui/client/notify', TPlayer.PlayerData.Source, "received-business", "You are now the CEO of "..BusinessName, 'success')
            else
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "already-exists-busi", "A business with this name already exists..", 'error')
            end
        end)
    end)

    EventsModule.RegisterServer('Ethnic-business/server/set-owner', function(Source, BusinessName, NewOwner) 
        ...
        DatabaseModule.Execute("SELECT * FROM player_business WHERE name = ?", {
            ...
            if BusinessResult[1] ~= nil then
                ...
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "set-owner", "Successfully set "..TPlayer.PlayerData.CharInfo.Firstname..' '..TPlayer.PlayerData.CharInfo.Lastname.." as the owner of "..BusinessName, 'success')
                TriggerClientEvent('Ethnic-ui/client/notify', TPlayer.PlayerData.Source, "received-business", "You are now the CEO of "..BusinessName, 'success')
            else
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "not-found", "Business not found..", 'error')
            end
        end)
    end)

    EventsModule.RegisterServer('Ethnic-business/server/set-logo', function(Source, BusinessName, NewLogo) 
        DatabaseModule.Execute("SELECT * FROM player_business WHERE name = ?", {
            BusinessName
        }, function(BusinessResult)
            if BusinessResult[1] ~= nil then
                ...
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "set-logo", "Successfully set the logo of "..BusinessName, 'success')
            else
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "not-found", "Business not found..", 'error')
            end
        end)
    ...
    EventsModule.RegisterServer('Ethnic-business/server/delete-business', function(Source, BusinessName) 
        DatabaseModule.Execute("SELECT * FROM player_business WHERE name = ?", {
            BusinessName
        }, function(BusinessResult)
            if BusinessResult[1] ~= nil then
                ...
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "deleted-business", "Successfully deleted "..BusinessName, 'success')
            else
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "not-found", "Business not found..", 'error')
            end
        end)
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/create-rank', function(Source, Cb, Data) 
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/edit-rank', function(Source, Cb, Data) 
        DatabaseModule.Execute("SELECT * FROM player_business WHERE name = ?", {
            Data['BusinessName']
        }, function(BusinessResult)
            if BusinessResult[1] ~= nil then
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/remove-rank', function(Source, Cb, Data) 
        DatabaseModule.Execute("SELECT * FROM player_business WHERE name = ?", {
            Data['BusinessName']
        }, function(BusinessResult)
            if BusinessResult[1] ~= nil then
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/add-employee', function(Source, Cb, Data) 
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/remove-employee', function(Source, Cb, Data) 
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/set-employee-rank', function(Source, Cb, Data) 
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/is-employed-at-business', function(Source, Cb)
        local Data = IsEmployedAtBusiness(Source)
        Cb({ IsBusiness = Data[1], Name = Data[2] })
    end)

    CallbackModule.CreateCallback('Ethnic-business/server/get-employed-businesses', function(Source, Cb)
        local BusinessList = {}
    ...
    CallbackModule.CreateCallback('Ethnic-business/server/get-businesses', function(Source, Cb)
        local BusinessList = {}
        DatabaseModule.Execute("SELECT * FROM player_business", {}, function(BusinessData)
            if BusinessData ~= nil then
                for BusinessId, BusinessData in pairs(BusinessData) do
