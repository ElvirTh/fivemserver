...
Citizen.CreateThread(function()
	...
	EventsModule.RegisterServer('Ethnic-inventory/server/open-other-inventory', function(Source, OtherData, Type, Slots, MaxWeight, ExtraData, ExtraType)
		local Player = PlayerModule.GetPlayerBySource(Source)
		local Slots = Slots ~= nil and Slots or DropSlots
		TriggerEvent('Ethnic-inventory/server/inventory-opened', Source, Type, OtherData)
		local TotalData = {}
		if Type == 'Glovebox' or Type == 'Trunk' or Type == 'Stash' or Type == 'Crafting' or Type == 'Temp' then
			if not OpenInventories[string.sub(Type, 1, 1)..OtherData] then
				...
				TriggerClientEvent('Ethnic-inventory/client/open-empty-other', Source)
				return
			end
		elseif Type == 'Player' then
			if not OpenInventories['OT'..OtherData] then
				...
				if OtherPlayer ~= nil then
					...
					TriggerClientEvent('Ethnic-inventory/client/set-inventory-state', tonumber(OtherData), true)
					Citizen.SetTimeout(5, function()
		...
		elseif Type == 'Drop' then
			if not OpenInventories['D'..OtherData['SubType']] then
				TriggerClientEvent('Ethnic-inventory/client/open-inventory-other', Source, OtherData)
				Citizen.SetTimeout(5, function()
		...
		TriggerClientEvent('Ethnic-inventory/client/open-inventory-other', Source, TotalData)
	end)

	CallbackModule.CreateCallback('Ethnic-inventory/server/do-item-data', function(Source, Cb, data)
		...
		if ToInventory == '.my-inventory-blocks' and FromInventory == '.other-inventory-blocks' then
			...
			elseif Type == 'Drop' then
				...
				if Player.Functions.AddItem(Config.Drops[SubType]['Items'][FromSlot]['ItemName'], Amount, ToSlot, Config.Drops[SubType]['Items'][FromSlot].Info, false, 'Inventory') then
					TriggerClientEvent("Ethnic-inventory/client/inventory-log", Source, "Picked up  "..Amount..'x '..Config.Drops[SubType]['Items'][FromSlot]['ItemName'].. " from Drop. ("..SubType..")")
					...
					TriggerClientEvent('Ethnic-inventory/client/update-drops', -1, Config.Drops[SubType], SubType)
					Cb(true)
				else
					Cb(false)
				end
			elseif Type == 'Trunk' or Type == 'Glovebox' or Type == 'Stash' or Type == 'Temp' then
				local DBItems = TypeItems[Type][SubType] ~= nil and TypeItems[Type][SubType] or TriggerClientEvent('Ethnic-inventory/client/force-close', Source)
				...
				if Player.Functions.AddItem(DBItems[FromSlot]['ItemName'], Amount, ToSlot, DBItems[FromSlot].Info, false, 'Inventory') then
					TriggerClientEvent("Ethnic-inventory/client/inventory-log", Source, "Took  "..Amount..'x '..DBItems[FromSlot]['ItemName'].. " out of "..Type.." ("..SubType..")")
			...
			elseif Type == 'Player' then
				...
				if Player.Functions.AddItem(Item, Amount, ToSlot, OtherPlayer.PlayerData.Inventory[FromSlot].Info, false, 'Inventory') then
					...
					TriggerClientEvent("Ethnic-inventory/client/inventory-log", Source, "Took "..Amount..'x '..Item.. " out of inventory of "..OtherPlayer.PlayerData.Name..".")
					OtherPlayer.Functions.RemoveItem(Item, Amount, FromSlot, false)
					if ItemDataFrom['Type'] == 'Weapon' then
						TriggerClientEvent('Ethnic-inventory/client/reset-weapon', OtherPlayer.PlayerData.Source)
					end
					TriggerClientEvent('Ethnic-inventory/client/update-player', OtherPlayer.PlayerData.Source)
					Cb(true)
				else
					Cb(false)
				end
			elseif Type == 'Crafting' then
				...
				if HasCraftingItems(Source, Shared.ItemList[Item]['Cost'], Amount) then
					...
					TriggerClientEvent('Ethnic-inventory/client/craft', Source, Item, Amount, ToSlot, OtherInventoryItems[FromSlot]['Info'], Shared.ItemList[Item]['Cost'])
					Cb('Crafting')
				else
					Cb(false)
				end
		...
		elseif ToInventory == '.other-inventory-blocks' and FromInventory == '.my-inventory-blocks' then
			if Type == 'Drop' then
				if Config.Drops[SubType] ~= nil and Config.Drops[SubType]['Items'] ~= nil then
					...
					if Player.Functions.RemoveItem(Item, Amount, FromSlot, false) then
						TriggerClientEvent("Ethnic-inventory/client/inventory-log", Source, "Threw "..Amount..'x '..Item.. " on ground. ("..SubType..")")
			...
			elseif Type == 'Trunk' or Type == 'Glovebox' or Type == 'Stash' or Type == 'Temp' then
				...
				local Item = Player.PlayerData.Inventory[FromSlot]['ItemName']:lower()
				if Player.Functions.RemoveItem(Item, Amount, FromSlot, false) then
					TriggerClientEvent("Ethnic-inventory/client/inventory-log", Source, "Put "..Amount..'x '..Item.. " in "..Type..". ("..SubType..")")
			...
			elseif Type == 'Player' then
				if OtherPlayer.Functions.AddItem(Player.PlayerData.Inventory[FromSlot].ItemName, Amount, ToSlot, Player.PlayerData.Inventory[FromSlot].Info, false, 'Inventory') then
					if Player.Functions.RemoveItem(Player.PlayerData.Inventory[FromSlot].ItemName, Amount, FromSlot, false) then
						TriggerClientEvent("Ethnic-inventory/client/inventory-log", Source, "Put "..Amount..'x '..Player.PlayerData.Inventory[FromSlot].ItemName.. " in inventory of "..OtherPlayer.PlayerData.Name..".")
						TriggerClientEvent('Ethnic-inventory/client/update-player', OtherPlayer.PlayerData.Source)
						Cb(true)
					else
						Cb(false)
...
RegisterNetEvent('Ethnic-inventory/server/check-other', function(Type, Name)
	...
	elseif Type == 'Player' then
		...
		TriggerClientEvent('Ethnic-inventory/client/set-inventory-state', tonumber(Name), false)
	end
end)

RegisterNetEvent('Ethnic-inventory/server/disable-other', function(PlayerId, State)
	local OtherPlayer = PlayerModule.GetPlayerBySource(tonumber(PlayerId))
	if OtherPlayer ~= nil then
		TriggerClientEvent('Ethnic-inventory/client/set-inventory-state', tonumber(PlayerId), State)
	end
end)

RegisterNetEvent('Ethnic-inventory/server/steal-money', function(TargetId)
	...
	if TargetPlayer then
		...
		if StealCash > 0 then
			if SourcePlayer.PlayerData.Job.Name == 'police' and SourcePlayer.PlayerData.Job.Duty then
				...
				TriggerClientEvent('Ethnic-inventory/client/update-player', Source)
			else
...
RegisterNetEvent('Ethnic-inventory/server/set-player-items', function(PlayerItems)
	local src = source
...
RegisterNetEvent('Ethnic-inventory/server/done-crafting', function(ItemName, Amount, ToSlot, Info, Cost)
	local src = source
	local Player = PlayerModule.GetPlayerBySource(src)
	if not Player then return end
...
RegisterNetEvent('Ethnic-inventory/server/done-combinding', function(FromSlot, FromItem, ToSlot, ToItem, Reward)
	local src = source
	local Player = PlayerModule.GetPlayerBySource(src)
...
RegisterNetEvent('Ethnic-inventory/server/add-new-drop-core', function(Source, ItemName, Amount, Info, Date)
	...
	for k, v in pairs(Config.Drops) do
		if Config.Drops[k] ~= nil and v['Coords'] ~= nil then
			...
			if Distance <= 1.5 then 
				...
				TriggerClientEvent('Ethnic-inventory/client/update-drops', -1, Config.Drops[k], k)
				goto continue
				return
			end
		end
	...
	TriggerClientEvent('Ethnic-inventory/client/update-drops', -1, Config.Drops[RandomId], RandomId)
