...
CreateThread(function()
    ...
    KeybindsModule.Add('toggleBelt', 'Vehicle', 'Toggle Belt', 'B', function(OnPress)
        ...
        local HarnessLevel = exports['Ethnic-vehicles']:GetVehicleMeta(Vehicle, "Harness")
        ...
        if CurrentVehicleData.Class ~= 8 and CurrentVehicleData.Class ~= 13 and CurrentVehicleData.Class ~= 14 and GetEntityModel(Vehicle) ~= GetHashKey('polbike') then
            if HarnessLevel and HarnessLevel > 0.0 then
                ...
                exports['Ethnic-ui']:ProgressBar(Text, Duration, false, false, false, false, function(DidComplete)
                    if DidComplete then
                        if Vehicle == 0 or Vehicle == -1 then return end

                        local CurrentHarnessLevel = exports['Ethnic-vehicles']:GetVehicleMeta(Vehicle, "Harness")
                        SetVehicleMeta(Vehicle, "Harness", CurrentHarnessLevel - 1.0)
        
                        HasHarness, HasBelt = not HasHarness, false
                        TriggerEvent('Ethnic-ui/client/play-sound', HasHarness and 'seatbelt-on' or 'seatbelt-off', 0.45)
                    end
                end)
            else
                HasBelt = not HasBelt
                TriggerEvent('Ethnic-ui/client/play-sound', HasBelt and 'seatbelt-on' or 'seatbelt-off', 0.45)
            end
        end
    end)
end)
    ...
    while true do
        ...
        if LocalPlayer.state.LoggedIn then
            ...
            if IsDisabledControlJustReleased(1, 75) then
                exports['Ethnic-ui']:Notify('unbuckle-harness', "Maybe unbuckle your harness?", "error")
            end
                ...
                if CurrentBody < CurrentBodyDamage or CurrentBody > CurrentBodyDamage and CurrentBody ~= CurrentBodyDamage then
                    if CurrentBody < Config.BodySafeGuard then
                        ...
                        if (CurrentBodyDamage - CurrentBody) > 45.0 and VehicleClass ~= 15 and VehicleClass ~= 13 then
                            EventsModule.TriggerServer('Ethnic-business/server/hayes/do-parts-damage', GetVehicleNumberPlateText(Vehicle), GetEntityModel(Vehicle), 'Body')
                        end
                ...
                if CurrentMotor < CurrentMotorDamage or CurrentMotor > CurrentMotorDamage and CurrentMotor ~= CurrentMotorDamage then
                    if CurrentMotor < Config.EngineSafeGuard then
                        ...
                        if (CurrentMotorDamage - CurrentMotor) > 45.0 and VehicleClass ~= 15 and VehicleClass ~= 13 then
                            ...
                            exports['Ethnic-ui']:Notify('vehicle-engine', "Vehicle engine stalled..", 'error')
                            EventsModule.TriggerServer('Ethnic-business/server/hayes/do-parts-damage', GetVehicleNumberPlateText(Vehicle), GetEntityModel(Vehicle), 'Engine')
...
function SetBeltStatus(Status)
    TriggerEvent('Ethnic-ui/client/play-sound', Status and 'seatbelt-on' or 'seatbelt-off', 0.45)
    HasBelt = Status
end
