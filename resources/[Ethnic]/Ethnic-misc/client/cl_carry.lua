            ...
            if Carrying or Carried then
                if Carrying then
                    ...
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('Ethnic-misc/server/stop-carry')
...
RegisterNetEvent('Ethnic-misc/client/try-carry', function()
    ...
    if ClosestPlayer['ClosestPlayerPed'] == -1 and ClosestPlayer['ClosestServer'] == -1 then
        TriggerEvent('Ethnic-ui/client/notify', 'interaction-error', "An error occured! (No one near!)", 'error')
        return
    end
    if not IsPedInAnyVehicle(PlayerPedId()) and not IsPedInAnyVehicle(ClosestPlayer['ClosestPlayerPed']) then
        TriggerEvent('Ethnic-ui/client/notify', 'carry', "Press E to stop carrying.", 'primary', 10000)
        TriggerServerEvent('Ethnic-misc/server/carry-target', ClosestPlayer['ClosestServer'])
...
RegisterNetEvent('Ethnic-misc/client/getting-carried', function(Source)
...
RegisterNetEvent('Ethnic-misc/client/stop-carry', function()
