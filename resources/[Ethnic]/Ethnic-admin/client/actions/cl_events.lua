...
RegisterNetEvent("Admin:Businesses:Create", function(Result)
    ...
    if CreateTimeout then 
        exports['Ethnic-ui']:Notify("create-timeout", "You are creating businesses too fast..", "error")
        return 
    end

    CreateTimeout = true
    EventsModule.TriggerServer('Ethnic-business/server/create-business', Result['name'], false, Result['logo'], Result['player'])
...
RegisterNetEvent("Admin:Businesses:SetOwner", function(Result)
    ...
    EventsModule.TriggerServer('Ethnic-business/server/set-owner', Result['name'], Result['player'])
end)

RegisterNetEvent("Admin:Businesses:SetLogo", function(Result)
    if not PlayerModule.IsPlayerAdmin() then return end

    EventsModule.TriggerServer('Ethnic-business/server/set-logo', Result['name'], Result['logo'])
end)

RegisterNetEvent("Admin:Businesses:AddEmployee", function(Result)
    if not PlayerModule.IsPlayerAdmin() then return end

    local Data = CallbackModule.SendCallback('Ethnic-business/server/add-employee', {
    ...
    if Data['Success'] then
        exports['Ethnic-ui']:Notify("add-employee-success", 'Successfuly added '..Result['player']..' to '..Result['name']..' as '..Result['rank'], "success")
    else
        exports['Ethnic-ui']:Notify("add-employee-failed", Data['Fail'], "error")
    end
end)
...
RegisterNetEvent("Admin:Businesses:Delete", function(Result)
    ...
    EventsModule.TriggerServer('Ethnic-business/server/delete-business', Result['name'])
end)
...
RegisterNetEvent("Admin:Set:Ammo", function(Result)
   ...
   TriggerEvent('Ethnic-weapons/client/set-ammo', Result['amount'])
end)
...
RegisterNetEvent('Admin:Spawn:Vehicle', function(Result)
    ...
    TriggerEvent("Ethnic-base/client/spawn-vehicle", Result['model'])
end)
...
RegisterNetEvent("Admin:Copy:Coords", function(Result)
    ...
    TriggerEvent('Ethnic-admin/client/copy-to-clipboard', 
...
RegisterNetEvent("Ethnic-admin/client/copy-to-clipboard", function(Text)
    SendNUIMessage({
        Action = 'Copy',
        String = Text,
    })
...
RegisterNetEvent('Ethnic-admin/client/set-model', function(Model)
    if FunctionsModule.RequestModel(Model) then
        ...
        exports['Ethnic-ui']:Notify("model-not-found", 'Model could not be loaded...', 'error')
    end
end)
