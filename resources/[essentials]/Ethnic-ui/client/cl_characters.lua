...
function SendToCharacterScreen(Bool)
    ...
    Citizen.SetTimeout(500, function()
        if inCharacterMenu then
            TriggerEvent('Ethnic-weathersync/client/set-default-weather', 21)
            ...
            local Characters = CallbackModule.SendCallback('Ethnic-ui/server/characters-get')
...
function BuildCharacterProps()
    ...
    for i = 1, 6, 1 do
        ...
        local SkinData = CallbackModule.SendCallback('Ethnic-ui/server/characters/get-skin', cid)
        ...
        if IsCustomSkin then 
            ...
            if ModelLoaded then
                ...
                TriggerEvent('Ethnic-clothing/client/load-clothing', SkinData, Ped)
...
RegisterNUICallback('Characters/GetCharacterNameByCid', function(Data, Cb)
    local Name = CallbackModule.SendCallback('Ethnic-ui/server/characters/get-character-name-by-cid', Data.Cid)
    Cb(Name)
end)
...
RegisterNUICallback('Characters/CreateCharacter', function(Data, Cb)
    ...
    EventsModule.TriggerServer("Ethnic-ui/server/create-character", Data)
end)
...
RegisterNUICallback('Characters/LoadCharacter', function(Data, Cb)
    ...
    Citizen.SetTimeout(450, function()
        ...
        FreezeEntityPosition(PlayerPedId(), false)
        SetEntityCollision(PlayerPedId(), true, true)
        EventsModule.TriggerServer("Ethnic-ui/server/load-character", Data) 
    end)
end)

RegisterNUICallback('Characters/DeleteCharacter', function(Data, Cb)
    EventsModule.TriggerServer("Ethnic-ui/server/delete-character", Data)
...
RegisterNetEvent('Ethnic-ui/client/send-to-character-screen', function()
    SendToCharacterScreen(true)
end)

RegisterNUICallback("Characters/GetFirstEmptyCid", function(Data, Cb)
    local EmptyCid = CallbackModule.SendCallback('Ethnic-ui/server/characters/get-first-empty-cid')
    Cb(EmptyCid)
end)
