...
RegisterNetEvent('Ethnic-business/client/used-gemstone', function(GemData)
    if not NearAppraisal then
        TriggerEvent('Ethnic-ui/client/notify', 'gallery-error', "Where is the appraisal table??", 'error')
        return
    end
...
RegisterNetEvent('Ethnic-polyzone/client/enter-polyzone', function(PolyData, Coords)
    if PolyData.name == 'gallery-gem-ap' then
        ...
        if not ShowingInteraction then
            ...
            exports['Ethnic-ui']:SetInteraction('Appraisal', 'primary')
        end
    else
        return
    end
