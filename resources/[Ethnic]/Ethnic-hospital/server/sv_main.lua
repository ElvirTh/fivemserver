...
Citizen.CreateThread(function() 
    ...
    -- [ Callbacks ] --
    CallbackModule.CreateCallback('Ethnic-hospital/server/is-serverid-dead', function(Source, Cb, ServerId)
    ...
    CallbackModule.CreateCallback('Ethnic-hospital/server/get-samples', function(Source, Cb, Type)
    ...
    CommandsModule.Add("revive", "Revive yourself", {}, false, function(source, args)
        ...
        if Player then
            ...
            TriggerClientEvent('Ethnic-hospital/client/revive', source, true)
        end
    end, "admin")
    ...
    CommandsModule.Add("revive#", "Revive a player", {{Name="Id", Help="Player ID"}}, false, function(source, args)
        if args[1] ~= nil then
            ...
            if Player then
                ...
                Player.Functions.SetMetaData("Water", 100)
                Player.Functions.SetMetaData("Stress", 0)
                TriggerClientEvent('Ethnic-hospital/client/revive', Player.PlayerData.Source, true)
            else
    ...
    EventsModule.RegisterServer("Ethnic-hospital/server/set-dead-state", function(Source, Bool)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if Player ~= nil then
    ...
    EventsModule.RegisterServer("Ethnic-hospital/server/take-blood", function(Source, ServerId)
    ...
    EventsModule.RegisterServer('Ethnic-hospital/server/receive-result', function(Source, Data)
        TriggerClientEvent('Ethnic-chat/client/post-message', Source, "Blood Result #"..Data.Blood, "Type: "..Data.Type, "error")
    end)

    EventsModule.RegisterServer("Ethnic-hospital/server/heal-player", function(Source, ServerId)
        local TargetPlayer = PlayerModule.GetPlayerBySource(ServerId)
        if TargetPlayer ~= nil then
            TriggerClientEvent('Ethnic-hospital/client/heal', TargetPlayer.PlayerData.Source)
        end
    end)

    EventsModule.RegisterServer("Ethnic-hospital/server/revive-player", function(Source, ServerId)
        local TargetPlayer = PlayerModule.GetPlayerBySource(ServerId)
        if TargetPlayer ~= nil then
            ...
            TriggerClientEvent('Ethnic-hospital/client/revive', TargetPlayer.PlayerData.Source, true)
        end
    end)

    EventsModule.RegisterServer("Ethnic-hospital/server/reset-vitals", function(Source)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if Player ~= nil then
            Player.Functions.SetMetaData("Food", 100)
            Player.Functions.SetMetaData("Water", 100)
...
RegisterNetEvent("Ethnic-hospital/server/set-hospital-bed-busy", function(BedId, Bool)
    Config.HospitalBeds[BedId]['Busy'] = Bool
    TriggerClientEvent('Ethnic-hospital/client/set-hospital-bed-busy', -1, BedId, Bool)
end)

RegisterNetEvent("Ethnic-hospital/server/save-vitals", function(PArmor, PHealth)
...
RegisterNetEvent("Ethnic-hospital/server/clear-inventory", function()
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
