...
Citizen.CreateThread(function() 
    ...
    CallbackModule.CreateCallback('Ethnic-housing/server/get-houses', function(Source, Cb)
        DatabaseModule.Execute("SELECT * FROM player_houses", function(HouseData)
            if HouseData ~= nil then
                for k, v in pairs(HouseData) do
                    ...
                    Citizen.Wait(25)
                    TriggerClientEvent('Ethnic-housing/client/sync-house-data', -1, v.house, Config.Houses[v.house])
                end
                Cb(Config.Houses)
            else
                Cb(Config.Houses)
    ...
    CallbackModule.CreateCallback('Ethnic-vehicles/server/get-house-garages', function(Source, Cb)
        local HouseGarages = {}
        DatabaseModule.Execute("SELECT * FROM player_houses", function(HouseData)
            if HouseData ~= nil then
                for k, v in pairs(HouseData) do
    ...
    CallbackModule.CreateCallback('Ethnic-housing/server/has-keys', function(Source, Cb, IsHouse)
    ...
    CommandsModule.Add("addhouse", "Add a house", {}, true, function(source, args)
        ...
        if Player.PlayerData.Job.Name == "realestate" then
            TriggerClientEvent("Ethnic-housing/client/try-add-new-house", source)
        end
    end)

    -- [ Events ] --

    EventsModule.RegisterServer("Ethnic-housing/server/delete-decoration", function(Source, DecorationId, HouseId)
        ...
        TriggerClientEvent('Ethnic-housing/client/sync-house-data', -1, HouseId, Config.Houses[HouseId], true)
    end)
    
    EventsModule.RegisterServer("Ethnic-housing/server/place-decoration", function(Source, HouseId, Model, Coords, Heading)
    ...
    EventsModule.RegisterServer("Ethnic-housing/server/add-house", function(Source, CoordsTable, Interior, Price, StreetLabel)
        local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        TriggerClientEvent('Ethnic-housing/client/sync-house-data', -1, Name, Config.Houses[Name])
...
RegisterNetEvent("Ethnic-housing/server/set-house-location", function(Data, InsideHouse)
    local src = source
                ...
                if v.house == Data['HouseId'] then
                    ...
                    if Locations ~= nil then
                        ...
                        DatabaseModule.Update("UPDATE player_houses SET locations = ? WHERE house = ? ", {json.encode(Locations), Data['HouseId']}, function()
                            ...
                            if InsideHouse then
                                TriggerClientEvent('Ethnic-housing/client/sync-house-data', -1, Data['HouseId'], Config.Houses[Data['HouseId']])
                            end
                        end)
                    end
                end
...
function ToggleDoorLocks(HouseId)
    ...
    TriggerClientEvent('Ethnic-housing/client/sync-house-data', -1, HouseId, Config.Houses[HouseId])
    return Config.Houses[HouseId]['Locked']
end
