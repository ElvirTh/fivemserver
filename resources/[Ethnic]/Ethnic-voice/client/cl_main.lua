...
RegisterNetEvent('Ethnic-base/client/on-login', function()
    ...
    Citizen.SetTimeout(1200, function()
        TriggerEvent('Ethnic-voice/client/voice-state', true)
        InitKeybinds()
    end)
end)

RegisterNetEvent('Ethnic-base/client/on-logout', function()
    TriggerEvent('Ethnic-voice/client/voice-state', false)
end)
...
AddEventHandler('Modules/client/ready', function()
    TriggerEvent('Modules/client/request-dependencies', {
        ...
        OnesyncModule = exports['Ethnic-base']:FetchModule('Onesync')
...
RegisterNetEvent('Ethnic-voice/client/voice-state', function(State)
    Config.VoiceEnabled = State
    TriggerServerEvent("Ethnic-voice/server/connection-state", State)
...
RegisterNetEvent("Ethnic-voice/client/transmission-state", function(ServerId, Context, Transmitting, Effect)
    if Transmissions:ContextExists(Context) then
        ...
        if Context == "Radio" and exports['Ethnic-ui']:IsRadioOn() then
            if TooFar then
                TriggerEvent('Ethnic-ui/client/play-sound', 'radio-distortion', 0.2)
            else
...
RegisterNetEvent('Ethnic-voice/client/proximity-override', function(Id, Mode, Range, Priority)
...
RegisterNetEvent('Ethnic-voice/client/set-muted', function(ServerId, Bool)
...
RegisterNUICallback("ResetMumble", function(Data, Cb)
    exports['Ethnic-ui']:Notify("reset-voice", "Resetted voice!", "success", 3000)
...
function AddGroupToTargetList(Group, Context)
    if Targets:ContextExists(Context) then
        ...
        TriggerServerEvent("Ethnic-voice/server/transmission-state-radio", Group, Context, true, true)
    end
end
...
function AddPlayerToTargetList(ServerId, Context, Transmit)
    if not Targets:TargetContextExist(ServerId, Context) then
        if Transmit then
            TriggerServerEvent("Ethnic-voice/server/transmission-state-radio", ServerId, Context, true, false)
        end
...
function RemoveGroupFromTargetList(Group, Context)
    if Targets:ContextExists(Context) then
        ...
        RefreshTargets()
        TriggerServerEvent("Ethnic-voice/server/transmission-state-radio", Group, Context, false, true)
    end
end
...
function RemovePlayerFromTargetList(ServerId, Context, Transmit, Refresh)
    if Targets:TargetContextExist(ServerId, Context) then
        ...
        if Transmit then
            TriggerServerEvent("Ethnic-voice/server/transmission-state-radio", ServerId, Context, false, false)
        end
        if Refresh then
            RefreshTargets()
        end
...
function CycleVoiceProximity()
    ...
    if Proximity == 3 then
        TriggerEvent('Ethnic-ui/client/set-hud-values', 'Voice', 'Value',  100)
    elseif Proximity == 2 then
        TriggerEvent('Ethnic-ui/client/set-hud-values', 'Voice', 'Value',  66)
    else
        TriggerEvent('Ethnic-ui/client/set-hud-values', 'Voice', 'Value',  33)
    end
end
