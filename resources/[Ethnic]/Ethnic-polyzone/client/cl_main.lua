...
local function _initDebug(poly, options)
    ...
    Citizen.CreateThread(function()
        while isDebug and not poly.destroyed do
            poly:draw()
            if isDebug and poly.lines then
                _drawGrid(poly)
            end
            Citizen.Wait(0)
        end
...
function PolyZone:new(points, options)
    ...
    local poly = {
        ...
        max = vector2(0, 0),
        min = vector2(0, 0),
        minZ = tonumber(points.minZ) or tonumber(options.minZ) or nil,
        maxZ = tonumber(points.maxZ) or tonumber(options.maxZ) or nil,
        useGrid = useGrid,
        gridDivisions = tonumber(options.gridDivisions) or 30,
        debugColors = options.debugColors or {},
        data = options.data or {},
        isPolyZone = true,
        extraData = points.extraData,
...
function PolyZone:Create(isMultiple, points, options, onEnter, onLeave)
    if isMultiple then
        for id, pointcoords in pairs(points) do
            if options.name and PolyZones[options.name] ~= nil and PolyZones[options.name][id] then
                PolyZones[options.name][id]:destroy()
                PolyZones[options.name][id] = nil
            end

            local poly = PolyZone:new(pointcoords, options)
            poly:onPlayerInOut(onEnter, onLeave)
            _initDebug(poly, options)

            if PolyZones[options.name] == nil then PolyZones[options.name] = {} end
            if options.name then PolyZones[options.name][id] = poly end
        end
    else
        if options.name and PolyZones[options.name] then
            PolyZones[options.name]:destroy()
            PolyZones[options.name] = nil
        end
        ...
        if options.name then PolyZones[options.name] = poly end
...
function PolyZone:onPointInOut(getPointCb, onEnter, onLeave, waitInMS)
    ...
    Citizen.CreateThread(function()
        ...
        while not self.destroyed do
            ...
            if newIsInside ~= isInside then
                if newIsInside then
                    if onEnter ~= nil then onEnter(self, point) end
                    TriggerEvent('Ethnic-polyzone/client/enter-polyzone', self, point)
                else
                    if onLeave ~= nil then onLeave(self, point) end
                    TriggerEvent('Ethnic-polyzone/client/leave-polyzone', self, point)
                end
...
function PolyZone:onPlayerInOut(onEnter, onLeave, waitInMS)
    self:onPointInOut(PolyZone.getPlayerPosition, onEnter, onLeave, waitInMS)
end
...
function CreatePoly(polys, options, onEnter, onLeave)
    if isDebug then
        print('---------------------------------')
        print('Creating PolyZone.. Data:')
        print('Options: ' .. json.encode(options))
        print('Multiple Zones: ' .. tostring(options.hasMultipleZones))
        if options.hasMultipleZones then
            print('Zones (' .. #polys .. '): ')
            for zone, points in pairs(polys) do
                print('    Zone ' .. zone .. ':')
                print('    Points: ' .. json.encode(points))
                print(' ')
            end
        else
            print('Points: ' .. json.encode(polys))
        end
        print('Done creating polyzone, nice.')
        print('---------------------------------')
    end

    return PolyZone:Create(options.hasMultipleZones, polys, options, onEnter, onLeave)
end

RegisterNetEvent('Ethnic-base/client/on-logout', function()
    if isDebug then
        isDebug = false
        exports['Ethnic-ui']:Notify('polyzone-debug', "PolyZone debug disabled", "error")
    end
end)

RegisterNetEvent('Ethnic-polyzone/Client/toggle-debug', function()
    isDebug = not isDebug
    exports['Ethnic-ui']:Notify('polyzone-debug', ("You %s PolyZone debug"):format(isDebug and "enabled" or "disabled"), (isDebug and "primary" or "error"))
