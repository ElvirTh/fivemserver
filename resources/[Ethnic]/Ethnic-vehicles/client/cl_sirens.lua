...
function SetSirenCycle(Vehicle, Cycle)
    ...
    if not SirenSound then
        return exports['Ethnic-ui']:Notify("no-siren", "This vehicle does not have this siren tone..", "error")
    end
...
function InitSirens()
    KeybindsModule.Add('toggleLights', 'Vehicle', 'Toggle Emergency Lights', 'Q', function(IsPressed)
        ...
        PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
        TriggerServerEvent('Ethnic-vehicles/server/mute-sirens', NetworkGetNetworkIdFromEntity(Vehicle))
    end)
...
RegisterNetEvent("Ethnic-vehicles/client/mute-sirens", function(NetVeh)
