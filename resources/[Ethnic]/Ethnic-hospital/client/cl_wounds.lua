    ...
    while true do
        ...
        if LocalPlayer.state.LoggedIn and not Config.Dead then
            ...
            if Player.MetaData['Food'] == 0 then
                TriggerEvent('Ethnic-ui/client/notify', "hospital-consumption-error", 'You notice that you feel hungry.', 'error', 4500)
            ...
            if Player.MetaData['Water'] == 0 then
                TriggerEvent('Ethnic-ui/client/notify', "hospital-consumption-error", 'You notice that you feel thirsty.', 'error', 4500)
                local CurrentHealth = GetEntityHealth(PlayerPedId())
                local MinAmount = OnOxy and math.random(1, 2) or math.random(2, 3)
                SetEntityHealth(PlayerPedId(), (CurrentHealth - MinAmount))
            end
        ...
        if LocalPlayer.state.LoggedIn and not Config.Dead and IsBleeding then
            ...
            if IsRunning then
                if math.random(1, 100) < 50 then
                    DoBleed = true
                    TriggerEvent('Ethnic-ui/client/notify', "hospital-blood", 'You notice blood oozing from your body the more you move.', 'error', 4500)
                end
            else
                if math.random(1, 100) < 20 then
                    DoBleed = true
                    TriggerEvent('Ethnic-ui/client/notify', "hospital-blood", 'You notice blood oozing from your body.', 'error', 4500)
                end
            end
            ...
            if DoBleed then
                ...
                Citizen.CreateThread(function()
                    ...
                    TriggerServerEvent("Ethnic-police/server/create-evidence", 'Blood')
                end)
            end
        ...
        if LocalPlayer.state.LoggedIn and not Config.Dead then
            ...
            elseif not IsFalling and FallingHeight ~= nil then
                ...
                if IsFallingRagdoll then
                    if not exports['Ethnic-police']:IsStatusAlreadyActive('sworebody') then
                        TriggerEvent('Ethnic-police/client/evidence/set-status', 'sworebody', 210)
                    end
                end
...
RegisterNetEvent("Ethnic-hospital/client/heal", function()
...
RegisterNetEvent("Ethnic-hospital/client/used-oxy", function()
    if OnOxy then TriggerEvent('Ethnic-ui/client/notify', "hospital-oxy", 'You\'re already on oxy..', 'error') return end
    exports['Ethnic-inventory']:SetBusyState(true)
    exports['Ethnic-ui']:ProgressBar('Popping Oxy ..', 2850, {['AnimName'] = 'pill', ['AnimDict'] = 'mp_suicide', ['AnimFlag'] = 49}, false, false, true, function(DidComplete)
        if DidComplete then
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'oxy', 1, false, true)
            if DidRemove then 
                OnOxy = true
...
RegisterNetEvent('Ethnic-hospital/client/clear-wounds', function()
    ClearPedLastDamageBone(PlayerPedId())
...
RegisterNetEvent('Ethnic-hospital/client/decrease-wounds', function()
...
RegisterNetEvent('Ethnic-hospital/client/clear-bleeding', function()
    ClearEntityLastDamageEntity(PlayerPedId())
    ClearPedLastDamageBone(PlayerPedId())
    IsBleeding = false
end)
    ...
    if BodyPart ~= nil and BodyPart ~= 'NONE' then
        ...
        if Config.BodyHealth[BodyPart] ~= nil then
            ...
            if Config.BodyHealth[BodyPart].Health < 100 then
                ...
                SetEntityHealth(PlayerPedId(), (CurrentHealth - MinAmount))
                TriggerEvent('Ethnic-ui/client/notify', "hospital-pain", 'You\'re '..Config.BodyHealth[BodyPart].Name..' feels painful.', 'error', 4500)
            end
        end
