...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback("Ethnic-casino/server/check-room-ownership", function(Source, Cb)
    ...
    CallbackModule.CreateCallback("Ethnic-casino/server/get-current-wheel-veh", function(Source, Cb)
        Cb(ServerConfig.WheelVehicle)
    end)

    -- Slots

    CallbackModule.CreateCallback("Ethnic-casino/server/slots/check-bet", function(Source, Cb, Amount)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    CallbackModule.CreateCallback("Ethnic-casino/server/slots/check-seat", function(Source, Cb, SeatId)
    ...
    RegisterNetEvent("Ethnic-casino/server/slots/clear-seat", function(SeatId)
        if ActiveSlotSeats[SeatId] ~= nil then
            ActiveSlotSeats[SeatId].Used = false
        end
    end)

    RegisterNetEvent("Ethnic-casino/server/slots/start", function(SeatId, Data)
        local src = source
        ...
        if ActiveSlotSeats[SeatId] then
            if Player.Functions.RemoveMoney("Casino", Data.bet, "slots-bet") then
                ...
                -- end
                TriggerClientEvent('Ethnic-casino/client/slots/start', src, SeatId, TickRate)
    ...
    RegisterNetEvent('Ethnic-casino/server/slots/check-for-win',function(Index, Data, BeginData)
        local src = source
    ...
    CallbackModule.CreateCallback("Ethnic-base/server/get-membership", function(Source, Cb)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer('Ethnic-casino/server/buy-chips', function(Source, Amount, Dirty)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if not Player then return end
    ...
    CallbackModule.CreateCallback('Ethnic-casino/server/withdraw-money', function(Source, Cb, Type)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer('Ethnic-casino/server/transfer-chips', function(Source, StateId, Amount)
        local Player = PlayerModule.GetPlayerBySource(Source)
...
RegisterNetEvent("Ethnic-casino/server/rooms/rent-stop", function()
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if not Player then return Player.Functions.Notify('player-not-found', 'Player not found', 'error') end
...
RegisterNetEvent('Ethnic-casino/server/fetch-storage', function(PolyInfo, Password)
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if not Player then return Player.Functions.Notify('player-not-found', 'Player not found', 'error') end
    DatabaseModule.Execute("SELECT * FROM hotel_rooms WHERE RoomId = ?", { PolyInfo }, function(Result)
        if Result[1] ~= nil and Result[1].RoomInfo ~= nil then
            ...
            if RoomInfo.StoragePass == Password then
                TriggerClientEvent('Ethnic-casino/client/hotel/open-storage', src, PolyInfo)
            else
...
RegisterNetEvent('Ethnic-casino/server/rooms/change-password', function(Password)
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if not Player then return Player.Functions.Notify('player-not-found', 'Player not found', 'error') end
