PreferencesModule = {
    LoadPreference = function()
        ...
        local KVPValue = GetResourceKvpString("Ethnic-preferences")
        ...
        SetResourceKvp("Ethnic-preferences", json.encode(Config.MyPreferences))
        TriggerEvent('Ethnic-preferences/client/update', Config.MyPreferences)
    end,
...
AddEventHandler('Modules/client/ready', function()
    exports['Ethnic-base']:CreateModule("Preferences", PreferencesModule, true)
end)

RegisterNetEvent('Ethnic-ui/client/preferences/toggle-visibility', function(Visibility)
    exports['Ethnic-ui']:SendUIMessage("Preferences", "ToggleVisibility", {
        Bool = Visibility,
...
RegisterNetEvent('Ethnic-ui/client/ui-reset', function()
    TriggerEvent('Ethnic-ui/client/preferences/toggle-visibility', false)
end)
...
RegisterNUICallback("Preferences/SavePreferences", function(Data, Cb)
    ...
    SetResourceKvp("Ethnic-preferences", json.encode(Config.MyPreferences))
    TriggerEvent('Ethnic-preferences/client/update', Config.MyPreferences)
    exports['Ethnic-ui']:Notify("Preferences", "Preferences saved!", "success", 5000)
    Cb('Ok')
end)

RegisterNUICallback("Preferences/ToggleVisibity", function(Data, Cb)
    TriggerEvent("Ethnic-ui/client/preferences/toggle-visibility", Data.Bool)
    Cb('Ok')
end)
                ...
                if Cooldown <= GetGameTimer() then
                    if IsControlJustReleased(0, v.KeyCode) then
                        if #PreferencesModule.GetPreferences().EmoteBinds[v.KeyName] ~= 0 then
                            ...
                            TriggerEvent('Ethnic-animations/client/play-animation', PreferencesModule.GetPreferences().EmoteBinds[v.KeyName])
                        end
                    end
                end
            end
