...
function PolyClothing()
    ...
    for i = 1, #Config.Stores do
        ...
        StoreZones[#StoreZones + 1] =  exports['Ethnic-polyzone']:CreateBox({
            center = Store['Coords'],
            length = Store['Length'],
            width = Store['Width'], 
        }, {
    ...
    for i = 1, #Config.ClothingRooms do
        ...
        RoomZones[#RoomZones + 1] = exports['Ethnic-polyzone']:CreateBox({
            center = Room['Coords'],
            length = Room['Length'],
            width = Room['Width'], 
        }, {
...
RegisterNetEvent('Ethnic-polyzone/client/enter-polyzone', function(PolyData, Coords)
    ...
    InZone = true
    if string.find(ZoneName, 'surgeon') then
        exports['Ethnic-ui']:SetInteraction('[E] Plastic Surgery', 'success')
    elseif string.find(ZoneName, 'clothing') then
        exports['Ethnic-ui']:SetInteraction('[E] Clothing Shop', 'success')
    elseif string.find(ZoneName, 'barber') then
        exports['Ethnic-ui']:SetInteraction('[E] Barber', 'success')
    elseif string.find(ZoneName, 'tattoos') then
        exports['Ethnic-ui']:SetInteraction('[E] Tattoo Shop', 'success')
    else
        ...
        if Room ~= nil then
            PlayerModule.GetPlayerData(function(PlayerData)
                ...
                if JobName == Config.ClothingRooms[ZoneId]['Job'] then
                    ZoneName = ZoneId
                    exports['Ethnic-ui']:SetInteraction('[E] Clothing Room', 'success')
                end
            end)
        end
    end
