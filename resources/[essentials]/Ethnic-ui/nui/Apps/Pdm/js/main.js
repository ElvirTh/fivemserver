...
OpenPdmCatalog = function(Data) {
    ...
    $('.pdm-intro').show(0, function() {
        ...
        setTimeout(function() {
            ...
            $.post('https://Ethnic-pdm/PDM/EnableMouse', JSON.stringify({}));
            PdmOpen = true;
            setTimeout(function() {
...
ClosePdmCatalog = function() {
    ...
    $('.pdm-intro').hide(0, function() {
        ...
        $.post('https://Ethnic-pdm/PDM/Close', JSON.stringify({}));
...
RenderVehicles = function(VehicleData) {
    ...
    $.each(VehicleData, function(Key, Value) {
        ...
        if (!FirstVehicle) {
            ...
            $.post('https://Ethnic-pdm/PDM/SetShowVehicle', JSON.stringify({Vehicle: PdmData.CurrentVehicle}));
        }
    ...
    setTimeout(function() {
        ...
        $.post('https://Ethnic-pdm/PDM/GetStats', JSON.stringify({Model: CurrentShopVehicle.Vehicle}), function(VehicleStats) {
...
SetNewDisplayVehicle = function(NewData) {
    ...
    $.post('https://Ethnic-pdm/PDM/SetShowVehicle', JSON.stringify({Vehicle: NewData.Vehicle}));
    $('.pdm-info-price').text(`$${AddCommas(Price)}.00`);
    ...
    setTimeout(function() {
        $.post('https://Ethnic-pdm/PDM/GetStats', JSON.stringify({Model: NewData.Vehicle}), function(VehicleStats) {
            SetNewDisplayStats(VehicleStats)
        });
    }, 50);
}
...
$(document).on('click', '.pdm-buy-button', function(Event) {
    Event.preventDefault();
    $.post('https://Ethnic-pdm/PDM/BuyVehicle', JSON.stringify({Model: PdmData.CurrentVehicle}));
    ClosePdmCatalog();
});
...
$(document).on({
    keydown: function(Event) {
        if (Event.keyCode == 27 && PdmOpen) {
            ...
            $.post('https://Ethnic-pdm/PDM/DoRPM', JSON.stringify({}));
        }
    },
});