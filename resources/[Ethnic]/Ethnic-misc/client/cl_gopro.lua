...
RegisterNetEvent("Ethnic-misc/client/gopros/view-camera", function(Data)
    local CameraData = GetGoProById(Data.CamId)
    if not CameraData then return exports['Ethnic-ui']:Notify("gopro-error", "Cam does not exist?", "error") end

    local PlayerJob = PlayerModule.GetPlayerData().Job
    if CameraData.IsEncrypted and (PlayerJob.Name ~= 'police' or not PlayerJob.Duty) then return exports['Ethnic-ui']:Notify("gopro-error", "You can't connect to this GoPro..", "error") end
    ...
    exports['Ethnic-ui']:SetInteraction('[ESC] Close', 'error')
...
RegisterNetEvent('Ethnic-misc/client/use-gopro', function(IsEncrypted)
    EntityModule.DoEntityPlacer('prop_spycam', 4.5, false, true, nil, function(DidPlace, Coords, Heading)
        if not DidPlace then return exports['Ethnic-ui']:Notify("GoPro-error", "You stopped placing the GoPro or an error occured..", "error") end
        local DidRemove = CallbackModule.SendCallback("Ethnic-base/server/remove-item", IsEncrypted and 'gopropd' or 'gopro', 1, false, true)
        if DidRemove then
            ...
            EventsModule.TriggerServer('Ethnic-misc/server/gopro-place', Coords, Heading, IsEncrypted, IsVehicle, Vehicle['Vehicle'])
        end
    end)
end)

RegisterNetEvent('Ethnic-misc/client/gopro-action', function(Type, Data)
...
RegisterNetEvent('Ethnic-misc/client/gopro-blurcamera', function(Data, Entity)
    local GoPro = GetGoProId(Entity)
    local Outcome = exports['Ethnic-ui']:StartSkillTest(1, { 1, 5 }, { 12000, 18000 }, false)
    if Outcome then
        TriggerServerEvent("Ethnic-misc/server/gopro-action", GoPro, "SetBlurred", true)
        exports['Ethnic-ui']:Notify("gopro-error", "Smudged camera!", "success")
    else
        exports['Ethnic-ui']:Notify("gopro-error", "Failed..", "error")
    end
end)
...
function InitGoPros()
    ...
    exports['Ethnic-ui']:AddEyeEntry(GetHashKey("prop_spycam"), {
        Type = 'Model',
        Model = 'prop_spycam',
        SpriteDistance = 5.0,
        Options = {
            {
                ...
                Label = 'Smudge',
                EventType = 'Client',
                EventName = 'Ethnic-misc/client/gopro-blurcamera',
                EventParams = '',
                Enabled = function(Entity)
                    return (not exports['Ethnic-misc']:IsGoProBlurred(Entity))
                end,
            },
        }
    })
