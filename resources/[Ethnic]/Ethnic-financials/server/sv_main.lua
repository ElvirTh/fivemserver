...
Citizen.CreateThread(function() 
    ...
    EventsModule.RegisterServer('Ethnic-financials/server/monitor-account', function(Source, AccountData)
    ...
    CallbackModule.CreateCallback('Ethnic-financials/server/get-accounts', function(Source, Cb)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local AccountTable = {}
                ...
                DatabaseModule.Execute("SELECT * FROM player_accounts", {}, function(AccountsData) 
                    if AccountsData[1] ~= nil then
                        for k, v in pairs(AccountsData) do
                            if exports['Ethnic-business']:IsPlayerInBusiness(Player, v.Name) and exports['Ethnic-business']:HasBusinessPermission(Player, v.Name, 'account_access') then
                                local AccountData = {}
                                AccountData['AccountOwner'] = exports['Ethnic-business']:GetBusinessOwnerName(v.Name)
    ...
    CallbackModule.CreateCallback('Ethnic-financials/server/get-current-balance', function(Source, Cb, AccountId)
        local Player = PlayerModule.GetPlayerBySource(Source)
        Cb(GetCurrentAccountBalance(AccountId, Player))
    end)

    CallbackModule.CreateCallback('Ethnic-financials/server/withdraw-money', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    CallbackModule.CreateCallback('Ethnic-financials/server/deposit-money', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local CurrentBalance = GetCurrentAccountBalance(Data['AccountId'])
    ...
    CallbackModule.CreateCallback('Ethnic-financials/server/transfer-money', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
...
RegisterNetEvent('Ethnic-financials/server/sync-main-bank', function(Source, BankAmount)
    local Player = PlayerModule.GetPlayerBySource(Source)
    if not Player then 
