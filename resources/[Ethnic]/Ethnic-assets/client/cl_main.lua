...
RegisterNetEvent('Ethnic-base/client/on-login', function()
	...
	KeybindsModule.Add('EyeWink', 'Player', 'Wink', '', nil, 'Ethnic-menu/client/player-wink')
...
-- RegisterNetEvent('Ethnic-assets/client/set-map-zoom', function()
...
function RemoveLockpickChance(IsAdvanced)
    ...
    if math.random(1, 100) > (IsAdvanced and 50 or 25) then
		EventsModule.TriggerServer('Ethnic-inventory/server/degen-item', exports['Ethnic-inventory']:GetSlotForItem(ItemName), 5.0)
    end
end
...
function LoadMisc()
    ...
    SetDiscordRichPresenceAction(0, 'Discord', 'https://dsc.gg/Ethnic-coll')

	Config.SavedDuiData = CallbackModule.SendCallback('Ethnic-assets/server/get-dui-data')
