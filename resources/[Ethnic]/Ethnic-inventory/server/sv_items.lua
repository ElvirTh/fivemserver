...
Citizen.CreateThread(function()
    ...
    EventsModule.RegisterServer('Ethnic-inventory/server/move-items-to-stash', function(Source, PlayerInv, ToInv)
    ...
    EventsModule.RegisterServer('Ethnic-inventory/server/use-item', function(Source, Slot)
        local Player = PlayerModule.GetPlayerBySource(Source)
            ...
            if FunctionsModule.CanUseItem(ItemData.ItemName) or ItemData['Type'] == 'Weapon' then
                if ItemData.Info.Quality ~= nil then
                    if ItemData.Info.Quality > 0 then
                        TriggerClientEvent('Ethnic-inventory/client/item-box', Source, 'Used', Shared.ItemList[ItemData.ItemName], false)
                        if ItemData['Type'] ~= 'Weapon' then
                            FunctionsModule.UseItem(Source, ItemData)
                            return
                        end
                        TriggerClientEvent('Ethnic-inventory/client/use-weapon', Source, ItemData)
                    else
    ...
    CallbackModule.CreateCallback('Ethnic-inventory/server/has-item-enough-quality', function(Source, Cb, MinQuality, Slot)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer('Ethnic-inventory/server/degen-item', function(Source, Slot, Amount)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local ItemData = Player.Functions.GetItemBySlot(Slot)
        ...
        if CurrentQuality == 0 then
            TriggerClientEvent('Ethnic-inventory/client/on-fully-degen-item', Source, ItemData)
            return
        end
    ...
    EventsModule.RegisterServer('Ethnic-inventory/server/add-item', function(Source, ItemName, Amount, Slot, ItemInfo, Show)
        local Player = PlayerModule.GetPlayerBySource(Source)
