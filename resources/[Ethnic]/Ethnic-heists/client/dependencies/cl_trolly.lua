...
RegisterNetEvent('Ethnic-heists/client/try-trolly-gabbing', function(Nothing, Entity)
    ...
    if Type == 'Fleeca' or Type == 'Paleto' then
        if not Config.Panels[Type][CurrentRobbery.BankKey].Trolly.Busy then
            TriggerServerEvent('Ethnic-heists/server/banks/set-trolly-busy', Type, CurrentRobbery.BankKey, true)
            CanRob = true
        end
    end
    if CanRob then
        TriggerEvent('Ethnic-heists/client/trolly-grab', Type, Entity)
        exports['Ethnic-ui']:ProgressBar('Grabbing..', 40000, false, false, true, false, function(DidComplete)
            if DidComplete then
                EventsModule.TriggerServer('Ethnic-heists/server/banks/receive-trolly-goods', Type)
            end
        end)
    end
end)

RegisterNetEvent('Ethnic-heists/client/trolly-grab', function(Nothing, Entity)
