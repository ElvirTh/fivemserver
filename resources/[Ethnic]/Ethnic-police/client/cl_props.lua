...
RegisterNetEvent("Ethnic-police/client/open-barricademenu", function(Props, ClosestData)
    ...
    for k, v in pairs(Props) do
        table.insert(MenuItems, {
            Title = v.Label,
            Desc = v.Desc,
            Data = { Event = 'Ethnic-police/client/place-barricade', Type = 'Client', Prop = v.Prop },
        })
    end
...
RegisterNetEvent("Ethnic-police/client/place-barricade", function(Data)
    local PlayerData = PlayerModule.GetPlayerData()
    if (PlayerData.Job.Name ~= 'police' and PlayerData.Job.Name ~= 'ems') or not PlayerData.Job.Duty then
        return
    end
    ...
    if Data.Prop == "delete_closest" then
        EventsModule.TriggerServer("Ethnic-police/server/delete-barricade")
        return
    end

	EntityModule.DoEntityPlacer(Data.Prop, 10.0, true, true, nil, function(DidPlace, Coords, Heading)
		if not DidPlace then
            return exports['Ethnic-ui']:Notify("object-error", "You stopped placing the object, or something went wrong..", "error")
        end

        exports['Ethnic-ui']:ProgressBar('Placing object...', 1500, {
            ...
            EventsModule.TriggerServer("Ethnic-police/server/place-barricade", Data.Prop, Coords, Heading)
        end)

    end)
end)

RegisterNetEvent("Ethnic-police/client/set-prop-data", function(Type, BarricadeId, Data)
