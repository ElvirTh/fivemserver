LoadEvidence = async (EvidenceId) => {
    ...
    $.post("https://Ethnic-mdw/MDW/Evidence/GetEvidences", JSON.stringify({}), function(Result){
...
$(document).on('click', '.mdw-evidence > .mdw-block-header > .mdw-block-header-icons > i', function(Event) {
    ...
    if (ClickType == 'Save') {
        if (MdwData.EvidenceData.EditingEvidence) {
            $.post('https://Ethnic-mdw/MDW/Evidence/UpdateEvidence', JSON.stringify({
        ...
        if (MdwData.EvidenceData.EditingEvidence) {
            $.post("https://Ethnic-mdw/MDW/Evidence/DeleteEvidence", JSON.stringify({
                Id: MdwData.EvidenceData.CurrentEditingData.id
            }), function(){
                DoMdwLoader(500, function() {
                    LoadEvidence();
