...
RegisterNetEvent('Ethnic-items/client/used-rental-papers', function(Plate)
    if not HasKeysToVehicle(Plate) then
        SetVehicleKeys(Plate, true)
        exports['Ethnic-ui']:Notify('rental-keys', "Received key for this vehicle!", 'success')
    else
        exports['Ethnic-ui']:Notify('rental-keys', "You already have keys to this vehicle..", 'error')
    end
end)

RegisterNetEvent('Ethnic-vehicles/client/try-rent', function(Data)
    local MenuItems = {}
        ...
        MenuData['SecondMenu'] = {
            {
                ...
                ['CloseMenu'] = true,
                ['Data'] = { 
                    ['Event'] = 'Ethnic-vehicles/client/rent-vehicle', 
                    ['Type'] = 'Client', 
...
RegisterNetEvent('Ethnic-vehicles/client/rent-vehicle', function(Data)
    ...
    if not CanSpawn then
        return exports['Ethnic-ui']:Notify('blocking', "There is a vehicle blocking the spot..", "error")
    end

    local HasPaid = CallbackModule.SendCallback("Ethnic-base/server/remove-cash", Data.Price)
    if HasPaid then
        exports['Ethnic-ui']:ProgressBar('Fixing up papers.. (Don\'t move)', 15000, {}, false, true, true, function(DidComplete)
            if DidComplete then
                local StartRentCoords = GetEntityCoords(PlayerPedId())
                if #(GetEntityCoords(PlayerPedId()) - StartRentCoords) > 2.0 then
                    return exports['Ethnic-ui']:Notify('too-far', "Idiot, you went too far..", "error")
                end
                ...
                if LoadedVehicle then
                    ...
                    if Vehicle ~= nil then        
                        SetTimeout(650, function()
                            EventsModule.TriggerServer('Ethnic-vehicles/server/receive-rental-papers', RandomPlate)
                            exports['Ethnic-vehicles']:SetVehicleKeys(RandomPlate, true, false)
        ...
        exports['Ethnic-ui']:Notify('rental-error', "Need more money than that..", 'error')
    end
end)
...
CreateThread(function()
    exports['Ethnic-ui']:AddEyeEntry("vehicle-rentals", {
        Type = 'Entity',
        EntityType = 'Ped',
        SpriteDistance = 10.0,
        Distance = 5.0,
        ...
        Options = {
            {
                ...
                Label = 'Rent Vehicle',
                EventType = 'Client',
                EventName = 'Ethnic-vehicles/client/try-rent',
    ...
    exports['Ethnic-ui']:AddEyeEntry("boat_rental_puerta", {
        Type = 'Entity',
        EntityType = 'Ped',
        SpriteDistance = 10.0,
        Distance = 1.0,
