...
RegisterNetEvent('Ethnic-police/client/helicam/sync-spotlight', function(Type, NetId, State, Coords)
    if Type == 1 then
...
function InitHelicam()
    ...
    KeybindsModule.Add('heliSpotlight', 'Heli', 'Spotlight', '', function(IsPressed)
        ...
        if EnabledSpotlights[NetId] == nil then
            ...
            TriggerServerEvent('Ethnic-police/server/helicam/sync-spotlight', 3, NetId, EnabledSpotlights[NetId].State, EnabledSpotlights[NetId].Coords)
        else
            EnabledSpotlights[NetId].State = not EnabledSpotlights[NetId].State
            TriggerServerEvent('Ethnic-police/server/helicam/sync-spotlight', 1, NetId, EnabledSpotlights[NetId].State, EnabledSpotlights[NetId].Coords)
        end
...
function ToggleHeliCamera(Bool)
    ...
    DisplayRadar(false)
    exports['Ethnic-ui']:SendUIMessage('Hud', 'SetAppVisiblity', { Visible = false })
    exports['Ethnic-ui']:SendUIMessage('HeliCam', 'SetAppVisiblity', { Visible = true })
    ...
    Citizen.CreateThread(function()
        ...
        while HasHeliCamera and GetVehiclePedIsIn(PlayerPedId()) ~= 0 do
            ...
            if GetGameTimer() > (LastUpdate + 500) then
                ...
                exports['Ethnic-ui']:SendUIMessage('HeliCam', 'SetData', {
                    Zone = Zone,
            ...
            if GetGameTimer() > (LastSpotlight + 150) then
                if EnabledSpotlights[VehToNet(Heli)] and EnabledSpotlights[VehToNet(Heli)].State then
                    ...
                    TriggerServerEvent('Ethnic-police/server/helicam/sync-spotlight', 2, NetId, EnabledSpotlights[NetId].State, EnabledSpotlights[NetId].Coords)
                end
            end
        ...
        Citizen.SetTimeout(200, function()
            if EnabledSpotlights[VehToNet(Heli)] then
                TriggerServerEvent('Ethnic-police/server/helicam/sync-spotlight', 3, VehToNet(Heli), false, EnabledSpotlights[VehToNet(Heli)].Coords)
            end
        end)

        DisplayRadar(true)
        exports['Ethnic-ui']:SendUIMessage('Hud', 'SetAppVisiblity', { Visible = true })
        exports['Ethnic-ui']:SendUIMessage('HeliCam', 'SetAppVisiblity', { Visible = false })
    end)
end
...
function CheckForPlateScan()
	...
	if EntityHit > 0 and IsEntityAVehicle(EntityHit) then
        ...
        exports['Ethnic-ui']:SendUIMessage('HeliCam', 'ScanPlate', {
            Cancel = false,
