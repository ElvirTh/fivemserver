...
LoadDashboard = async () => {
    ...
    $.post("https://Ethnic-mdw/MDW/Profiles/GetWarrents", JSON.stringify({}), function(Warrents){
    ...
    if (MdwData.UserData.IsPublic) {
        ...
        $.post('https://Ethnic-mdw/MDW/Announcements/GetAnnouncements', JSON.stringify({}), function(Result) {
...
$(document).on('click', '.block-three > .mdw-dashboard > .mdw-block-header > .mdw-block-header-icons > i', function(e){
    ...
    if (ClickType == 'Add') {
        DoMdwPicker('Announcements', (Result) => {
            $.post("https://Ethnic-mdw/MDW/Dashboard/CreateAnnouncement", JSON.stringify({
                Title: Result.Title,
                Text: Result.Text,
            }), function(){
