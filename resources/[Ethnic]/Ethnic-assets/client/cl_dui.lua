function GenerateNewDui(URL, Width, Height, DuiId)
    if Config.SavedDuiData[DuiId] == nil then
        ...
        TriggerServerEvent('Ethnic-assets/server/set-dui-data', DuiId, ReturnData)
        return ReturnData
    else
        return false
    end
...
RegisterNetEvent('Ethnic-assets/client/set-dui-url', function(DuiId, URL)
    if Config.SavedDuiData[DuiId] ~= nil then
        local Settings = Config.SavedDuiData[DuiId]
        Config.DuiLinks[DuiId] = URL
        Settings['DuiUrl'] = URL
...
RegisterNetEvent('Ethnic-assets/client/set-dui-data', function(DuiId, DuiData)
    Config.DuiLinks[DuiId] = DuiData['DuiUrl']
    Config.SavedDuiData[DuiId] = DuiData
end)

RegisterNetEvent('Ethnic-assets/client/change-dui-url', function(DuiId)
    if DuiId == "police" then
        DuiId = exports['Ethnic-police']:GetMeetingRoomId()
    else
        DuiId = DuiId
    end
    Citizen.SetTimeout(450, function()
        local Data = {{Name = 'Url', Label = 'URL', Icon = 'fas fa-link'}}
        local DUIInput = exports['Ethnic-ui']:CreateInput(Data)
        if DUIInput['Url'] then
            TriggerServerEvent('Ethnic-assets/server/set-dui-url', DuiId, DUIInput['Url'])
        end
    end)
end)