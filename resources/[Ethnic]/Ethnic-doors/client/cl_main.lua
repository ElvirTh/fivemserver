...
RegisterNetEvent('Ethnic-base/client/on-login', function()
    Citizen.SetTimeout(450, function()
        local Result = CallbackModule.SendCallback('Ethnic-doors/server/get-door-config')
        Config.Doors = Result

        Citizen.SetTimeout(200, function()
            TriggerEvent('Ethnic-doors/client/setup-doors')
            InitElevator()
            InitZones()
        end)
    end)
    ...
    while true do
        ...
        if LocalPlayer.state.LoggedIn and (CurrentDoor ~= nil and (CurrentDoor == 16 or CurrentDoor == 17)) or CurrentDoor == nil and NearBollards ~= nil then
            if IsControlJustReleased(0, 246) then   
                TriggerEvent('Ethnic-doors/client/press-special-door')
            end
        else
            Citizen.Wait(450)
        end 
...
RegisterNetEvent('Ethnic-doors/client/setup-doors', function()
...
RegisterNetEvent('Ethnic-doors/client/sync-doors', function(DoorId, DoorData)
    ...
    if #(GetEntityCoords(PlayerPedId()) - Config.Doors[DoorId].Coords) < 2.0 then
        ...
        exports['Ethnic-ui']:SetInteraction((HasAccess and "[E] %s" or "%s"):format(DoorState and 'Locked' or 'Unlocked'), DoorState and 'error' or 'success', true)
    end
end)
            ...
            while Listening do
                ...
                if CurrentDoorLockState ~= LockState and not IsHidden then
                    if #(GetEntityCoords(PlayerPedId()) - Config.Doors[CurrentDoorId].Coords) < Distance then
                        ...
                        exports['Ethnic-ui']:SetInteraction((HasAccess and "[E] %s" or "%s"):format(LockState and 'Locked' or 'Unlocked'), LockState and 'error' or 'success')
                    end
                end
                if IsControlJustReleased(0, 38) then
                    ...
                    if HasAccess and #(GetEntityCoords(PlayerPedId()) - Config.Doors[CurrentDoorId].Coords) < Distance then
                        if CurrentDoorId ~= 'MRPD_BOLLARDS_01' and CurrentDoorId ~= 'MRPD_BOLLARDS_02' then
                            -- Toggle Locks
                            TriggerServerEvent('Ethnic-doors/server/toggle-locks', CurrentDoorId)
                            if Config.Doors[CurrentDoorId].Connected ~= nil and next(Config.Doors[CurrentDoorId].Connected) then
                                for k, v in pairs(Config.Doors[CurrentDoorId].Connected) do
                                    TriggerServerEvent('Ethnic-doors/server/toggle-locks', v)
                                end
                            end
                            -- Play Sound
                            if Config.Doors[CurrentDoorId].IsGate then
                                ...
                                TriggerEvent('Ethnic-ui/client/play-sound', 'door-lock', 0.55)
                            end
                        end
                    end
                end
...
function HasDoorAccess(DoorId)
    ...
    if Config.Doors[DoorId] ~= nil then
        ...
        for k, v in pairs(Config.Doors[DoorId]['Access']['Business']) do
            if exports['Ethnic-business']:HasPlayerBusinessPermission(v, 'property_keys') then
                return true
            end
        end
        if Config.Doors[DoorId]['Access']['Item'] == nil then return false end
        for k, v in pairs(Config.Doors[DoorId]['Access']['Item']) do
            if exports['Ethnic-inventory']:HasEnoughOfItem(v, 1) then
                return true
            end
        end
    end
