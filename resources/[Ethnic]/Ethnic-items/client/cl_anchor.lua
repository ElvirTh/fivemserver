...
RegisterNetEvent('Ethnic/client/on-login', function()
 	Citizen.SetTimeout(1250, function()
        local Config = CallbackModule.SendCallback('Ethnic-items/server/sync-anchor-config')
        AnchoredBoats = Config
 	end)
end)

RegisterNetEvent('Ethnic-items/client/sync-item-anchor', function(Plate, Toggle, NewAnchoredBoats)
...
RegisterNetEvent('Ethnic-items/client/use-anchor-lower', function(Source) 
    local Vehicle = VehicleModule.GetClosestVehicle()
    if GetVehicleClass(Vehicle['Vehicle']) == 14 then
        exports['Ethnic-inventory']:SetBusyState(true)
        EventsModule.TriggerServer('Ethnic-ui/server/play-sound-on-entity', 'anchorDrop', Vehicle['Vehicle'], 5000)
        exports['Ethnic-ui']:ProgressBar('Lowering Anchor..', 5000, {}, false, false, true, function(DidComplete)
            exports['Ethnic-inventory']:SetBusyState(false)
            if DidComplete then
                TriggerServerEvent('Ethnic-items/server/sync-item-anchor', GetVehicleNumberPlateText(Vehicle['Vehicle']), true)
                TaskPlayAnim(PlayerPedId(), 'missfbi4prepp1', '_bag_drop_garbage_man', 8.0, -8, -1, 16, 0, 0, 0, 0);
                exports['Ethnic-ui']:Notify('anchor-raised', 'Anchor lowered..', 'success')
            end
        end)
    end
end)

RegisterNetEvent('Ethnic-items/client/use-anchor-raise', function() 
    local Vehicle = VehicleModule.GetClosestVehicle()
    EventsModule.TriggerServer('Ethnic-ui/server/play-sound-on-entity', 'anchorRaise', Vehicle['Vehicle'], 5000)
    exports['Ethnic-inventory']:SetBusyState(true)
    exports['Ethnic-ui']:ProgressBar('Raising Anchor..', 5000, {}, false, false, true, function(DidComplete)
        exports['Ethnic-inventory']:SetBusyState(false)
        if DidComplete then
            TriggerServerEvent('Ethnic-items/server/sync-item-anchor', GetVehicleNumberPlateText(Vehicle['Vehicle']), false)
            exports['Ethnic-ui']:Notify('anchor-raised', 'Anchor raised..', 'success')
        end
    end)
end)
                    ...
                    if InVeh then
                        ...
                        if not IsPedSittingInAnyVehicle(PlayerPedId()) then
                            ...
                            if not ShowedNotify then 
                                if Count == 2 then
                                    exports['Ethnic-ui']:Notify('cannot-sail', 'You can sail again when the anchor is raised..', 'error')
                                    ShowedNotify = true
                                    Count = 0
                                end
                            end
