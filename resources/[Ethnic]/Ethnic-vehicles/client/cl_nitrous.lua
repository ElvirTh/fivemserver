...
RegisterNetEvent('Ethnic-items/client/used-nitrous', function()
    SetTimeout(450, function()
        ...
        if IsGovVehicle(Vehicle) then
            exports['Ethnic-ui']:Notify('nitrous-error', "I don\'t think this vehicle should take some nitrous..", 'error')
            return
        end
        if not IsToggleModOn(Vehicle, 18) then
            exports['Ethnic-ui']:Notify('nitrous-error', "Looks like this vehicle doesn\'t even have a turbo installed..", 'error')
            return 
        end

        exports['Ethnic-inventory']:SetBusyState(true)
        exports['Ethnic-ui']:ProgressBar('Nitrous..', 3500, false, false, false, true, function(DidComplete)
            if DidComplete then
                local DidRemove = CallbackModule.SendCallback("Ethnic-base/server/remove-item", 'nitrous', 1, false, true)
                if DidRemove then
                    SetVehicleMeta(Vehicle, "Nitrous", 100.0)
                    EventsModule.TriggerServer('Ethnic-inventory/server/decay-item', 'nitrous', exports['Ethnic-inventory']:GetSlotForItem('nitrous', ''), 100.0)
                end
            end
...
RegisterNetEvent('Ethnic-vehicles/client/set-vehicle-flames', function(Veh, Bool)
...
RegisterNetEvent('Ethnic-vehicles/client/set-vehicle-purge', function(Veh, Bool)
    local Vehicle = NetToVeh(Veh)
    if Bool then
...
RegisterNetEvent('Ethnic-vehicles/client/nitrous-usage', function(OnPress)
    ...
    if Nitrous ~= nil and Nitrous > 0 then
        ...
        exports['Ethnic-ui']:Notify('nitrous', "Set nitrous usage to: "..NitrousStages[NitrousStage])
    end

end)

RegisterNetEvent('Ethnic-vehicles/client/nitrous-type', function(OnPress)
    if not OnPress then return end
    local Vehicle = CurrentVehicleData.Vehicle
    local Plate = CurrentVehicleData.Plate
    if Vehicle == 0 or Vehicle == -1 or not CurrentVehicleData.IsDriver then return end
    ...
    if Nitrous ~= nil and Nitrous > 0 then
        ...
        exports['Ethnic-ui']:Notify('nitrous', "Set nitrous type to: "..NitrousType)
    end
end)

RegisterNetEvent('Ethnic-vehicles/client/nitrous', function(OnPress)
    if OnPress then
        local Vehicle = CurrentVehicleData.Vehicle
        local Plate = CurrentVehicleData.Plate
        if Vehicle == 0 or Vehicle == -1 or not CurrentVehicleData.IsDriver then return end
        ...
        if NitrousLevel ~= nil and NitrousLevel > 0 then
            ...
            if NitrousType == 'Nitrous' then
                ...
                TriggerServerEvent('Ethnic-vehicles/server/set-vehicle-flames', VehToNet(Vehicle), true)
            elseif NitrousType == 'Purge' then
                TriggerServerEvent('Ethnic-vehicles/server/set-vehicle-purge', VehToNet(Vehicle), true)
            end
        ...
        if DoingNitrous then
            ...
            if NitrousType == 'Nitrous' then
                ...
                TriggerServerEvent('Ethnic-vehicles/server/set-vehicle-flames', VehToNet(Vehicle), false)
                SetTimeout(100, function()
                    TriggerServerEvent('Ethnic-vehicles/server/set-vehicle-purge', VehToNet(Vehicle), true)
                    Wait(1500)
                    TriggerServerEvent('Ethnic-vehicles/server/set-vehicle-purge', VehToNet(Vehicle), false)
                end)
            elseif NitrousType == 'Purge' then
                TriggerServerEvent('Ethnic-vehicles/server/set-vehicle-purge', VehToNet(Vehicle), false)
            end
    ...
    CreateThread(function()
        while DoingNitrous do
            ...
            if NewNitrous > 0 then
                ...
                TriggerEvent('Ethnic-ui/client/set-hud-values', 'Nos', 'Value', NewNitrous)
            else
                DoingNitrous = false
...
function InitNitrous()
    ...
    KeybindsModule.Add('useNitrous', 'Vehicle', 'Use Nitrous', '', false, 'Ethnic-vehicles/client/nitrous')
    KeybindsModule.Add('setNitrous', 'Vehicle', 'Set Nitrous Usage', '', false, 'Ethnic-vehicles/client/nitrous-usage')
    KeybindsModule.Add('swapNitrousMode', 'Vehicle', 'Set Nitrous Type', '', false, 'Ethnic-vehicles/client/nitrous-type')
end