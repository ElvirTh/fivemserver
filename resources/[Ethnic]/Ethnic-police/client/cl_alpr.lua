...
RegisterNetEvent('Ethnic-base/client/on-logout', function()
	AlprVisible = false
    exports['Ethnic-ui']:SendUIMessage("ALPR", "SetVisibility", { Visible = AlprVisible})
end)
...
RegisterNetEvent("Ethnic-threads/entered-vehicle", function()
    ...
    if AlprVisible then
        if not exports['Ethnic-vehicles']:IsGovVehicle(Vehicle) then 
            AlprVisible = false
            return 
        end
        ...
        exports['Ethnic-ui']:SendUIMessage("ALPR", "SetVisibility", { Visible = true })
    end
end)
...
RegisterNetEvent("Ethnic-threads/exited-vehicle", function()
    ...
    exports['Ethnic-ui']:SendUIMessage("ALPR", "SetVisibility", { Visible = false })
end)
...
function InitAlpr()
    KeybindsModule.Add('enableAlpr', 'Services', 'Toggle ALPR', '', function(IsPressed)
        ...
        if not exports['Ethnic-vehicles']:IsGovVehicle(Vehicle) then return end
    ...
    KeybindsModule.Add('lockAlpr', 'Services', 'Lock ALPR', '', function(IsPressed)
        ...
        if AlprLocked then
            exports['Ethnic-ui']:Notify("police-alpr", "ALPR Locked!", "error")
        else
            exports['Ethnic-ui']:Notify("police-alpr", "ALPR Unlocked!", "success")
        end
    end, function()end)
end
            ...
            if not AlprLocked then
                ...
                if DoesEntityExist(ForwardVehicle) and IsEntityAVehicle(ForwardVehicle) then
                    ...
                    if LastForwardScan ~= ForwardPlate then
                        local Result = CallbackModule.SendCallback("Ethnic-police/server/is-plate-flagged", ForwardPlate)
                        if Result then
                            if Result.Flagged == 1 then
                                ...
                                Citizen.Wait(100)
                                PlaySoundFrontend(-1, "Beep_Red", "DLC_HEIST_HACKING_SNAKE_SOUNDS", 1)
                                exports['Ethnic-ui']:Notify("police-alpr-front", "Forward vehicle seems to be flagged in the system!", "error")
                            end
                        end
                    end
                ...
                if DoesEntityExist(BackwardVehicle) and IsEntityAVehicle(BackwardVehicle) then
                    ...
                    if LastBackwardScan ~= BackwardPlate then
                        local Result = CallbackModule.SendCallback("Ethnic-police/server/is-plate-flagged", BackwardPlate)
                        if Result then 
                            if Result.Flagged == 1 then
                                ...
                                Citizen.Wait(100)
                                PlaySoundFrontend(-1, "Beep_Red", "DLC_HEIST_HACKING_SNAKE_SOUNDS", 1)
                                exports['Ethnic-ui']:Notify("police-alpr-front", "Backward vehicle seems to be flagged in the system!", "error")
                            end
                        end
                    end
                ...
                exports['Ethnic-ui']:SendUIMessage("ALPR", "SetValues", {
