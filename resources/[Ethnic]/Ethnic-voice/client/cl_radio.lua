...
RegisterNetEvent("Ethnic-voice/client/radio-connect", function(RadioId, Subscribers)
...
RegisterNetEvent("Ethnic-voice/client/radio-disconnect", function(RadioId)
...
RegisterNetEvent("Ethnic-voice/client/radio-added", function(RadioId, ServerId)
    if not RadioChannels[RadioId] then return end
...
RegisterNetEvent("Ethnic-voice/client/radio-removed", function(RadioId, ServerId)
    if not RadioChannels[RadioId] then return end
...
function StartRadioTransmission()
    ...
    if exports['Ethnic-ui']:IsRadioOn() and CurrentChannel ~= nil and CurrentRadioId > 0 and not PlayerData.MetaData['Dead'] and not PlayerData.MetaData['Handcuffed'] then
...
function StopRadioTransmission(Forced)
    local PlayerData = PlayerModule.GetPlayerData()
    if not exports['Ethnic-ui']:IsRadioOn() or CurrentRadioId <= 0 then return end
...
function PlayRadioClick(Bool)
    ...
    if Bool then
        TriggerEvent('Ethnic-ui/client/play-sound', 'radio-01-on', RadioClickVolume)
    else
        TriggerEvent('Ethnic-ui/client/play-sound', 'radio-01-off', RadioClickVolume)
    end
end
