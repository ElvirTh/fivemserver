        ...
        if NetworkIsPlayerActive(PlayerId()) then  
            if LocalPlayer.state.LoggedIn then    
                if IsEntityDead(PlayerPedId()) and not Config.Dead then
                    ...
                    TriggerEvent('Ethnic-hospital/client/do-dead-on-player', false)
                else
                    Citizen.Wait(200)
                end
            else
...
RegisterNetEvent('Ethnic-hospital/client/do-dead-on-player', function(Forced)
    ...
    TriggerEvent('Ethnic-animations/client/reset-chair')
    TriggerEvent('Ethnic-police/client/reset-escort')
    TriggerEvent('Ethnic-phone/client/close-phone')

    TriggerEvent('Ethnic-hospital/client/on-player-death')
    ...
    Citizen.SetTimeout(1500, function()
        ...
        EventsModule.TriggerServer('Ethnic-ui/server/send-civ-injured', StreetLabel)
    end)
end)

RegisterNetEvent('Ethnic-hospital/client/revive', function(UseAnim)
    ...
    if UsingAnim then
        TriggerEvent('Ethnic-hospital/client/revive-anim')
    else
        ClearPedTasks(PlayerPedId())
    end
    ...
    EventsModule.TriggerServer('Ethnic-hospital/server/set-dead-state', false)
    PressingTime, Doingtimer = 5, false
    Config.InHospitalBed = false
    Config.Dead = false
end)

RegisterNetEvent('Ethnic-hospital/client/revive-anim', function()
    ClearPedTasks(PlayerPedId())
    Citizen.SetTimeout(250, function()
        ...
        Citizen.Wait(1850)
        ClearPedTasks(PlayerPedId())
        TriggerEvent('Ethnic-assets/client/set-my-walkstyle')
    end)
end)
...
function IsTargetDead(ServerId)
    local IsDead = CallbackModule.SendCallback('Ethnic-hospital/server/is-serverid-dead', ServerId)
    return IsDead
end
            ...
            elseif Config.Timer <= 0 then
                if IsControlPressed(0, 38) then
                    ...
                    if PressingTime <= 0 then
                        if exports['Ethnic-police']:IsInJailZone() and exports['Ethnic-police']:IsInJail() then
                            TriggerEvent('Ethnic-hospital/client/revive', true)
                        else
                            TriggerEvent('Ethnic-hospital/client/try-send-to-bed', true)
                        end
                        Citizen.Wait(500)
                    end
                else
