    ...
    while true do
        ...
        if LocalPlayer.state.LoggedIn and PlayerData ~= nil and PlayerData.Job ~= nil then
            if PlayerData.Job.Name == 'police' and PlayerData.Job.Duty then
                if exports['Ethnic-items']:IsInPDCam() or (IsPlayerFreeAiming(PlayerId()) and GetSelectedPedWeapon(PlayerPedId()) == GetHashKey("WEAPON_FLASHLIGHT")) then
    ...
    while true do
        ...
        if LocalPlayer.state.LoggedIn and CurrentStatusList ~= nil then
            for k, v in pairs(CurrentStatusList) do
                ...
                TriggerServerEvent('Ethnic-police/server/evidence/set-status', CurrentStatusList)
            end
            Citizen.Wait(5000)
        else
            Citizen.Wait(450)
...
AddEventHandler('gameEventTriggered', function(Name, Args)
    ...
    if Name == "CEventNetworkEntityDamage" and IsMeleeAttack and IsSelfAttacker and GetCloudTimeAsInt() > NextMeleeAction then
        ...
        if math.random(1, 100) < 30 then
            ...
            EventsModule.TriggerServer('Ethnic-ui/server/send-fighting-progress', StreetLabel, false)
    ...
    if Name == "CEventNetworkEntityDamage" and IsPedArmed(PlayerPedId(), 1) and IsSelfAttacker and GetCloudTimeAsInt() > NextStabbingAction then
        local StreetLabel = FunctionModule.GetStreetName()
        if  math.random(1, 100) < 30 then
            EventsModule.TriggerServer('Ethnic-ui/server/send-fighting-progress', StreetLabel, true)
    ...
    if Name == "CEventNetworkEntityDamage" and IsPedArmed(PlayerPedId(), 6) and not IsExemptWeapon(Args[7]) and IsSelfAttacker and GetCloudTimeAsInt() > NextShootingAction then
        if (PlayerData ~= nil and PlayerData.Job.Name ~= 'police' and PlayerData.Job.Duty) then
            ...
            EventsModule.TriggerServer('Ethnic-ui/server/send-shooting-progress', StreetLabel, IsInVehicle, exports['Ethnic-vehicles']:GetVehicleDescription())
            NextShootingAction = GetCloudTimeAsInt() + 100
        end
    end
end)

RegisterNetEvent('Ethnic-police/client/open-police-evidence', function()
    Citizen.SetTimeout(450, function()
        local Data = {{Name = 'EviId', Label = 'Evidence Number', Icon = 'fas fa-sort-numeric-up-alt'}}
        local EvidenceInput = exports['Ethnic-ui']:CreateInput(Data)
        if EvidenceInput['EviId'] then
            if exports['Ethnic-inventory']:CanOpenInventory() then
                EventsModule.TriggerServer('Ethnic-inventory/server/open-other-inventory', "evidence_"..EvidenceInput['EviId'], 'Stash', 30, 250.0)
            end
        else
            TriggerEvent('Ethnic-ui/client/notify', 'evidence-error', "An error occured! (You can\'t leave this empty)", 'error')
        end
    end)
end)

RegisterNetEvent('Ethnic-police/client/try-status-self', function()
    exports['Ethnic-ui']:ProgressBar('Checking Status..', 3000, false, false, false, true, function(DidComplete)
        if DidComplete then
            TriggerServerEvent('Ethnic-police/server/get-target-status', GetPlayerServerId(PlayerId()))
        end
...
RegisterNetEvent('Ethnic-police/client/try-status', function()
    ...
    if ClosestPlayer['ClosestPlayerPed'] == -1 and ClosestPlayer['ClosestServer'] == -1 then
        TriggerEvent('Ethnic-ui/client/notify', 'evidence-error', "An error occured! (No one near!)", 'error')
        return
    end
    exports['Ethnic-ui']:ProgressBar('Checking Status..', 3000, false, false, false, true, function(DidComplete)
        if DidComplete then
            TriggerServerEvent('Ethnic-police/server/get-target-status', ClosestPlayer['ClosestServer'])
        end
        ClearPedTasks(PlayerPedId())
    end)
end)

RegisterNetEvent('Ethnic-police/client/try-finger-test', function()
    local ClosestPlayer = PlayerModule.GetClosestPlayer(nil, 2.0)
    if ClosestPlayer['ClosestPlayerPed'] == -1 and ClosestPlayer['ClosestServer'] == -1 then
        TriggerEvent('Ethnic-ui/client/notify', 'evidence-error', "An error occured! (No one near!)", 'error')
        return
    ...
    exports['Ethnic-ui']:ProgressBar('Finger Scanning..', 5000, false, false, false, true, function(DidComplete)
        if DidComplete then
            TriggerServerEvent('Ethnic-police/server/finger-result', ClosestPlayer['ClosestServer'])
        end
        ClearPedTasks(PlayerPedId())
    end)
end)

RegisterNetEvent('Ethnic-police/client/try-gsr-test', function()
    local ClosestPlayer = PlayerModule.GetClosestPlayer(nil, 2.0)
    if ClosestPlayer['ClosestPlayerPed'] == -1 and ClosestPlayer['ClosestServer'] == -1 then
        TriggerEvent('Ethnic-ui/client/notify', 'evidence-error', "An error occured! (No one near!)", 'error')
        return
    ...
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_STAND_MOBILE", 0, 1)
    exports['Ethnic-ui']:ProgressBar('GSR Testing..', 5000, false, false, false, true, function(DidComplete)
        if DidComplete then
            TriggerServerEvent('Ethnic-police/server/gsr-result', ClosestPlayer['ClosestServer'])
        end
        ClearPedTasks(PlayerPedId())
    end)
end)

RegisterNetEvent('Ethnic-police/client/evidence/set-status', function(StatusName, Time)
    if Time > 0 and StatusList[StatusName] ~= nil and not IsStatusAlreadyActive(StatusName) then
        ...
        if StatusList[StatusName].Show then
            TriggerEvent('Ethnic-chat/client/post-message', 'Status', StatusList[StatusName].Text, 'warning')
        end
    end
    TriggerServerEvent('Ethnic-police/server/evidence/set-status', CurrentStatusList)
end)

RegisterNetEvent('Ethnic-items/client/used-evidence', function()
    if EvidenceEnabled then 
        if CanCollect then -- Prevent Spam
            CanCollect = false
            local ClosestEvidence = GetClosestEvidence()
            if ClosestEvidence ~= false then
                EventsModule.TriggerServer('Ethnic-police/server/receive-evidence', ClosestEvidence)
            end
        end
    else
        TriggerEvent('Ethnic-ui/client/notify', 'evidence-error', "An error occured! (You can\'t use this in this state)", 'error')
    end
end)

RegisterNetEvent('Ethnic-police/client/set-evidence', function(EvidenceId, EvidenceData)
    Evidence[EvidenceId] = EvidenceData
end)

RegisterNetEvent('Ethnic-police/client/can-collect-again', function()
    CanCollect = true
end)
