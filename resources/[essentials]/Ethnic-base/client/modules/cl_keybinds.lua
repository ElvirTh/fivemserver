...
KeybindsModule = {
    Add = function(CommandString, Category, Description, DefaultParameter, OnKey, Event, IsHold, DefaultWrapper)
        local Pressed = false
        RegisterCommand('+Ethnic_'..CommandString, function()
        ...
        RegisterKeyMapping('+Ethnic_'..CommandString, Category..' - ' .. Description, DefaultWrapper or 'keyboard', DefaultParameter)
    end,
    ...
    GetCustomizedKey = function(CommandString)
        local RawKey = GetControlInstructionalButton(2, GetHashKey("+Ethnic_" .. CommandString) | 0x80000000, true)
