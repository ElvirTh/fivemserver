...
RegisterNetEvent("Ethnic-pool:setTableState", function(TableData)
    local src = source 
    ...
    if not TableGames[GameId] or not TableGames[GameId]["player"] then
        if Config["PayForSettingBalls"] then
            TriggerEvent("Ethnic-pool:payForPool", src, function()
...
RegisterNetEvent("Ethnic-pool:internalPlayerHasTakenCue", function(cueNetId, Coords)
    local src = source
    TriggerEvent("Ethnic-pool:playerHasTakenCue", src)
    TablePlayers[src] = {cueNetId, Coords}
    TriggerEvent("Ethnic-pool:onTakeCue", src)
end)

RegisterNetEvent("Ethnic-pool:internalDeleteCue", function()
    local src = source 
    StopPlaying(src)
end)

RegisterNetEvent("Ethnic-pool:registerTable", function(pos)
...
RegisterNetEvent("Ethnic-pool:requestTurn", function(GameId)
    local src = source 
    if TableGames[GameId] and not TableGames[GameId]["player"]then
        TableGames[GameId]["player"] =src 
        TableGames[GameId]["lastPlayer"] =src
        SyncGames(GameId)
        TriggerClientEvent("Ethnic-pool:turnGranted", src, GameId)
    end 
end)

RegisterNetEvent("Ethnic-pool:releaseControl", function(GameId)
    local src = source 
...
RegisterNetEvent("Ethnic-pool:syncFinalTableState", function(GameId, balls)
    local src = source 
    if TableGames[GameId] and TableGames[GameId]["player"] == src then
        TableGames[GameId]["player"] = nil
...
RegisterNetEvent("Ethnic-pool:syncCueBallVelocity", function(GameId, cueBallPosition, cueBallVelocity, hitStrength)
    local src = source 
    if TableGames[GameId] and TableGames[GameId]["player"] == src then
...
RegisterNetEvent("Ethnic-pool:setBallInHandData", function(GameId, NewPos)
    local src = source 
    if TableGames[GameId] and TableGames[GameId]["player"] == src then
        local BallId = TableGames[GameId]["cueBallIdx"]
...
RegisterNetEvent("Ethnic-pool:pocketed", function(GameId, ballNum)
    local src = source
    if TableGames[GameId] and TableGames[GameId]["lastPlayer"] == src then
        ...
        TriggerClientEvent("Ethnic-pool:internalNotification", -1, src, GameId, FormattedString)
    end
end) 

RegisterNetEvent("Ethnic-pool:ballInHandNotify", function(GameId)
    local src = source
    if TableGames[GameId] and TableGames[GameId]["lastPlayer"] == src then
        TriggerClientEvent("Ethnic-pool:internalNotification", -1, src, GameId, Config["Text"]["BALL_IN_HAND_NOTIFY"])
    end 
end)

RegisterNetEvent("Ethnic-pool:requestTables", function()
    local src = source
    for addr, GameData in pairs(TableGames) do
        TriggerClientEvent("Ethnic-pool:syncTableState", src, addr, GameData)
        Wait(1000)
    end
end)
...
SyncDistance = function(GameId, src)
    ...
    for _, ServerId in ipairs(GetPlayers()) do
        if src ~= tonumber(ServerId) then
            ...
            Distance = Distance - 1
            TriggerClientEvent("Ethnic-pool:syncTableState", ServerId, GameId, TableGames[GameId])
        end
    end
end

SyncGames = function(GameId, isCueBallHit, hitStrength)
    TriggerClientEvent("Ethnic-pool:syncTableState", -1, GameId, TableGames[GameId], isCueBallHit, hitStrength)
end 
...
StopPlaying = function(ServerId)
    if TablePlayers[ServerId] then
        ...
        TriggerEvent("Ethnic-pool:onReturnCue", ServerId)
