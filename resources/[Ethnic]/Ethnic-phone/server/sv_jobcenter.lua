...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/jobcenter/is-vpn-required', function(Source, Cb, Job)
        Cb(ServerConfig.Jobs[Job]["RequiresVPN"])
    end)

    CallbackModule.CreateCallback('Ethnic-phone/server/jobcenter/get-jobs', function(Source, Cb)
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/jobcenter/get-groups', function(Source, Cb, Job)
        Cb(ServerConfig.Groups[Job])
    end)

    CallbackModule.CreateCallback('Ethnic-phone/server/jobcenter/create-group', function(Source, Cb, Job)
        if Player then
            local Player = PlayerModule.GetPlayerBySource(Source)
            ...
            TriggerClientEvent('Ethnic-phone/client/jobcenter/refresh-group', Source, NewGroup)
            Cb(true)
        else
            Cb(false)
        end
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/jobcenter/start-job-offer', function(Source, Cb, Job)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if ServerConfig.Jobs[Job]['IsAvailable'] then
            ...
            for _, MemberData in pairs(Group['Members']) do
                TriggerClientEvent('Ethnic-phone/client/jobcenter/refresh-group', MemberData['Source'], Group)
                TriggerClientEvent('Ethnic-phone/client/jobcenter/start-job-offer', MemberData['Source'])
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/jobcenter/request-join', function(Source, Cb, Job, Data)
        ...
        TriggerClientEvent('Ethnic-phone/client/notification', LeaderPlayer.PlayerData.Source, {
            Id = math.random(11111111, 99999999),
            Title = "Job Center",
            ...
            Buttons = {
                {
                    ...
                    Tooltip = "Accept",
                    Event = "Ethnic-phone/client/jobcenter/accept-join-request",
                    EventData = {
                        ["Data"] = Data,
                        ["Source"] = Source,
                    },
                ...
                },
                {
                    ...
                    Tooltip = "Decline",
                    Event = "Ethnic-phone/client/jobcenter/decline-join-request",
                    EventData = {
                        ["Data"] = Data,
                        ["Source"] = Source,
                    },
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/check-paycheck-amount', function(Source, Cb)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer('Ethnic-phone/server/jobcenter/job-done', function(Source, Job)
        local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        if Group['Busy'] then
            ...
            for _, MemberData in pairs(Group['Members']) do
                ...
                if Money ~= nil then
                    ...
                    TriggerClientEvent('Ethnic-phone/client/notification', MemberData['Source'], {
                        Id = math.random(11111111, 99999999),
                        Title = "Job Center",
...
RegisterNetEvent("Ethnic-phone/server/jobcenter/check-in", function(Job)
    local src = source
    TriggerClientEvent('Ethnic-phone/client/jobcenter/check-in', src, Job)
end)

RegisterNetEvent("Ethnic-phone/server/jobcenter/check-out", function()
    local src = source
    TriggerClientEvent('Ethnic-phone/client/jobcenter/check-out', src)
end)

RegisterNetEvent("Ethnic-phone/server/jobcenter/request-paycheck", function()
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if not Player then return end
    local Amount = Player.PlayerData.MetaData['JobsPaycheck']
    if Amount ~= nil then
        TriggerClientEvent('Ethnic-phone/client/notification', src, {
            Id = math.random(11111111, 99999999),
            Title = "Paycheck",
...
RegisterNetEvent("Ethnic-phone/server/jobcenter/leave-group", function(Job)
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    ...
    if Player.PlayerData.CitizenId == Group['Leader'] then -- Leader leaves group
        ...
        for MemberId, MemberData in pairs(Group["Members"]) do -- Disband group for every member
            TriggerClientEvent('Ethnic-phone/client/jobcenter/reset-group', MemberData['Source'])
            TriggerClientEvent('Ethnic-phone/client/jobcenter/refresh-all-groups', MemberData["Source"], Job)
            TriggerClientEvent('Ethnic-phone/client/notification', MemberData['Source'], {
                Id = math.random(11111111, 99999999),
                Title = "Job Center",
        ...
        if MemberData["Source"] == src then
            ...
            for _, MemberData2 in pairs(Group["Members"]) do
                if MemberData["Source"] ~= MemberData2['Source'] then
                    TriggerClientEvent('Ethnic-phone/client/notification', MemberData2['Source'], {
                        Id = math.random(11111111, 99999999),
                        Title = "Job Center",
                    ...
                    TriggerClientEvent('Ethnic-phone/client/jobcenter/refresh-group', MemberData2["Source"], Group)
                end
            end
        end
...
RegisterNetEvent("Ethnic-phone/server/jobcenter/cancel-join-request", function(Job, Data)
...
RegisterNetEvent("Ethnic-phone/server/jobcenter/answer-request", function(Job, Data, Accept)
    local Player = PlayerModule.GetPlayerBySource(Data.Source)
    if Data['Target'] ~= Player.PlayerData.CitizenId then return end -- Check if player is leader

    if Accept then
        AcceptedJoinRequest = true
        TriggerClientEvent('Ethnic-phone/client/notification', Data.Source, {
            Id = math.random(11111111, 99999999),
            Title = "Job Center",
...
RegisterNetEvent("Ethnic-phone/server/jobcenter/success-task", function(Job, TaskId, Finished)
    -- Get player group
    local src = source
    local Group, Key = GetPlayerGroup(src, Job)
    if not Group then return end -- User is not in any group
    ...
    for _, MemberData in pairs(Group['Members']) do
        ...
        TriggerClientEvent('Ethnic-phone/client/jobcenter/success-task', MemberData['Source'], TaskId, Finished)
    end
end)

RegisterNetEvent("Ethnic-phone/server/jobcenter/set-ready", function(Job, Bool)
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if not Player then return end
    ...
    for k, MemberData in pairs(Group['Members']) do
        ...
        TriggerClientEvent('Ethnic-phone/client/jobcenter/check-for-jobs', MemberData['Source'], Group["Ready"], IsLeader)
    end    

end)

RegisterNetEvent("Ethnic-phone/server/jobcenter/update-task", function(Job, TaskId, ExtraDone)
    local src = source
    local Group, Key = GetPlayerGroup(src, Job)
    if not Group then return end -- User is not in any group

    for _, MemberData in pairs(Group['Members']) do
        TriggerClientEvent('Ethnic-phone/client/jobcenter/update-task', MemberData['Source'], TaskId, ExtraDone)
    end
end)

RegisterNetEvent("Ethnic-phone/server/jobcenter/join-group", function(Job, Leader)
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if not Player then return end
    ...
    for GroupId, GroupData in pairs(ServerConfig.Groups[Job]) do
        if GroupData['Leader'] == Leader then -- Get correct group
            ...
            for _, MemberData in pairs(ServerConfig.Groups[Job][GroupId]["Members"]) do
                TriggerClientEvent('Ethnic-phone/client/jobcenter/refresh-group', MemberData['Source'], ServerConfig.Groups[Job][GroupId])
            end
        end
    end
end)

RegisterNetEvent("Ethnic-phone/server/jobcenter/cancel-task", function(Job)
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if not Player then return end
    ...
    if Group['Leader'] == Player.PlayerData.CitizenId then
        ...
        for _, MemberData in pairs(Group['Members']) do
            TriggerClientEvent('Ethnic-phone/client/jobcenter/quit-task', MemberData['Source'])
            TriggerClientEvent('Ethnic-phone/client/jobcenter/refresh-group', MemberData['Source'], Group)
        end
    end
end)
...
AddEventHandler('playerDropped', function (reason)
    ...
    if Group['Leader'] == Player.PlayerData.CitizenId then
        ...
        for _, MemberData in pairs(Group['Members']) do
            TriggerClientEvent('Ethnic-phone/client/jobcenter/reset-group', MemberData['Source'])
            TriggerClientEvent('Ethnic-phone/client/jobcenter/refresh-all-groups', MemberData["Source"], Job)
            TriggerClientEvent('Ethnic-phone/client/jobcenter/on-crash', MemberData['Source'], Job)
            TriggerClientEvent('Ethnic-phone/client/notification', MemberData['Source'], {
                Id = math.random(11111111, 99999999),
                Title = "Job Center",
                Message = "The group has been disbanded",
        ...
        -- TriggerClientEvent('Ethnic-phone/client/jobcenter/update', -1)
        return
    end
    ...
    for _, MemberData in pairs(Group['Members']) do
        if MemberData['Source'] == Player.PlayerData.Source then
            ...
            for _, NewMemberData in pairs(Group['Members']) do
                TriggerClientEvent('Ethnic-phone/client/notification', NewMemberData['Source'], {
                    Id = math.random(11111111, 99999999),
                    Title = "Job Center",
                ...
                TriggerClientEvent('Ethnic-phone/client/jobcenter/refresh-group', NewMemberData['Source'], Group)
            end
        end
    end
end)
