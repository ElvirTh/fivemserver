...
AddEventHandler('Modules/client/ready', function()
    TriggerEvent('Modules/client/request-dependencies', {
        ...
        CallbackModule.CreateCallback('Ethnic-vehicles/client/get-entity-player-is-looking-at', function(Cb)
            local Entity = exports['Ethnic-base']:FetchModule('Functions').GetEntityPlayerIsLookingAt(3.0, 0.2, 286, PlayerPedId())
...
RegisterNetEvent('Ethnic-vehicle/client/pickup-wheelchair', function()
    ...
    if GetEntityModel(Vehicle['Vehicle']) == GetHashKey('wheelchair') then
        ...
        if Distance <= 1.6 then
            ...
            EventsModule.TriggerServer('Ethnic-inventory/server/add-item', "wheelchair", 1, false, nil, true, false)
            exports['Ethnic-ui']:Notify('wheelchair-pickup', 'You picked up the wheelchair.', 'success', 3500)
        else
            exports['Ethnic-ui']:Notify('wheelchair-pickup', 'You are too far away from the wheelchair..', 'error', 3500)
        end
    else
        exports['Ethnic-ui']:Notify('wheelchair-pickup', 'There is no wheelchair nearby', 'error', 3500)
    end
end)
...
RegisterNetEvent("Ethnic-threads/entered-vehicle", function()
    ...
    DisplayRadar(true)
    -- TriggerEvent('Ethnic-assets/client/set-map-zoom')
end)

RegisterNetEvent('Ethnic-vehicles/client/try-flip-vehicle', function(Nothing, Entity)
    exports['Ethnic-ui']:ProgressBar('Flipping Vehicle..', 5000, {['AnimName'] = 'pushcar_offcliff_f', ['AnimDict'] = 'missfinale_c2ig_11', ['AnimFlag'] = 49}, false, true, true, function(DidComplete)
        if DidComplete then
...
RegisterNetEvent('Ethnic-vehicles/client/toggle-door', function(State, DoorId)
    if State == 'open' then
...
RegisterNetEvent('Ethnic-vehicles/client/carry-bicycle', function(Nothing, Entity)
	if not CurrentCarryData.Carrying then
		local PlayerBone = GetPedBoneIndex(PlayerPedId(), 0xE5F3)
		NetworkRequestControlOfEntity(Entity)
		exports['Ethnic-ui']:SetInteraction('[E] Drop', 'primary')
		...
		exports['Ethnic-ui']:Notify('in-hands','You have something in your hands..', 'error', 5500)
	end
end)
...
function InitMain()
    KeybindsModule.Add('toggleHorn', 'Vehicle', 'Emergency Horn', 'E', false, 'Ethnic-vehicles/client/sirens-horn')
    KeybindsModule.Add("CurrentLimitData.SpeedLimiter", "Vehicle", "Limiter", '', function(IsPressed)
        ...
        if CurrentLimitData.Enabled then
            ...
            exports['Ethnic-ui']:Notify('limiter-toggled', "Limiter disabled", 'error')
            ...
            exports['Ethnic-ui']:Notify('limiter-toggled', "Limiter set on " .. tostring(math.floor(MPHSpeed)) .. "mp/h")
...
function HasOverdueDebts(Plate)
	local Debts = CallbackModule.SendCallback("Ethnic-phone/server/get-overdue-debts", "Vehicle")
