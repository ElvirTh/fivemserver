...
AddEventHandler("polyzone:pzcreate", function(zoneType, name)
    if createdZone ~= nil then
        exports['Ethnic-ui']:Notify('polyzone-creation', 'A shape is already being created!', 'error')
        return
    end
    ...
    elseif zoneType == "circle" then
        ...
        if radius == nil then
            exports['Ethnic-ui']:Notify('polyzone-creation', 'CircleZone requires a radius (must be a number)!', 'error')
            return
        end
    ...
    elseif zoneType == "box" then
        ...
        if length == nil or length < 0.0 then
            exports['Ethnic-ui']:Notify('polyzone-creation', 'BoxZone requires a length (must be a positive number)!', 'error')
            return
        end
        local width = tonumber(GetUserInput("Enter width:"))
        if width == nil or width < 0.0 then
            exports['Ethnic-ui']:Notify('polyzone-creation', 'BoxZone requires a width (must be a positive number)!', 'error')
            return
        end
...
AddEventHandler("polyzone:pzfinish", function()
    ...
    exports['Ethnic-ui']:Notify('polyzone-creation', 'PolyZone finished!', 'success')
...
AddEventHandler("polyzone:pzlast", function()
    ...
    if lastCreatedZoneType == 'poly' then
        exports['Ethnic-ui']:Notify('polyzone-creation', 'This only supports BoxZone and CircleZone for now..', 'error')
    end
...
AddEventHandler("polyzone:pzcancel", function()
    ...
    exports['Ethnic-ui']:Notify('polyzone-creation', 'PolyZone canceled!', 'error')
