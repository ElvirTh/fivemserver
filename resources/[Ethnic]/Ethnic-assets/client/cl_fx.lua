...
RegisterNetEvent('Ethnic-assets/client/toggle-effect', function(TargetId, Effect, TimeOut, Bool)
    if Bool then
...
function CreatePedFx(TargetId, Effect, TimeOut)
    ...
    if MyServerId == TargetId and Effect == 'Puke' then 
        ...
        if EffectData['Anim'] ~= nil then
            TriggerEvent('Ethnic-animations/client/play-animation', EffectData['Anim'])
        end
    end
    ...
    Citizen.SetTimeout(TimeOut, function()
        ...
        TriggerServerEvent('Ethnic-assets/server/toggle-effect', TargetId, Effect, false)
    end)
end
