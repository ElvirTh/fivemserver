...
RegisterNetEvent('Ethnic-clothing/client/create-new-char', function()
    PlayerModule.GetPlayerData(function(PlayerData)
        ...
        EventsModule.TriggerServer('Ethnic-base/server/bucketmanager/set-routing-bucket')
        TriggerEvent('Ethnic-weathersync/client/set-default-weather', 15)
...
RegisterNetEvent("Ethnic-clothing/client/load-skin", function(Model, Skin, Tattoos)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('Ethnic-clothing/client/load-clothing', SkinData, PlayerPedId())
    end)
end)

RegisterNetEvent('Ethnic-clothing/client/load-clothing', function(Data, PlayerPed)
...
RegisterNUICallback('Close', function(_, Cb)
    if CreatingNew then 
        ...
        Citizen.SetTimeout(500, function()
            ...
            TriggerEvent('Ethnic-base/client/on-login')
            Citizen.Wait(5000)
            TriggerEvent('Ethnic-apartment/client/spawn-apartment', true)
        end)
        ToggleCam(false)
    else
        ToggleCam(false)
...
RegisterNUICallback('RemoveOutfit', function(Data, Cb)
    ...
    Citizen.SetTimeout(500, function()
        ...
        exports['Ethnic-ui']:Notify("removed-outfit", Lang:t('info.removed_outfit', { outfit = Data.OutfitName}), 'success')
    end)
    Cb('Ok')
end)
...
RegisterNUICallback("RenameOutfit", function(Data, Cb)
    ...
    Citizen.SetTimeout(500, function()
        ...
        exports['Ethnic-ui']:Notify("renamed-outfit", Lang:t('info.renamed_outfit', { fromOutfit = OutfitData.OutfitName, toOutfit = Data.Name}), 'success')
    end)
    Cb('Ok')
end)

RegisterNetEvent('Ethnic-clothing/client/take-off-face-wear', function(Type, Extra)
    if Type == 'Hat' then
        ...
        if HasHat ~= -1 then
            ...
            TriggerServerEvent('Ethnic-clothing/server/receive-clothing', 'Hat', HatProp, HatTexture)
        end
    elseif Type == 'Mask' then
        ...
        if HasMask ~= -1 then
            ...
            TriggerServerEvent('Ethnic-clothing/server/receive-clothing', 'Mask', MaskProp, MaskTexture)
        end
    elseif Type == 'Glasses' then
        ...
        if HasGlasses ~= -1 then
            ...
            TriggerServerEvent('Ethnic-clothing/server/receive-clothing', 'Glasses', GlassesProp, GlassesTexture)
        end
...
RegisterNetEvent('Ethnic-clothing/client/take-on-face-wear', function(Type, Info, Slot, Ignore)
    if Type == 'Hat' then
        if Ignore ~= nil and not Ignore then
            ...
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'clothing-hat', 1, Slot, true)
        else
    ...
    elseif Type == 'Mask' then
        if Ignore ~= nil and not Ignore then
            ...
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'clothing-mask', 1, Slot, true)
        else
    ...
    elseif Type == 'Glasses' then
        if Ignore ~= nil and not Ignore then
            ...
            local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'clothing-glasses', 1, Slot, true)
        else
