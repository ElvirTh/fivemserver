...
RegisterNetEvent('Ethnic-vehicles/client/get-in-trunk', function(Data, Entity)
    if Data.Forced then 
    ...
    if LockStatus ~= 0 and LockStatus ~= 1 and LockStatus ~= 4 then return exports['Ethnic-ui']:Notify("enter-trunk", "Vehicle is locked..", "error") end
    if (not GetIsDoorValid(Entity, 5)) or Config.DisabledTrunk[GetEntityModel(Entity)] then return exports['Ethnic-ui']:Notify("enter-trunk", "The vehicles does not have a trunk!", "error") end
    if GetVehicleDoorAngleRatio(Entity, 5) == 0 then return exports['Ethnic-ui']:Notify("enter-trunk", "The trunk is closed!", "error") end

    local IsEmpty = CallbackModule.SendCallback("Ethnic-vehicles/server/is-trunk-empty", GetVehicleNumberPlateText(Entity))
    if not IsEmpty then return exports['Ethnic-ui']:Notify("enter-trunk", "There is already someone in the trunk..", "error") end

    StartTrunkLoop(Entity)
    TriggerServerEvent('Ethnic-vehicles/server/set-trunk-occupied', GetVehicleNumberPlateText(Entity), true)
end)
...
function StartTrunkLoop(Vehicle)
    ...
    if not PlayerModule.GetPlayerData().MetaData['Handcuffed'] then
        exports['Ethnic-ui']:SetInteraction("[G] Open/Close, [F] Climb Out")
    else
        exports['Ethnic-ui']:SetInteraction("[F] Climb Out")
    end
    ...
    CreateThread(function()
        ...
        while InTrunk do
            ...
            if IsDisabledControlJustReleased(0, 23) then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0.0 then
                    ...
                    exports['Ethnic-ui']:Notify("enter-trunk", "The trunk is closed!", "error")
                end
            end
        ...
        if DoesEntityExist(Vehicle) then
            ...
            TriggerServerEvent('Ethnic-vehicles/server/set-trunk-occupied', GetVehicleNumberPlateText(Vehicle), false)
	    end
    end)
end
