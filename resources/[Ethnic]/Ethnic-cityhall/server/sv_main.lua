...
Citizen.CreateThread(function() 
    ...
    CommandsModule.Add("setlawyer", "Make person lawyer..", {}, false, function(Source, args)
        ...
        if Player.PlayerData.Job.Name == 'judge' and Player.PlayerData.Job.Duty then
            TriggerClientEvent('Ethnic-cityhall/client/lawyer/add-closest', Source)
        else
    ...
    CommandsModule.Add("setprosecutor", "Make person prosecutor..", {}, false, function(Source, args)
        local Player = PlayerModule.GetPlayerBySource(Source)
        if Player.PlayerData.Job.Name == 'judge' and Player.PlayerData.Job.Duty then
            TriggerClientEvent('Ethnic-cityhall/client/prosecutor/add-closest', Source)
        else
            Player.Functions.Notify('not-judge', 'You can\'t do this..', 'error')
        end
    end)
    ...
    EventsModule.RegisterServer('Ethnic-cityhall/server/lawyer/add', function(Source, Target)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer('Ethnic-cityhall/server/prosecutor/add', function(Source, Target)
        local Player = PlayerModule.GetPlayerBySource(Source)
        local TPlayer = PlayerModule.GetPlayerBySource(tonumber(Target))
        if not TPlayer then return end
    ...
    EventsModule.RegisterServer('Ethnic-cityhall/server/do-license-things', function(Source, Type, StateId, LicenseType) -- Give License
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer('Ethnic-cityhall/server/create-account', function(Source, AccountName, StateId, AccountType) 
        ...
        DatabaseModule.Execute("SELECT * FROM player_accounts WHERE name = ?", {
            ...
            if AccountResult[1] == nil then
                ...
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "gave-account", "Successfully created a bank account. ("..AccountName..") - ("..AccountType..")", 'success')
            else
                TriggerClientEvent('Ethnic-ui/client/notify', Source, "already-exists-account", "An account with this name already exists..", 'error')
            end
        end)
    end)

    EventsModule.RegisterServer('Ethnic-cityhall/server/do-account-monitor', function(Source, AccountId, Bool) 
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer('Ethnic-cityhall/server/do-account-active', function(Source, AccountId, Bool) 
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    EventsModule.RegisterServer('Ethnic-cityhall/server/do-house-active', function(Source, HouseName, Bool) 
        local Player = PlayerModule.GetPlayerBySource(Source)
...
RegisterNetEvent("Ethnic-cityhall/server/purchase-id", function()
    local src = source
