...
LoadProfiles = async function(Id) {
    ...
    $.post('https://Ethnic-mdw/MDW/Profiles/GetProfiles', JSON.stringify({}), function(Profiles) {
...
ClickedOnProfile = async function(ProfileData) {
    DoMdwLoader(250, function() {
        ...
        $.post('https://Ethnic-mdw/MDW/Profiles/RequestData', JSON.stringify({Id: MdwData.ProfileData.CurrentEditingData.id, CitizenId: ProfileData.citizenid}), function(ProfileData) {
...
$(document).on('click', '.mdw-profiles > .mdw-block-header > .mdw-block-header-icons > i', function(Event) {
    ...
    if (ClickType == 'Save') {
        if (MdwData.ProfileData.EditingProfile) {
            $.post('https://Ethnic-mdw/MDW/Profiles/UpdateProfile', JSON.stringify({
            ...
            $.post('https://Ethnic-mdw/MDW/Profiles/CreateProfile', JSON.stringify({
                CitizenId: GetProfileInputValue('StateId'),
                Name: GetProfileInputValue('Name'),
                Image: GetProfileInputValue('Image'),
                Notes: GetProfileInputValue('Notes'),
        ...
        if (MdwData.ProfileData.EditingProfile) {
            $.post('https://Ethnic-mdw/MDW/Profiles/DeleteProfile', JSON.stringify({
                Id: MdwData.ProfileData.CurrentEditingData.id
            }), function() {
                DoMdwLoader(1300, function() {
                    ClearEditProfile();
...
$(document).on('click', '.mdw-information-data-tag > i', function(Event) {
    ...
    if ( $(this).data('TagData') != undefined ) {
        $.post('https://Ethnic-mdw/MDW/Profiles/RemoveTag', JSON.stringify({
            Id: MdwData.ProfileData.CurrentEditingData.id,
        ...
        $.post('https://Ethnic-mdw/MDW/Profiles/RemoveLicense', JSON.stringify({
            Id: MdwData.ProfileData.CurrentEditingData.id,
...
$(document).on('click', '.block-three > .mdw-profiles > #tags > .mdw-information-add > i', function(Event) {
    ...
    DoMdwPicker('TagSearch', (Result) => {
        $.post('https://Ethnic-mdw/MDW/Profiles/AddTag', JSON.stringify({
            Id: MdwData.ProfileData.CurrentEditingData.id,
            Tags: Result,
        }), function() {
            DoMdwLoader(100, function() {
