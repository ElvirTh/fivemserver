...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-heists/server/jewellery/get-state', function(Source, Cb)
        Cb(Config.JewelleryState)
    end)

    CallbackModule.CreateCallback('Ethnic-heists/server/jewellery/can-rob-vitrine', function(Source, Cb, VitrineId)
        Cb(not Config.JewelleryVitrines[VitrineId])
    end)

    EventsModule.RegisterServer("Ethnic-heists/server/jewellery/set-vitrine-state", function(Source, VitrineId, Bool)    
        Config.JewelleryVitrines[VitrineId] = Bool
    end)

    EventsModule.RegisterServer("Ethnic-heists/server/jewellery/set-state", function(Source, Bool)
        Config.JewelleryState = Bool
        TriggerClientEvent('Ethnic-heists/client/jewellery/sync-state', -1, Config.JewelleryState)
        if Bool then
            Citizen.SetTimeout((1000 * 60) * Config.ResetTimes['Jewellery'], function() -- 30 Mins
                Config.JewelleryState = false
                TriggerClientEvent('Ethnic-heists/client/jewellery/sync-state', -1, Config.JewelleryState)
                TriggerEvent('Ethnic-doors/server/set-locks', Config.JewelleryDoors[1], 1)
                TriggerEvent('Ethnic-doors/server/set-locks', Config.JewelleryDoors[2], 1)
    ...
    EventsModule.RegisterServer("Ethnic-heists/server/jewellery/give-reward", function(Source)
