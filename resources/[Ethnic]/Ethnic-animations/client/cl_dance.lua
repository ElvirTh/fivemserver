        ...
        if LocalPlayer.state.LoggedIn then
            if Dancing then
                if IsControlJustReleased(0, 73) then
                    TriggerEvent('Ethnic-dances/client/clear-dance')
                end
            else
                Citizen.Wait(450)
            end
...
RegisterNetEvent('Ethnic-dances/client/clear-dance', function()
    ClearPedTasks(PlayerPedId())
    Dancing = false
end)

RegisterNetEvent('Ethnic-dances/client/dance', function(DanceNumber)
