...
RegisterNetEvent('Ethnic-base/client/set-playerdata', function(PlayerData)
...
PlayerModule = {
    ...
    GetPlayerCitizenIdBySource = function(Source)
        ...
        local CitizenId = CallbackModule.SendCallback('Ethnic-base/server/get-citizenid-by-source', Source)
        return CitizenId
    end,
    GetPlayers = function()
        local Callback = exports[GetCurrentResourceName()]:FetchModule('Callback')
        local Players = CallbackModule.SendCallback('Ethnic-base/server/get-active-players')
        return Players
    end,
    IsPlayerAdmin = function() 
        local Callback = exports[GetCurrentResourceName()]:FetchModule('Callback')
        local IsAdmin = CallbackModule.SendCallback('Ethnic-base/server/is-me-admin')
        return IsAdmin
    end,
    GetPlayersInArea = function(Coords, Radius)
        local Callback = exports[GetCurrentResourceName()]:FetchModule('Callback')
        local Players = CallbackModule.SendCallback('Ethnic-base/server/get-active-players-in-radius', Coords, Radius)
        return Players
    end,
    GetClosestPlayer = function(Coords, Radius)
        ...
        local ClosestPlayer = CallbackModule.SendCallback('Ethnic-base/server/get-closest-player', Coords, Radius)
