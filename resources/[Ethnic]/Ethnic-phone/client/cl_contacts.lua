...
function Contacts.InitKeybinds()
    KeybindsModule.Add('phoneAnswer', 'Phone', 'Answer Call', '', function(IsPressed)
        ...
        TriggerEvent('Ethnic-phone/client/calling/answer-call', { Declined = false, Id = Contacts.CallId })
    end)
    ...
    KeybindsModule.Add('phoneDecline', 'Phone', 'Decline Call', '', function(IsPressed)
        ...
        local PlayerData = PlayerModule.GetPlayerData()
        if PlayerData.MetaData['Dead'] or PlayerData.MetaData['Handcuffed'] then return end
        TriggerEvent('Ethnic-phone/client/calling/answer-call', { Declined = true, Id = Contacts.CallId })
    end)
end

function Contacts.Render()
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/contacts/get-contacts")
    exports['Ethnic-ui']:SendUIMessage("Phone", "RenderContactsApp", {
        Contacts = Result or {}
    })
end

RegisterNUICallback("Contacts/AddContact", function(Data, Cb)
    EventsModule.TriggerServer("Ethnic-phone/server/contacts/add-contact", Data)
    Cb('Ok')
end)

RegisterNUICallback("Contacts/RemoveContact", function(Data, Cb)
    EventsModule.TriggerServer("Ethnic-phone/server/contacts/remove-contact", Data)
    Cb('Ok')
end)
...
RegisterNetEvent('Ethnic-phone/client/contacts/refresh', function()
    local Result = CallbackModule.SendCallback("Ethnic-phone/server/contacts/get-contacts")
    exports['Ethnic-ui']:SendUIMessage("Phone", "RenderContactsApp", {
        Contacts = Result or {}
    })
...
RegisterNUICallback('Contacts/CallContact', function(Data, Cb)
    EventsModule.TriggerServer("Ethnic-phone/server/contacts/call-contact", Data)
    Cb('Ok')
end)

RegisterNetEvent('Ethnic-phone/client/calling/answer-call', function(Data)
    if Contacts.CallId ~= nil and Contacts.CallId ~= Data.Id then return end
    EventsModule.TriggerServer('Ethnic-phone/server/calling/answer-call', Data)
end)

local InCall = false
RegisterNetEvent('Ethnic-phone/client/contacts/set-call-thread', function(Enabled, NotificationId)
    InCall = Enabled
    ...
    if Enabled then
        ...
        TriggerEvent('Ethnic-phone/client/set-notification-buttons', NotificationId, {})
...
RegisterNetEvent('Ethnic-phone/client/contacts/connect-voice', function(StartCall, PhoneNumber, Receiver)
    if StartCall then
        TriggerServerEvent('Ethnic-voice/server/phone/start-call', PhoneNumber, Receiver)
    else
        InCall = false
        TriggerServerEvent('Ethnic-voice/server/phone/end-call', PhoneNumber)
    end
end)

RegisterNetEvent('Ethnic-phone/client/set-call-id', function(CallId)
    Contacts.CallId = CallId
end)

RegisterNetEvent('Ethnic-phone/client/call/do-anim', function()
...
RegisterNetEvent('Ethnic-phone/client/call/force-disconnect', function(Payphone)
    if Contacts.CallId ~= nil then
        if Payphone ~= nil and Payphone then
            TriggerEvent('Ethnic-phone/client/calling/answer-call', { Declined = true, CallName = "Payphone", IsPayphone = true, Id = Contacts.CallId })
        else
            TriggerEvent('Ethnic-phone/client/calling/answer-call', { Declined = true, Id = Contacts.CallId })
        end
    end
