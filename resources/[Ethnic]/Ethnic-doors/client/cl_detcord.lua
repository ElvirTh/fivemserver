    ...
    Citizen.SetTimeout(450, function()
        ...
        if DoorId ~= nil then
            if Config.Doors[DoorId].CanLockpick then
                ...
                if Outcome then
                    TriggerServerEvent('Ethnic-doors/server/set-locks', DoorId, 0)
                    TriggerServerEvent('Ethnic-doors/server/toggle-locks', DoorId, 0)
                else
                    exports['Ethnic-assets']:RemoveLockpickChance(IsAdvanced)
                    TriggerEvent('Ethnic-ui/client/notify', 'lockpick-error', "Failed attempt..", 'error')
                end
            else
                TriggerEvent('Ethnic-ui/client/notify', "lockpick-error", "This door can\'t be lockpicked..", 'error')
            end
        else
            return
        end
...
RegisterNetEvent('Ethnic-doors/client/used-detcord', function()
    Citizen.SetTimeout(450, function()
        local PlayerData = PlayerModule.GetPlayerData()
        if PlayerData.Job.Name ~= 'police' then TriggerEvent('Ethnic-ui/client/notify', "detcord-error", "You can\'t use this..", 'error') return end

        -- Housing
        if exports['Ethnic-housing']:IsNearHouse() then 
            ...
            if DetcordDone then
                TriggerEvent('Ethnic-housing/client/lock-house-id', HouseData.Name)
            end
        else
            -- Doorlocks
            local Entity, EntityType, EntityCoords = FunctionsModule.GetEntityPlayerIsLookingAt(4.0, 0.2, 286, PlayerPedId())
            ...
            if not CanDetcordDoor(DoorId) then 
                TriggerEvent('Ethnic-ui/client/notify', "detcord-error", "This door can\'t be detcorded..", 'error')
                return 
            end
            ...
            if DetcordDone then
                ...
                if Config.Doors[DoorId].Connected ~= nil and next(Config.Doors[DoorId].Connected) then
                    for k, v in pairs(Config.Doors[DoorId].Connected) do
                        TriggerServerEvent('Ethnic-doors/server/toggle-locks', v, 0)
                    end
                end
            end
        end
...
function DoDetcord(Coords)
    ...
    local DidRemove = CallbackModule.SendCallback('Ethnic-base/server/remove-item', 'detcord', 1, false, true)
    
    Citizen.Wait(1500)

    TriggerServerEvent('Ethnic-heists/server/thermite-syncFx', Coords, true)
