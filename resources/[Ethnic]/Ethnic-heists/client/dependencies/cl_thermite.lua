...
function DoThermite(Coords, IsColor)
    ...
    local OnSuccess = function(Success)
        if Success then
            TriggerServerEvent('Ethnic-heists/server/thermite-syncFx', Coords, false)
    ...
    if IsColor then
        exports['Ethnic-ui']:ColorMinigame(OnSuccess)
    else
        exports['Ethnic-ui']:MemoryMinigame(OnSuccess)
    end
...
RegisterNetEvent('Ethnic-heists/client/thermite/sync-fx', function(Coords, Detcord)
