...
function InitWheel(Bool)
    if Bool then
        if WheelDui then -- If wheel exists, reset
            exports['Ethnic-assets']:ChangeDuiURL(WheelDui['DuiId'], Config.WheelURL)
            AddReplaceTexture('vw_prop_vw_luckywheel_01a', 'script_rt_casinowheel', WheelDui['TxdDictName'], WheelDui['TxdName'])
        else
            WheelDui = exports['Ethnic-assets']:GenerateNewDui(Config.WheelURL, 1024, 1024, 'Casino-Wheel')
...
function IsWheelActive()
    if IsWheelSpun then
        exports['Ethnic-ui']:Notify("wheel-active", "The wheel is already spinning!", "error")
        return true
    end
    return false
end