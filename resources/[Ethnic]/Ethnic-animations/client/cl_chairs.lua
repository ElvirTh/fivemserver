    ...
    while true do
        ...
        if LocalPlayer.state.LoggedIn and SittingInChair then
            if IsControlJustReleased(0, 73) then
                TriggerEvent('Ethnic-animations/client/clear-chair')
            end
        else
            Citizen.Wait(450)
        end
...
RegisterNetEvent('Ethnic-animations/client/sit-chair', function(Data, Entity)
...
RegisterNetEvent('Ethnic-animations/client/clear-chair', function()
...
RegisterNetEvent('Ethnic-animations/client/reset-chair', function()
    PrevCoords, GoingUp, SittingInChair = vector3(0, 0, 0), false, false
end)
    ...
    Citizen.CreateThread(function()
        for k, v in pairs(Config.Chairs) do
            if v.Type == 'Entity' then
                exports['Ethnic-ui']:AddEyeEntry(GetHashKey(v.Model), {
                    Type = 'Model',
                    Model = v.Model,
                    SpriteDistance = 1.7,
                    Options = {
                        {
                            ...
                            Label = 'Sit',
                            EventType = 'Client',
                            EventName = 'Ethnic-animations/client/sit-chair',
            ...
            elseif v.Type == 'Poly' then
                exports['Ethnic-ui']:AddEyeEntry("chair_"..k, {
                    Type = 'Zone',
                    SpriteDistance = 2.0,
                    Distance = 1.7,
                    State = false,
