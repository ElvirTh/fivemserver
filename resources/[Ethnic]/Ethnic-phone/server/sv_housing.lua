...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/housing/get-owned-houses', function(Source, Cb)
            ...
            if Houses[1] ~= nil then
                for k, v in pairs(Houses) do
                    HouseData[v.house] = {
                        Owner = v.citizenid,
                        Name = v.house,
                        Locked = exports['Ethnic-housing']:IsHouseLocked(v.house),
                        Category = v.category,
                        Adres = v.label,
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/housing/purchase-house', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        if Player.Functions.RemoveMoney('Bank', Data.House.Price, "Purchased house") then
            ...
            DatabaseModule.Update('UPDATE player_houses SET citizenid = ?, owned = ?, keyholders = ? WHERE house = ?', {
                ...
                exports['Ethnic-housing']:RefreshHousing()
                Cb(true)
            end)
        else
            Cb(false)
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/housing/toggle-locks', function(Source, Cb, Data)
        local DoorState = exports['Ethnic-housing']:ToggleDoorLocks(Data.HouseId)
        Cb(DoorState)
    end)

    CallbackModule.CreateCallback('Ethnic-phone/server/housing/sell-house', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/housing/get-keyholders', function(Source, Cb, Data)
        DatabaseModule.Execute('SELECT * FROM player_houses WHERE house = ?', {
            Data['HouseId'],
        }, function(House)
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/housing/add-key', function(Source, Cb, Data)
        DatabaseModule.Execute('SELECT * FROM player_houses WHERE house = ?', {
            Data['HouseId'],
        }, function(Houses)
            if Houses[1] ~= nil then
                ...
                DatabaseModule.Update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {
                    ...
                    TriggerClientEvent('Ethnic-phone/client/notification', TPlayer.PlayerData.Source, {
                        Id = math.random(100000, 999999),
                        Title = 'Housing',
    ...
    CallbackModule.CreateCallback('Ethnic-phone/server/housing/remove-key', function(Source, Cb, Data)
        DatabaseModule.Execute('SELECT * FROM player_houses WHERE house = ?', {
            Data['HouseId'],
        }, function(Houses)
            if Houses[1] ~= nil then
