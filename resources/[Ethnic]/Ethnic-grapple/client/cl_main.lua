...
RegisterNetEvent('Ethnic-grapple/client/used-grapple', function(ForceRemove)
            ...
            if FreeAiming then
                ...
                if Hit == 1 or Coords ~= vector3(0, 0, 0) then
                    if not ShowingInteraction and HoldingGrapple then
                        ...
                        exports['Ethnic-ui']:SetInteraction('[E] Grapple!')
                    end
                    if IsControlJustPressed(0, 38) then
                        if not DoingGrapple then
                            EventsModule.TriggerServer('Ethnic-inventory/server/degen-item', exports['Ethnic-inventory']:GetSlotForItem('weapon_grapple'), 25.0)
                            DoingGrapple = true
...
RegisterNetEvent('Ethnic-grapple/client/do-rope', function(SourceServer, Type, RopeId, SourceId, EndCoords)
    if Type == 'Create' then
...
function DoGrappleToCoords(Coords)
    ...
    TriggerServerEvent('Ethnic-grapple/server/do-rope', RopeId, Coords, 'Create')
    ...
    Citizen.CreateThread(function()
        ...
        TriggerServerEvent('Ethnic-grapple/server/do-rope', RopeId, vector3(0, 0, 0), 'Delete')
        if not exports['Ethnic-inventory']:HasEnoughOfItem('weapon_grapple', 1) then
            TriggerEvent('Ethnic-grapple/client/used-grapple', true)
        end
    end)
end
