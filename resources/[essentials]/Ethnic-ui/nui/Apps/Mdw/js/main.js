...
CloseMobileData = function() {
    $('.tooltip-css').remove();
    $.post('https://Ethnic-mdw/MDW/Close', JSON.stringify({}));
...
DoMdwStartup = async function(CitizenId, IsPublic) {
    ...
    if (!MdwData.UserData.IsPublic) {
        $.post('https://Ethnic-mdw/MDW/Main/GetUser', JSON.stringify({
            CitizenId: CitizenId,
        }), function(Data) {
...
DoMdwPicker = function(Type, OnClick, ExtraData) {
    if (Type == 'Profiles') {
        ...
        $.post("https://Ethnic-mdw/MDW/Profiles/GetProfiles", JSON.stringify({}), function(Result){
