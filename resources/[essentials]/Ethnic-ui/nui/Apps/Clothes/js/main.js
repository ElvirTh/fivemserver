...
OpenSkinMenu = function(Data) {
    ...
    $('.main-clothes-container').animate({'right': '0vw' }, 250, function() {
        ...
        $.post('https://Ethnic-clothes/Clothes/SwitchCategory', JSON.stringify({}));
    });
}

CloseSkinMenu = function(Canceled) {
    $.post(`https://Ethnic-clothes/Clothes/CloseSkinMenu`, JSON.stringify({Canceled: Canceled}));
...
$(document).on('input', '.option-block-number', async function(e) {
    ...
    if (ClothesChangeType == 'Item') {
        if (MaxValues[ClothesCategory] != null && Value > -2 && Value <= MaxValues[ClothesCategory]['MaxItem']) {
            ...
            $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({What: ClothesCategory, Who: "Item", CurrentValue: Value}));
            $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({What: ClothesCategory, Who: "Texture", CurrentValue: 0}));
        } else {
            $(this).val(0);
        };
    } else {
        if (MaxValues[ClothesCategory] != null && Value > -1 && Value <= MaxValues[ClothesCategory]['MaxTexture']) {
            $(this).val(Value);
            $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({What: ClothesCategory, Who: "Texture", CurrentValue: Value}));
        } else {
            $(this).val(0);
        }
    };
...
$(document).on('click', '.option-block-buttons-left', async function(e) {
    ...
    if (ClothesChangeType == 'Item') {
        if (MaxValues[ClothesCategory] != null && MinValue > -2 && MinValue <= MaxValues[ClothesCategory]['MaxItem']) {
            ...
            $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({ What: ClothesCategory, Who: "Item", CurrentValue: MinValue}));
            $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({ What: ClothesCategory, Who: "Texture", CurrentValue: 0}));
        } else {
        ...
        if (MaxValues[ClothesCategory] != null && MinValue > -1 && MinValue <= MaxValues[ClothesCategory]['MaxTexture']) {
            $(SearchInputValue).val(MinValue);
            $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({What: ClothesCategory, Who: "Texture", CurrentValue: MinValue}));
        } else {
            $(SearchInputValue).val(0);
        }
    }
...
$(document).on('click', '.option-block-buttons-right', async function(e) {
    ...
    if (ClothesChangeType == 'Item') {
        if (MaxValues[ClothesCategory] != null && PlusValue > -2 && PlusValue <= MaxValues[ClothesCategory]['MaxItem']) {
            ...
            var TextureInputValue = $(this).parent().parent().find(`[data-type=Texture]`);
            TextureInputValue.val(0);
            $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({What: ClothesCategory, Who: "Item", CurrentValue: PlusValue}));
            $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({What: ClothesCategory, Who: "Texture", CurrentValue: 0}));
        } else {
            $(SearchInputValue).val(0);
        }
        ...
        if (MaxValues[ClothesCategory] != null && PlusValue > -1 && PlusValue <= MaxValues[ClothesCategory]['MaxTexture']) {
            $(SearchInputValue).val(PlusValue);
            $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({What: ClothesCategory, Who: "Texture", CurrentValue: PlusValue}));
        } else {
            $(SearchInputValue).val(0);
        }
    }
...
$(document).on('click', '.hair-pallette-item', function(e) {
    ...
    $.post(`https://Ethnic-clothes/Clothes/ChangeSkinValue`, JSON.stringify({What: ColorChangeType, Who: ColorType, CurrentValue: ColorNumber}));
});
...
$(document).on('click', '.menu-option', function(e) {
    ...
    var Type = $(this).data('type');
    if (Type == 'Camera-Face') {
        $.post('https://Ethnic-clothes/Clothes/SetCameraPosition', JSON.stringify({Type: 'Face'}));
    } else if (Type == 'Camera-Chest') {
        $.post('https://Ethnic-clothes/Clothes/SetCameraPosition', JSON.stringify({Type: 'Chest'}));
    } else if (Type == 'Camera-Feet') {
        $.post('https://Ethnic-clothes/Clothes/SetCameraPosition', JSON.stringify({Type: 'Feet'}));
    } else if (Type == 'Toggle-Hat') {
        $.post("https://Ethnic-clothes/Clothes/ToggleClothes", JSON.stringify({ Type: 'Hat' }))
    } else if (Type == 'Toggle-Glasses') {
        $.post("https://Ethnic-clothes/Clothes/ToggleClothes", JSON.stringify({ Type: 'Glasses' }))
    } else if (Type == 'Toggle-Shirt') {
        $.post("https://Ethnic-clothes/Clothes/ToggleClothes", JSON.stringify({ Type: 'Shirt' }))
    }
});
...
$(document).on({
    keydown: function(e) {
        if (e.keyCode == 68 && ClothingMenuOpen) {
            $.post('https://Ethnic-clothes/Clothes/RotatePlayer', JSON.stringify({Type: 'Right'}));
        } else if (e.keyCode == 65 && ClothingMenuOpen) {
            $.post('https://Ethnic-clothes/Clothes/RotatePlayer', JSON.stringify({Type: 'Left'}));
