...
Citizen.CreateThread(function() 
    ...
    CommandsModule.Add({"sv", "car"}, "Spawn vehicle", {}, false, function(source, args)
        ...
        TriggerClientEvent('Ethnic-base/client/spawn-vehicle', source, VehModel)
    end, 'admin')

    CommandsModule.Add({"dv", "delveh"}, "Delete vehicle", {}, false, function(source, args)
        TriggerClientEvent('Ethnic-base/client/delete-vehicle', source)
    end, 'admin')

    CommandsModule.Add({"fix", "fixveh"}, "Fix vehicle", {}, false, function(source, args)
        TriggerClientEvent('Ethnic-base/client/repair-vehicle', source)
    end, 'admin')
...
RegisterNetEvent("Ethnic-admin/server/set-environment", function(Weather, Hour, Minute)
    ...
    local Player = PlayerModule.GetPlayerBySource(src)
    if Weather ~= nil then
        local UpdatedWeather = exports['Ethnic-weathersync']:SetWeather(Weather:upper())
    ...
    if Hour ~= nil and Minute ~= nil then
        local UpdatedTime = exports['Ethnic-weathersync']:SetTime(Hour, Minute)
...
RegisterNetEvent("Ethnic-admin/server/revive-all", function()
    ...
    for k, v in pairs(PlayerModule.GetPlayers()) do
		...
		if Player ~= nil then
            ...
            TriggerClientEvent('Ethnic-hospital/client/revive', v, true)
		end
	end
end)
...
RegisterNetEvent("Ethnic-admin/server/revive-target", function(ServerId)
    ...
    if Player then
        ...
        Player.Functions.SetMetaData("Water", 100)
        Player.Functions.SetMetaData("Stress", 0)
        TriggerClientEvent('Ethnic-hospital/client/revive', PlayerSource, true)
...
RegisterNetEvent("Ethnic-admin/server/open-clothing", function(ServerId)
    ...
    local Player = PlayerModule.GetPlayerBySource(src)
    TriggerClientEvent('Ethnic-admin/client/force-close', PlayerSource)
    SetTimeout(500, function()
        TriggerClientEvent('Ethnic-clothing/client/open-menu', PlayerSource)
    end)
