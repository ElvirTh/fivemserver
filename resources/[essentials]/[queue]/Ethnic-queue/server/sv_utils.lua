...
function Queue:IsWhitelisted(user)
	...
	if identifierDiscord then
		local roles = exports['Ethnic-api']:GetDiscordRoles(user);
		if not (roles == false) then 
			for i = 1, #roles do 
				for roleID, list in pairs(Config.Rankings) do
					if exports['Ethnic-api']:CheckEqual(roles[i], roleID) then 
						return true;
					end
				end
			end
...
function Queue:SetupPriority(user) 
	...
	if license ~= nil then 
		...
		if identifierDiscord then 
			local discordName = exports['Ethnic-api']:GetDiscordName(user);
