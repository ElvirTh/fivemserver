...
AddEventHandler('Modules/client/ready', function()
    TriggerEvent('Modules/client/request-dependencies', {
        ...
        Citizen.SetTimeout(250, function()
            exports['Ethnic-ui']:AddLocation("Apartment", {
            ...
            if Offsets ~= nil then
                if #(GetEntityCoords(PlayerPedId()) - vector3(Config.Apartment['Pos'].x + Offsets['Exit'].x, Config.Apartment['Pos'].y + Offsets['Exit'].y, (Config.Apartment['Pos'].z - 35.0) + Offsets['Exit'].z)) < 1.5 then
                    if not ShowingInteraction then
                        exports['Ethnic-ui']:SetInteraction("[E] Leave Apartment")
                        ShowingInteraction = true
                    end
                    
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent('Ethnic-apartment/client/leave-apartment')
                    end
                elseif #(GetEntityCoords(PlayerPedId()) - vector3(Config.Apartment['Pos'].x + Offsets['Stash'].x, Config.Apartment['Pos'].y + Offsets['Stash'].y, (Config.Apartment['Pos'].z - 35.0) + Offsets['Stash'].z)) < 1.5 then
                    if not ShowingInteraction then
                        exports['Ethnic-ui']:SetInteraction("[E] Storage")
                        ShowingInteraction = true
                    end
                    ...
                    if IsControlJustReleased(0, 38) then
                        if exports['Ethnic-inventory']:CanOpenInventory() then
                            ...
                            if (not GetApartmentLockdownStatus(TargetRoomId)) or ((PlayerData.Job.Name == 'police' or PlayerData.Job.Name == 'judge') and PlayerData.Job.Duty) then
                                TriggerEvent('Ethnic-ui/client/play-sound', 'stash-open', 0.75)
                                EventsModule.TriggerServer('Ethnic-inventory/server/open-other-inventory', 'Apartment'..TargetRoomId, 'Stash', 30, 700.0)
                            else
                                exports['Ethnic-ui']:Notify("apartment", "Apartment on lockdown, you may not open the stash.", "error")
                            end
                        end
                    end
                elseif #(GetEntityCoords(PlayerPedId()) - vector3(Config.Apartment['Pos'].x + Offsets['Wardrobe'].x, Config.Apartment['Pos'].y + Offsets['Wardrobe'].y, (Config.Apartment['Pos'].z - 35.0) + Offsets['Wardrobe'].z)) < 1.5 then
                    if not ShowingInteraction then
                        exports['Ethnic-ui']:SetInteraction("[E] Wardrobe")
                        ShowingInteraction = true
                    end
                    
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent('Ethnic-clothing/client/open-wardrobe', true)
                    end
                elseif #(GetEntityCoords(PlayerPedId()) - vector3(Config.Apartment['Pos'].x + Offsets['Logout'].x, Config.Apartment['Pos'].y + Offsets['Logout'].y, (Config.Apartment['Pos'].z - 35.0) + Offsets['Logout'].z)) < 1.5 then
                    if not ShowingInteraction then
                        exports['Ethnic-ui']:SetInteraction("[E] Sleep")
                        ShowingInteraction = true
                    end
                    
                    if IsControlJustReleased(0, 38) then
                        local Interior = exports['Ethnic-interiors']:DespawnInterior(ApartmentObject)
                        ApartmentObject = nil
                        Offsets = nil
                ...
                if #(GetEntityCoords(PlayerPedId()) - Config.Apartment['Pos']) < Config.Apartment['Distance'] then
                    if not ShowingInteraction then
                        exports['Ethnic-ui']:SetInteraction("[E] Enter Apartment; [G] More Options")
                        ShowingInteraction = true
                    end
                    
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent('Ethnic-apartment/client/enter-apartment')
                    end

                    if IsControlJustReleased(0, 47) then
                        TriggerEvent('Ethnic-apartment/client/apartment-options')
                    end
                else
                    if ShowingInteraction then
                        exports['Ethnic-ui']:HideInteraction()
...
RegisterNetEvent('Ethnic-apartment/client/spawn-apartment', function(NewChar)
    DoScreenFadeOut(250)

    while not IsScreenFadedOut() do Citizen.Wait(1) end

    local Interior = exports['Ethnic-interiors']:CreateInterior('gabz_pinkcage', vector3(Config.Apartment['Pos'].x, Config.Apartment['Pos'].y, Config.Apartment['Pos'].z - 35.0), false)
    if NewChar then 
        EventsModule.TriggerServer('Ethnic-items/server/receive-first-items') 
    else
        EventsModule.TriggerServer('Ethnic-base/server/bucketmanager/set-routing-bucket') -- Set new routing bucket for apartments.
    end
    ...
    if not Interior then
        ...
        exports['Ethnic-ui']:Notify("apartment-error", "Something went wrong whilst loading the apartment, you've been spawned outside..", "error")
        goto Skip
    end
...
RegisterNetEvent('Ethnic-apartment/client/enter-apartment', function(Data)
    local PlayerData = PlayerModule.GetPlayerData()
    if Data ~= nil and Data.RoomId then RoomId = Data.RoomId else RoomId = nil end
    if RoomId ~= nil and (not GetApartmentLockStatus(RoomId)) then exports['Ethnic-ui']:Notify("apartment", "The apartment is locked..", "error") return end
    ...
    if LockdownStatus and ((PlayerData.Job.Name ~= 'police' and PlayerData.Job.Name ~= 'judge') or not PlayerData.Job.Duty) then
        return exports['Ethnic-ui']:Notify("apartment", "Apartment on lockdown, only Police or DOJ may enter.", "error")
    end

    DoScreenFadeOut(250)
    TriggerEvent('Ethnic-assets/client/toggle-items', true)
    ...
    EventsModule.TriggerServer('Ethnic-base/server/bucketmanager/set-routing-bucket', RoomId)
...
RegisterNetEvent('Ethnic-apartment/client/leave-apartment', function()
    DoScreenFadeOut(250)
    TriggerEvent('Ethnic-assets/client/play-door-animation')
    TriggerEvent('Ethnic-ui/client/play-sound', 'door-open', 0.75)
    while not IsScreenFadedOut() do Citizen.Wait(1) end
    ...
    EventsModule.TriggerServer('Ethnic-base/server/bucketmanager/set-routing-bucket', 0)
...
RegisterNetEvent('Ethnic-apartment/client/apartment-options', function()
    local PlayerData = PlayerModule.GetPlayerData()
    ...
    local MenuItems = {
        {
            ...
            Data = {['Event'] = 'Ethnic-apartment/server/toggle-lock', ['Type'] = 'Server', ['StateId'] = PlayerData.CitizenId},
            Type = 'Click',
        },
        {
            Title = 'Apartments',
    ...
    if (PlayerData.Job.Name == 'police' or PlayerData.Job.Name == 'judge') and PlayerData.Job.Duty then
        table.insert(MenuItems, {
            ...
            Data = {['Event'] = 'Ethnic-apartments/client/lockdown-apartment', ['Type'] = 'Client'},
            Type = 'Click',
        })
    end
    
    local Apartments = CallbackModule.SendCallback('Ethnic-apartment/server/get-unlocked-apps')
        ...
        MenuData['SecondMenu'] = {
            {
                ...
                ['Type'] = 'Click',
                ['Data'] = {['Event'] = 'Ethnic-apartment/client/enter-apartment', ['Type'] = 'Client', ['RoomId'] = v }
            },
        }
...
RegisterNetEvent('Ethnic-apartments/client/lockdown-apartment', function(Data)
    Citizen.SetTimeout(250, function()
        local Data = {{ Name = 'state_id', Label = 'State Id', Icon = 'fas fa-id-card' }}
        local LockdownInput = exports['Ethnic-ui']:CreateInput(Data)
        if LockdownInput['state_id'] then
            TriggerServerEvent('Ethnic-apartment/server/toggle-lockdown', LockdownInput['state_id'])
        else
            exports['Ethnic-ui']:Notify("apartment", "No valid state id..", "error")
        end
    end)
end)
...
function GetApartmentLockStatus(RoomId)
    local LockStatus = CallbackModule.SendCallback('Ethnic-apartment/server/get-lockstatus-by-roomid', RoomId)
    return LockStatus
end

function GetApartmentLockdownStatus(RoomId)
    local LockStatus = CallbackModule.SendCallback('Ethnic-apartment/server/get-lockdown-by-roomid', RoomId)
    return LockStatus
end
