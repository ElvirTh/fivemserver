...
RegisterNetEvent('Ethnic-ui/client/send-emergency-alert', function(AlertData, ForBoth, SendLocation)
    ...
    if Player.Job ~= nil and (Player.Job.Name == 'police' or (ForBoth and Player.Job.Name == 'ems')) and Player.Job.Duty then
        ...
        if AlertData.AlertType == 'alert-panic' then
            EventsModule.TriggerServer('Ethnic-ui/server/play-sound-on-entity', '10-1314', GetPlayerServerId(PlayerId()), 3000, 15.0, nil, true)
        elseif AlertData.AlertType == 'alert-red' then
            EventsModule.TriggerServer('Ethnic-ui/server/play-sound-on-entity', 'HighPrioCrime', GetPlayerServerId(PlayerId()), 3000, 15.0, nil, true)
        else
...
RegisterNetEvent('Ethnic-ui/client/open-alerts-menu', function(OnPress)
...
RegisterNetEvent('Ethnic-ui/client/ui-reset', function()
    exports['Ethnic-ui']:SendUIMessage("Police", "CloseDispatch")
end)

-- [ Functions ] --

AddInitialize(function()
    KeybindsModule.Add("alertMenu", 'Services', 'Dispatch Menu', '', false, 'Ethnic-ui/client/open-alerts-menu')
end)
...
RegisterNUICallback('Police/SetWaypoint', function(Data, Cb)
    ...
    exports['Ethnic-ui']:Notify("police-waypoint", "Waypoint set.", "primary")
    Cb('Ok')
end)

RegisterNUICallback('Police/GetOnDutyPeople', function(Data, Cb)
    local Data = CallbackModule.SendCallback('Ethnic-ui/server/police/get-duty-people')
    Cb(Data)
end)

RegisterNUICallback('Police/GetDispatchData', function(Data, Cb)
    local Data = CallbackModule.SendCallback('Ethnic-ui/server/police/get-dispatch-data')
    local DispatchData = {
