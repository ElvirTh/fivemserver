...
CloseBankContainer = function() {
    $.post('https://Ethnic-financials/Financials/Close', JSON.stringify({IsAtBank: IsAtBank}));
...
SetCashBalance = function() {
    $.post('https://Ethnic-financials/Financials/GetCash', JSON.stringify({}), function(Amount) {
...
RefreshBankingData = function() {
    $.post('https://Ethnic-financials/Financials/GetAccounts', JSON.stringify({}), function(AccountData) {
        ...
        $('.bank-banking-data').hide(0, function() {
            ...
            setTimeout(function() {
                if (CurrentContextType == 'Withdraw') {
                    $.post('https://Ethnic-financials/Financials/Withdraw', JSON.stringify({Amount: Math.floor(Amount), Reason: Reason, AccountId: CurrentContextData.AccountId}), function(IsDone) {
                        if (IsDone) {
                            CloseBankContext();
                            setTimeout(function() {
                                RefreshBankingData();
                            ...
                            $.post('https://Ethnic-financials/Financials/ErrorSound');
                            CloseBankContext();
                        }
                    });
                } else if (CurrentContextType == 'Deposit') {
                    $.post('https://Ethnic-financials/Financials/Deposit', JSON.stringify({Amount: Math.floor(Amount), Reason: Reason, AccountId: CurrentContextData.AccountId}), function(IsDone) {
                        if (IsDone) {
                            CloseBankContext();
                            setTimeout(function() {
                                RefreshBankingData();
                    ...
                    $.post('https://Ethnic-financials/Financials/Transfer', JSON.stringify({Amount: Math.floor(Amount), Reason: Reason, ToAccountId: TransferNumber.toString(), AccountId: CurrentContextData.AccountId}), function(IsDone) {
                        if (IsDone) {
                            CloseBankContext();
                            setTimeout(function() {
                                RefreshBankingData();
