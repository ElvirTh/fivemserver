...
RegisterNetEvent('Ethnic-spawn/client/open-spawn-selector', function()
    TriggerEvent('Ethnic-weathersync/client/set-default-weather', 17)
    ...
    Citizen.SetTimeout(750, function()
        ...
        -- Own Houses
        local Houses = exports['Ethnic-housing']:GetHouseConfig()
        ...
        -- Keyholder Houses
        local Result = CallbackModule.SendCallback('Ethnic-houses/server/get-houses-keyholder', PlayerData.CitizenId)
...
RegisterNUICallback('Spawn/SelectSpawn', function(Data, Cb)
    ...
    if Data.Id == 'last_location' then
        local PlayerData = exports['Ethnic-base']:FetchModule('Player').GetPlayerData()
        if PlayerData.Position ~= nil then
            SpawnData = {
                Type = 'Location',
                Coords = {
    ...
    local PlayerData = exports['Ethnic-base']:FetchModule('Player').GetPlayerData()
    while PlayerData == nil do
        PlayerData = exports['Ethnic-base']:FetchModule('Player').GetPlayerData()
	Wait(100)
    end
    ...
    elseif SpawnData.Type == 'Apartment' then
        ...
        Citizen.SetTimeout(500, function()
            FreezeEntityPosition(PlayerPedId(), false)
            Citizen.Wait(2500)
            TriggerEvent('Ethnic-apartment/client/spawn-apartment', false)
        end)
    elseif SpawnData.Type == 'Jail' then
        ...
        Citizen.SetTimeout(500, function()
            FreezeEntityPosition(PlayerPedId(), false)
            Citizen.Wait(1250)
            TriggerEvent('Ethnic-police/client/enter-jail', PlayerData.MetaData['Jail'], 0, true)
        end)
    elseif SpawnData.Type == 'House' then
        EventsModule.TriggerServer("Ethnic-base/server/bucketmanager/set-routing-bucket", 0)
        local HouseData = exports['Ethnic-housing']:GetHouseById(string.sub(Data.Id, 7, #Data.Id))
        ...
        Citizen.SetTimeout(2500, function()
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('Ethnic-housing/client/enter-house', string.sub(Data.Id, 7, #Data.Id))
        end)
    end
