                        ...
                        if DoesEntityExist(v.tableObject) then
                            ...
                            if GetDistanceBetweenCoords(PlayerCoords, Offset.x, Offset.y, Offset.z, true) <= 1.5 then
                                ...
                                if not ShowingInteraction then
                                    ...
                                    exports['Ethnic-ui']:SetInteraction('[E] - Slots ($'..v.data.bet..'.00)', HasCasinoMembership() and 'success' or 'error')
                                end
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    local IsSeatFree = CallbackModule.SendCallback('Ethnic-casino/server/slots/check-seat', v.index)
                                    if IsSeatFree then
                                        local HasEnoughBet = CallbackModule.SendCallback('Ethnic-casino/server/slots/check-bet', v.data.bet)
                                        if HasEnoughBet then
                                            ...
                                            exports['Ethnic-ui']:Notify("casino-slots-bet", "You don't have enough chips to bet on this slot machine..", "error")
                                        end
                                    end
                                end
                                break
...
RegisterNetEvent("Ethnic-casino/client/slots/do-action", function(Data)
...
RegisterNetEvent("Ethnic-hospital/client/on-player-death", function()
...
RegisterNetEvent('Ethnic-casino/client/slots/start', function(Index, TickRate)
...
CreateSlots = function(index, data)
	...
	self.StartPlaying = function(State)
		if State then
                        ...
                        if not ShowingContext then
                            ...
                            local MenuData = {
                                ...
                                },
                                {
                                    ...
                                    ['Data'] = { ['Event'] = 'Ethnic-casino/client/slots/do-action', ['Type'] = 'Client', ['Action'] = 'Leave', ['Index'] = self.index, ['Offset'] = Offset, }
                                },
                                {
                                    ['Title'] = 'Spin',
                                    ['Desc'] = 'Spin the slot machine.',
                                    ['Data'] = { ['Event'] = 'Ethnic-casino/client/slots/do-action', ['Type'] = 'Client', ['Action'] = 'Spin', ['Index'] = self.index, ['Offset'] = Offset, }
                                },
                            }
                            exports['Ethnic-ui']:OpenContext({ ['MainMenuItems'] = MenuData,
                                ['CloseEvent'] = { ['Event'] = 'Ethnic-casino/client/slots/do-action', ['Type'] = 'Client', ['Action'] = 'Closed', }
                            })
                        end
                    end
                    OpenSlotsContext()
			...
			TriggerServerEvent('Ethnic-casino/server/slots/clear-seat', self.index)
            DoingSlotsAction = false
		end
	end
	...
	self.Spin = function(tickRate)
		...
		TriggerServerEvent('Ethnic-casino/server/slots/check-for-win', self.index, tickRate, self.data)
	end
...
function StartSpinning(Index, Data)
    if Slots[Index] then
        TriggerServerEvent('Ethnic-casino/server/slots/start', Index, Data)
    end
end
