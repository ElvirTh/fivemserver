...
$(document).on('click', '.preferences-pages .preferences-pages-button', function(e){
    ...
    $.post("https://Ethnic-ui/Preferences/SavePreferences", JSON.stringify({
        PreferenceData: ReturnValue,
    }));
});

$(document).on('click', '.preferences-reset-mumble', function(e){
    $.post("https://Ethnic-voice/ResetMumble");
});
...
$(document).on({
    keydown: function(e) {
        if (e.keyCode == 27 && HasPreferencesOpen) {
            $.post("https://Ethnic-ui/Preferences/ToggleVisibity", JSON.stringify({
                Bool: false,
            }));
        }
    },
