...
function DoPermsAction(Action, CommandId, Group)
    ...
    if Action == 'add' then
        if CommandId == 'all' then
            ...
            exports['Ethnic-ui']:Notify("perms-added", Lang:t('info.perms_added_all'), 'success')
        end
    elseif Action == 'remove' then
        if CommandId == 'all' then -- Remove From Every Table
            if not Group then return exports['Ethnic-ui']:Notify("perms-group-exist", Lang:t('info.perms_group_exist'), 'error') end
            ...
            exports['Ethnic-ui']:Notify("perms-removed", Lang:t('info.perms_removed_all'), 'error')
        end
    end
    for p, q in pairs(Config.CommandList) do
        ...
        for j=1, #Command['Items'] do
            ...
            if not Item['UseKVPGroups'] then return exports['Ethnic-ui']:Notify("perms-group-exist", Lang:t('info.perms_disabled'), 'error') end
            if Groups ~= nil then
                if Action == 'add' then
                    if CommandId ~= 'all' and Groups[CommandId] ~= nil and Item['Id'] == CommandId then 
                        for k, v in pairs(Groups[CommandId]) do -- Check if Group already exists
                            if v == Group then 
                                return exports['Ethnic-ui']:Notify("perms-exist", Lang:t('info.perms_exist'), 'error')
                            end
                        end
                        table.insert(Groups[CommandId], Group)
                        exports['Ethnic-ui']:Notify("perms-added", Lang:t('info.perms_added'), 'success')
                    end
                elseif Action == 'remove' then
                    if CommandId ~= 'all' and Groups[CommandId] ~= nil and Item['Id'] == CommandId then
                        for k, v in pairs(Groups[CommandId]) do
                            if k == #Groups[CommandId]+1 and v ~= Group then return exports['Ethnic-ui']:Notify("perms-not-exist", Lang:t('info.perms_not_exist'), 'error') end
                            if v == Group then  
                                ...
                                exports['Ethnic-ui']:Notify("perms-removed", Lang:t('info.perms_removed'), 'error')
                            end
                        end
                    end
                elseif Action == 'list' then
                    if Groups[CommandId] ~= nil then 
                        if Item['Id'] == CommandId then
                            return exports['Ethnic-ui']:Notify("perms-list", Lang:t('info.perms_list', { command = CommandId, groups = json.encode(Groups[CommandId]) }), 'success')
                        end
                    else
                        return exports['Ethnic-ui']:Notify("perms-command-exist", Lang:t('info.perms_command_exist'), 'error')
                    end
                end
            end
        end
...
function GetBusinesses()
    ...
    local Prom = promise:new()
    local BusinessList = {}
    local Businesses = CallbackModule.SendCallback('Ethnic-business/server/get-businesses')
