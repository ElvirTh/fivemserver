...
Citizen.CreateThread(function()
    ...
    CommandsModule.Add("m0", "Take mask off..", {}, false, function(source, args)
        TriggerClientEvent('Ethnic-clothing/client/take-off-face-wear', source, 'Mask', nil, nil, true)
    end)
    ...
    CommandsModule.Add('skin', 'Clothing Menu', {}, false, function(source, args)
        ...
        TriggerClientEvent('Ethnic-clothing/client/open-menu', source)
    end, "admin")
...
RegisterNetEvent("Ethnic-clothing/server/receive-clothing", function(Type, Prop, Texture)
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    if Type == 'Mask' then
...
RegisterNetEvent("Ethnic-clothing/server/steal-shoes", function(ServerId, ToShoes)
    local src = source
    local Player = PlayerModule.GetPlayerBySource(src)
    Player.Functions.AddItem("weapon_shoe", 1, false, false, true)
    TriggerClientEvent('Ethnic-clothing/client/take-off-face-wear', ServerId, 'Shoes', ToShoes)
end)
