...
function InitCityZones() 
    Citizen.CreateThread(function()
        exports['Ethnic-ui']:AddEyeEntry("cityhall-options", {
            Type = 'Entity',
            EntityType = 'Ped',
            SpriteDistance = 5.0,
            ...
            Options = {
                {
                    ...
                    EventType = 'Server',
                    EventName = 'Ethnic-cityhall/server/purchase-id',
                    EventParams = {},
        ...
        exports['Ethnic-ui']:AddEyeEntry("judge-gavel", {
            Type = 'Zone',
            SpriteDistance = 5.0,
            Distance = 4.0,
            ZoneData = {
            ...
            Options = {
                {
                    ...
                    Label = 'Gavel Hit',
                    EventType = 'Client',
                    EventName = 'Ethnic-cityhall/client/gavel-hit',
                    EventParams = {},
                    Enabled = function(Entity)
        ...
        exports['Ethnic-ui']:AddEyeEntry("judge-check-in", {
            Type = 'Zone',
            SpriteDistance = 5.0,
            Distance = 4.0,
            ZoneData = {
...
RegisterNetEvent('Ethnic-polyzone/client/enter-polyzone', function(PolyData, Coords)
    if PolyData.name == 'metaldetect_stuff' then
        if not NearAction then
            ...
            if not ShowingInteraction then
                ...
                exports['Ethnic-ui']:SetInteraction('[E] Retrieve Stuff', 'primary')
            end
            Citizen.CreateThread(function()
                while NearAction do
                    Citizen.Wait(4)
                    if IsControlJustReleased(0, 38) then
                        ...
                        if exports['Ethnic-inventory']:CanOpenInventory() then
                            ...
                            EventsModule.TriggerServer('Ethnic-inventory/server/open-other-inventory', "cityhall-"..MyStateId, 'Stash', 45, 0.0)
                        end
                    end
                end
            end)
    ...
    elseif PolyData.name == 'public_mdw' then
        if not NearAction then
            ...
            if not ShowingInteraction then
                ShowingInteraction = true
                exports['Ethnic-ui']:SetInteraction('[E] Public MDW', 'primary')
            end
            Citizen.CreateThread(function()
                while NearAction do
                    Citizen.Wait(4)
                    if IsControlJustReleased(0, 38) then
                        exports['Ethnic-ui']:HideInteraction()
                        TriggerEvent('Ethnic-mdw/client/open-MDW')
                    end
                end
            end)
        end
