...
Citizen.CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.LoggedIn then
            local InsideInterior = exports['Ethnic-interiors']:IsInsideInterior()
...
RegisterNetEvent('Ethnic-weathersync/client/sync', function(ConfigData)
    if CanSync then
...
RegisterNetEvent('Ethnic-weathersync/client/set-default-weather', function(Hour)
    Citizen.SetTimeout(500, function()
...
function GetWeatherConfig()
    local Config = CallbackModule.SendCallback("Ethnic-weathersync/server/get-data")
    return Config
end
