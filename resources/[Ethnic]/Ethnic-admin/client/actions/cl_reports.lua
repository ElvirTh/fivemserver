...
RegisterNetEvent("mc-admin/client/close-report", function()
    if HasReport() then
        ...
        if ReportId then
            ...
            if Success then
                exports['Ethnic-ui']:Notify("report-closed-self", Lang:t('info.report_closed_self'), 'success')
            end
        end
    else
        exports['Ethnic-ui']:Notify("report-not-open", Lang:t('info.report_not', { chatcommand = Config.Commands['ReportNew'] }), 'error')
    end
end)
...
RegisterNetEvent("mc-admin/client/reply-report", function(Message, Time)
    if HasReport() then
        ...
        if ReportId then
            ...
            if Success then
                exports['Ethnic-ui']:Notify("reply-success", Lang:t('info.report_reply_success'), 'success')
            else
                exports['Ethnic-ui']:Notify("reply-error", Lang:t('info.report_reply_error'), 'error')
            end
        end
    else
        exports['Ethnic-ui']:Notify("report_not", Lang:t('info.report_not', { chatcommand = Config.Commands['ReportNew'] }), 'error')
    end
end)
